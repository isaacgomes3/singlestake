...
const sportingbet2fConfigStatus = document.getElementById("sportingbet2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, sportingbet2fConfig) {
  ...
  const noGale =
    sportingbet2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = sportingbet2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? sportingbet2fConfig?.enabledPairIds,
  );
  ...
  if (!on) {
    ice2fStatus.textContent = "Parada — clique Ligar para seguir a mesa 227 via DGA.";
    return;
  }
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.sportingbet2fAutopilot, mode, status?.sportingbet2fConfig);

  const cfg = status?.sportingbet2fConfig ?? {};
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa Sportingbet 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Roulette 1 no Sportingbet num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa Sportingbet 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-sportingbet2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-sportingbet2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-sportingbet2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.sportingbet2fConfig ?? {};
  await send("set-sportingbet2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-sportingbet2f-config", { config });
  if (sportingbet2fConfigStatus) {
    sportingbet2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
