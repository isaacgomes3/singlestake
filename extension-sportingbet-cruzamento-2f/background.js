importScripts("shared.js", "sportingbet2f-engine.js", "dga-hub.js", "sportingbet2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogSportingbet2fAutopilotEnabled: false });
  }
  void ensureSportingbet2fPanelOnOpenTabs();
});
...
let sportingbet2fPanelWindowId = null;

async function openOrFocusSportingbet2fPanel() {
  if (sportingbet2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(sportingbet2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      sportingbet2fPanelWindowId = null;
    }
  }
  ...
  sportingbet2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusSportingbet2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === sportingbet2fPanelWindowId) sportingbet2fPanelWindowId = null;
});

const SPORTINGBET2F_DEFAULT_TABLE_ID = 227;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const SPORTINGBET2F_CALIB_SITE_KEY = "sportingbet.bet.br|pragmatic-roulette";

function isSportingbet2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "sportingbet.bet.br" || host.endsWith(".sportingbet.bet.br") || host.endsWith(".br4.bet.br");
  } catch {
    return /sportingbet\.bet\.br/i.test(url);
  }
}

function isSportingbet2fRoulettePageUrl(url) {
  if (!isSportingbet2fHost(url)) return false;
  try {
    const u = new URL(url);
...
async function ensureSportingbet2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isSportingbet2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "sportingbet2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-sportingbet2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureSportingbet2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isSportingbet2fRoulettePageUrl(tab.url)) {
      await ensureSportingbet2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isSportingbet2fRoulettePageUrl(url)
  ) {
    registerMesaTab(SPORTINGBET2F_DEFAULT_TABLE_ID, tabId);
    void ensureSportingbet2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-sportingbet2f-autopilot") {
    void SinglestakeSportingbet2fSignalRunner.setSportingbet2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureSportingbet2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-sportingbet2f-autopilot") {
    void SinglestakeSportingbet2fSignalRunner.getSportingbet2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-sportingbet2f-stats") {
    void SinglestakeSportingbet2fSignalRunner.resetSportingbet2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-sportingbet2f-config") {
    void SinglestakeSportingbet2fSignalRunner.getSportingbet2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-sportingbet2f-config") {
    void SinglestakeSportingbet2fSignalRunner.setSportingbet2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isSportingbet2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(SPORTINGBET2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isSportingbet2fRoulettePageUrl(t.url)) ??
  ...
  const sportingbet2fAutopilot = await SinglestakeSportingbet2fSignalRunner.getSportingbet2fAutopilotStatus();
  const sportingbet2fConfig = await SinglestakeSportingbet2fSignalRunner.getSportingbet2fConfigForPopup();
  return {
    ok: true,
    mode,
    sportingbet2fAutopilot,
    sportingbet2fConfig,
...
function isSportingbet2fCrossingContext(context) {
  return context?.strategy === "sportingbet2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isSportingbet2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.SPORTINGBET2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isSportingbet2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.SPORTINGBET2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isSportingbet2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "sportingbet2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isSportingbet2fSignal =
    isSportingbet2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("sportingbet2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isSportingbet2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isSportingbet2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isSportingbet2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isSportingbet2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isSportingbet2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeSportingbet2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeSportingbet2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (SPORTINGBET2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isSportingbet2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Roulette 1 no Sportingbet num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "sportingbet.bet.br" || host.endsWith(".sportingbet.bet.br")) {
      return SPORTINGBET2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[SPORTINGBET2F_CALIB_SITE_KEY]) {
    return { siteKey: SPORTINGBET2F_CALIB_SITE_KEY, site: sites[SPORTINGBET2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: SPORTINGBET2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isSportingbet2fHost(url)) {
    for (const [key, site] of Object.entries(sites)) {
      if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
    }
    if (sites[SPORTINGBET2F_CALIB_SITE_KEY]) return { siteKey: SPORTINGBET2F_CALIB_SITE_KEY, site: sites[SPORTINGBET2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isSportingbet2fHost(tab.url) ? SPORTINGBET2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === SPORTINGBET2F_CALIB_SITE_KEY && isSportingbet2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isSportingbet2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isSportingbet2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isSportingbet2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeSportingbet2f !== "undefined" && SinglestakeSportingbet2f.SPORTINGBET2F_MESA_URL) ||
          "https://sportingbet.bet.br/",
        mesaProvider: null,
        currentTableId: SPORTINGBET2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Sportingbet encontrada. Abra Roulette 1 no Sportingbet num separador normal do Chrome (não neste painel).",
    };
  }
...
SinglestakeSportingbet2fSignalRunner.initSportingbet2fSignalRunner(handleBridgePayload);
