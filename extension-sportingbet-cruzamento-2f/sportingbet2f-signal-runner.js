...
function knownPairIds() {
  const known = SinglestakeSportingbet2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeSportingbet2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeSportingbet2f !== "undefined"
      ? SinglestakeSportingbet2f.applyIce2fEnabledPairIds
        ? SinglestakeSportingbet2f
        : SinglestakeSportingbet2f.default
      : null;
...
async function getSportingbet2fAutopilotStatus() {
  ...
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeSportingbet2f?.ice2fPairLabel === "function"
        ? SinglestakeSportingbet2f.ice2fPairLabel(id)
...
function initSportingbet2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeSportingbet2fBridgeHandler = handleBridgePayload;
  void readSportingbet2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes[STORAGE_SPORTINGBET2F_CONFIG] && bridgeHandler) {
      void readSportingbet2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readSportingbet2fAutopilotEnabled().then((on) => {
