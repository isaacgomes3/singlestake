...
var SinglestakeSportingbet2f = (() => {
  ...
  // extension-sportingbet-cruzamento-2f/sportingbet2f-strategy-entry.ts
  var sportingbet2f_strategy_entry_exports = {};
  __export(sportingbet2f_strategy_entry_exports, {
    ...
    SPORTINGBET2F_MAX_GALES: () => SPORTINGBET2F_MAX_GALES,
    SPORTINGBET2F_MESA_URL: () => SPORTINGBET2F_MESA_URL,
    SPORTINGBET2F_TABLE_ID: () => SPORTINGBET2F_TABLE_ID,
    ...
    createSportingbet2fEngine: () => createSportingbet2fEngine,
    default: () => sportingbet2f_strategy_entry_default,
  ...
  // extension-sportingbet-cruzamento-2f/sportingbet2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var SPORTINGBET2F_TABLE_ID = 227;
  var SPORTINGBET2F_MESA_URL = "";
  var SPORTINGBET2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var SPORTINGBET2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var SPORTINGBET2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var SPORTINGBET2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of SPORTINGBET2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createSportingbet2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && SPORTINGBET2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && SPORTINGBET2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && SPORTINGBET2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && SPORTINGBET2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && SPORTINGBET2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && SPORTINGBET2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && SPORTINGBET2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = SPORTINGBET2F_MESA_URL) {
      ...
      const signalId = `sportingbet2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `Sportingbet 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `Sportingbet 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `Sportingbet 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: SPORTINGBET2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? SPORTINGBET2F_MESA_URL,
          mesaProvider: "outro",
    ...
    return {
      tableId: SPORTINGBET2F_TABLE_ID,
  ...
  var api = {
    SPORTINGBET2F_TABLE_ID,
    SPORTINGBET2F_MESA_URL,
    SPORTINGBET2F_MAX_GALES,
    ICE_2F_BET_DELAY_MS,
    ...
    createSportingbet2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeSportingbet2f = api;
  }
  var sportingbet2f_strategy_entry_default = api;
  return __toCommonJS(sportingbet2f_strategy_entry_exports);
})();
