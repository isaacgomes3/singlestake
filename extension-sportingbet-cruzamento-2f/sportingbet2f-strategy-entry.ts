/**
 * Bundle entry — motor Sportingbet Cruzamento 2F para extensão Chrome.
 */
...
export const SPORTINGBET2F_TABLE_ID = 227;
export const SPORTINGBET2F_MESA_URL =
  "";
export const SPORTINGBET2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const SPORTINGBET2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const SPORTINGBET2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const SPORTINGBET2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));

export type Sportingbet2fPersistedMachine = {
...
function parsePersistedWatch(
  raw: Sportingbet2fPersistedMachine["watch"],
...
export type CreateSportingbet2fEngineOptions = {
  maxRecovery?: number;
  initialStats?: RotatingRoomSessionStats | null;
  initialMachine?: Sportingbet2fPersistedMachine | null;
};
...
function pendingRecoveryFromSaved(saved: Sportingbet2fPersistedMachine): number {
  const stored =
...
export type Sportingbet2fEngineSpinResult = {
...
export function createSportingbet2fEngine(options: CreateSportingbet2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && SPORTINGBET2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        SPORTINGBET2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && SPORTINGBET2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
      ...(restoredWatch ? { watch: restoredWatch } : {}),
    };
  }
  let pendingRestore: Sportingbet2fPersistedMachine | null = options.initialMachine ?? null;
  let stats =
  ...
  function runTick(): Sportingbet2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && SPORTINGBET2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && SPORTINGBET2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        SPORTINGBET2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        SPORTINGBET2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = SPORTINGBET2F_MESA_URL) {
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `Sportingbet 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `Sportingbet 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `Sportingbet 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "sportingbet2fcruzamento" as const,
...
// named exports; sem isto SinglestakeSportingbet2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeSportingbet2f?: typeof api }).SinglestakeSportingbet2f = api;
}
