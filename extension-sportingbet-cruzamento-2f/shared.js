...
const GOG = {
  ...
  SPORTINGBET2F_FACTOR_BRIDGE_STAGGER_MS: 5,
  ...
  SPORTINGBET2F_DOUBLE_CLICK_STAGGER_MS: 20,
...
function recoveryFromContext(context) {
  ...
  if (typeof signalId === "string" && signalId.trim()) {
    ...
    // Prefixo `rN` (Sportingbet 2F): ice2f:…:r2
...
/** Unidades Sportingbet 2F: 1 · 2 · 4 · 8 · 16 · 32 · 64 · 128 · 256 (entrada + 8 gales). */
const SPORTINGBET2F_STAKE_UNITS = [1, 2, 4, 8, 16, 32, 64, 128, 256];

function sportingbet2fStakeUnitsForRecovery(recovery) {
  const r = Math.max(0, Math.floor(recovery));
  return SPORTINGBET2F_STAKE_UNITS[Math.min(r, SPORTINGBET2F_STAKE_UNITS.length - 1)] ?? 1;
}

/** Gale 3 (8 unidades) — referência de ritmo de digitação ICE 3F. */
const SPORTINGBET2F_GALE3_REFERENCE_UNITS = 8;

/** Intervalo entre cliques de ficha — 2F (gale 4/5 fixos). */
function sportingbet2fChipClickStaggerMs(recovery) {
  const r = Math.max(0, Math.floor(recovery));
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isSportingbet2fCrossingContext(context) && context?.singleFactorMode !== true) {
    return sportingbet2fChipClickStaggerMs(r);
  }
...
function panelSignalToBridge(data) {
  ...
  return {
    ...
    context: {
      ...
      signalId,
      stakeAmount: 0.5 * sportingbet2fStakeUnitsForRecovery(recovery),
