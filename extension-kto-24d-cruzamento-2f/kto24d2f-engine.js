...
var SinglestakeKto24d2f = (() => {
  ...
  // extension-kto-24d-cruzamento-2f/kto24d2f-strategy-entry.ts
  var kto24d2f_strategy_entry_exports = {};
  __export(kto24d2f_strategy_entry_exports, {
    ...
    KTO24D2F_MAX_GALES: () => KTO24D2F_MAX_GALES,
    KTO24D2F_MESA_URL: () => KTO24D2F_MESA_URL,
    KTO24D2F_TABLE_ID: () => KTO24D2F_TABLE_ID,
    ...
    createKto24d2fEngine: () => createKto24d2fEngine,
    default: () => kto24d2f_strategy_entry_default,
  ...
  // extension-kto-24d-cruzamento-2f/kto24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var KTO24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var KTO24D2F_MESA_URL = "https://www.kto.bet.br/app/cassino/game/24d-spin/";
  var KTO24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var KTO24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var KTO24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var KTO24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of KTO24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createKto24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && KTO24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && KTO24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && KTO24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && KTO24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && KTO24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && KTO24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && KTO24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = KTO24D2F_MESA_URL) {
      ...
      const signalId = `kto24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: KTO24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? KTO24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createKto24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeKto24d2f = api;
  }
  var kto24d2f_strategy_entry_default = api;
  return __toCommonJS(kto24d2f_strategy_entry_exports);
})();
