...
const ice2fStatus = document.getElementById("kto24d2fStatus");
...
const ice2fOnBtn = document.getElementById("kto24d2fOn");
const ice2fOffBtn = document.getElementById("kto24d2fOff");
const ice2fResetBtn = document.getElementById("kto24d2fReset");
const ice2fCasinoId = document.getElementById("kto24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("kto24d2fSave");
const kto24d2fConfigStatus = document.getElementById("kto24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, kto24d2fConfig) {
  ...
  const noGale =
    kto24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = kto24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? kto24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.kto24d2fAutopilot, mode, status?.kto24d2fConfig);

  const cfg = status?.kto24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "kto.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.kto24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (kto24d2fConfigStatus) {
    kto24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
