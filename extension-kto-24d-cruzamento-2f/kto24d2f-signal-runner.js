/** Autopilot KTO — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_KTO24D2F_AUTOPLAY = "gogKto24d2fAutopilotEnabled";
const STORAGE_KTO24D2F_CONFIG = "gogKto24d2fConfig";
const STORAGE_KTO24D2F_STATUS = "gogKto24d2fAutopilotStatus";
const STORAGE_KTO24D2F_STATS = "gogKto24d2fAutopilotSessionStats";
const STORAGE_KTO24D2F_MACHINE = "gogKto24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeKto24d2f['createKto24d2fEngine']>|null} */
let engine = null;
...
function bumpKto24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function kto24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeKto24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeKto24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const KTO24D2F_DEFAULTS = {
  tableId: SinglestakeKto24d2f?.KTO24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeKto24d2f?.KTO24D2F_MESA_URL ??
    "https://www.kto.bet.br/app/cassino/game/24d-spin/",
...
function knownPairIds() {
  const known = SinglestakeKto24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeKto24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeKto24d2f !== "undefined"
      ? SinglestakeKto24d2f.applyIce2fEnabledPairIds
        ? SinglestakeKto24d2f
        : SinglestakeKto24d2f.default
      : null;
...
async function readKto24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_AUTOPLAY]);
  return data[STORAGE_KTO24D2F_AUTOPLAY] === true;
}

async function readKto24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_CONFIG]);
  const stored = data[STORAGE_KTO24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...KTO24D2F_DEFAULTS,
      maxRecoveryPreference: KTO24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? KTO24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? KTO24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : KTO24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? KTO24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : KTO24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : KTO24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : KTO24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : KTO24D2F_DEFAULTS.currency,
    noGale,
...
async function readKto24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_MACHINE]);
  const raw = data[STORAGE_KTO24D2F_MACHINE];
...
async function persistKto24d2fMachineState(machine) {
...
async function clearKto24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_KTO24D2F_MACHINE]);
}

async function readKto24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_STATS]);
  const raw = data[STORAGE_KTO24D2F_STATS];
...
async function persistKto24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_KTO24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, kto24d2fIdleIndicationPatch());
  }
  await writeKto24d2fStatus(patch);
}

async function resetKto24d2fStats() {
  const cfg = await readKto24d2fConfig();
  const empty =
    globalThis.SinglestakeKto24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistKto24d2fStats(empty, cfg.maxRecovery);
  await clearKto24d2fMachineState();
...
async function getKto24d2fConfigForPopup() {
  return readKto24d2fConfig();
}

async function setKto24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_CONFIG]);
  const stored =
    data[STORAGE_KTO24D2F_CONFIG] && typeof data[STORAGE_KTO24D2F_CONFIG] === "object"
      ? data[STORAGE_KTO24D2F_CONFIG]
      : {};
  const prev = await readKto24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_KTO24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearKto24d2fMachineState();
    await writeKto24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readKto24d2fConfig()) };
}

async function writeKto24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_STATUS]);
  const prev = data[STORAGE_KTO24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_KTO24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeKto24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistKto24d2fMachineState(live.machine);
    await writeKto24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readKto24d2fConfig();
    ...
    await writeKto24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readKto24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeKto24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeKto24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeKto24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeKto24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeKto24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `kto24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistKto24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistKto24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpKto24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeKto24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeKto24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeKto24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeKto24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeKto24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startKto24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeKto24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readKto24d2fAutopilotEnabled();
  if (!enabled) {
    await writeKto24d2fStatus({ running: false, reason: "autopilot KTO 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeKto24d2f?.createKto24d2fEngine) {
    await writeKto24d2fStatus({
      running: false,
      reason: "kto24d2f-engine.js em falta — corra npm run extension:kto24d2f:build",
    });
    return;
  }

  stopKto24d2fAutopilot();

  const cfg = await readKto24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readKto24d2fStats(cfg.maxRecovery);
  const savedMachine = await readKto24d2fMachineState();
  engine = SinglestakeKto24d2f.createKto24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeKto24d2fStatus({ log: msg }),
    onStatus: (status) => void writeKto24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readKto24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readKto24d2fConfig().then((cfgNow) =>
...
function stopKto24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpKto24d2fIndicationEpoch();
  void writeKto24d2fStatus(kto24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setKto24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_KTO24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startKto24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeKto24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeKto24d2fBridgeHandler) {
      await writeKto24d2fStatus({
        running: false,
  ...
  const status = await getKto24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getKto24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_KTO24D2F_STATUS, STORAGE_KTO24D2F_AUTOPLAY, STORAGE_KTO24D2F_STATS]);
  const statsPack = data[STORAGE_KTO24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_KTO24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_KTO24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_KTO24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_KTO24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readKto24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_KTO24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readKto24d2fConfig();
  const streak =
    globalThis.SinglestakeKto24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeKto24d2f?.ice2fPairLabel === "function"
        ? SinglestakeKto24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_KTO24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_KTO24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initKto24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeKto24d2fBridgeHandler = handleBridgePayload;
  void readKto24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readKto24d2fAutopilotEnabled().then((on) => {
    if (on) void startKto24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_KTO24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_KTO24D2F_AUTOPLAY].newValue === true) {
        void startKto24d2fAutopilot(bridgeHandler);
      } else {
        stopKto24d2fAutopilot();
      }
    }
    if (changes[STORAGE_KTO24D2F_CONFIG] && bridgeHandler) {
      void readKto24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readKto24d2fAutopilotEnabled().then((on) => {
          if (on) void startKto24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeKto24d2fSignalRunner = {
    initKto24d2fSignalRunner,
    startKto24d2fAutopilot,
    stopKto24d2fAutopilot,
    setKto24d2fAutopilotEnabled,
    getKto24d2fAutopilotStatus,
    getKto24d2fConfigForPopup,
    setKto24d2fConfigFromPopup,
    resetKto24d2fStats,
    STORAGE_KTO24D2F_AUTOPLAY,
    STORAGE_KTO24D2F_CONFIG,
  };
}
