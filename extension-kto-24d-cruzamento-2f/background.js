importScripts("shared.js", "kto24d2f-engine.js", "dga-hub.js", "kto24d2f-signal-runner.js");
...
async function openOrFocusKto24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (kto24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(kto24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      kto24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    kto24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: kto24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogKto24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureKto24d2fPanelOnOpenTabs();
  void openOrFocusKto24d2fPanel();
});
...
/** Janela fixa do painel KTO 24D 2F (não fecha ao clicar na mesa). */
let kto24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === kto24d2fPanelWindowId) kto24d2fPanelWindowId = null;
});

const KTO24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const KTO24D2F_CALIB_SITE_KEY = "kto.bet.br|pragmatic-24d-spin";
...
function isKto24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "kto.bet.br" || host.endsWith(".kto.bet.br"))) return false;
    return /\/app\/cassino\/game\/(24d[-_]?spin|24dspin)/i.test(path) || /24d[-_]?spin|24dspin|3426/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureKto24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isKto24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "kto24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-kto24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureKto24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isKto24d2fGameUrl(tab.url)) {
      await ensureKto24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isKto24d2fGameUrl(url)
  ) {
    registerMesaTab(KTO24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureKto24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeKto24d2fSignalRunner.setKto24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureKto24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusKto24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusKto24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeKto24d2fSignalRunner.getKto24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeKto24d2fSignalRunner.resetKto24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeKto24d2fSignalRunner.getKto24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeKto24d2fSignalRunner.setKto24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isKto24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(KTO24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isKto24d2fGameUrl(t.url)) ??
  ...
  const kto24d2fAutopilot = await SinglestakeKto24d2fSignalRunner.getKto24d2fAutopilotStatus();
  const kto24d2fConfig = await SinglestakeKto24d2fSignalRunner.getKto24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    kto24d2fAutopilot,
...
function isKto24d2fCrossingContext(context) {
  return context?.strategy === "kto24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isKto24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isKto24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isKto24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "kto24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isKto24d2fSignal =
    isKto24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("kto24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isKto24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isKto24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isKto24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isKto24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isKto24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeKto24d2f?.kto24d2fPadFactorPlacementMs === "function"
          ? SinglestakeKto24d2f.kto24d2fPadFactorPlacementMs(padUnits)
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "kto.bet.br" || host.endsWith(".kto.bet.br") || host.endsWith(".kto.bet.br")) {
      return KTO24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[KTO24D2F_CALIB_SITE_KEY]) {
    return { siteKey: KTO24D2F_CALIB_SITE_KEY, site: sites[KTO24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: KTO24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[KTO24D2F_CALIB_SITE_KEY]) return { siteKey: KTO24D2F_CALIB_SITE_KEY, site: sites[KTO24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? KTO24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === KTO24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isKto24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeKto24d2f !== "undefined" && SinglestakeKto24d2f.KTO24D2F_MESA_URL) ||
          "https://www.kto.bet.br/app/cassino/game/24d-spin/",
        mesaProvider: null,
        currentTableId: KTO24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba KTO 24D encontrada. Abra o 24D Spin (kto.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeKto24d2fSignalRunner.initKto24d2fSignalRunner(handleBridgePayload);
