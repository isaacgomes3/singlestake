...
/** Pausa entre cliques do plano bridge — ritmo Lotogreen 2F entre factores. */
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    (isDois2FatoresBridgeContext(context) || isIce3fCrossingContext(context)) &&
    ...
    if (isIce3fCrossingContext(context)) {
      return ice3fFactorBridgeStaggerMs(recoveryFromContext(context));
    }
    return GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? CLICK_STAGGER_MS * 2;
  }
  if (isIce3fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.ICE2F_DOUBLE_CLICK_STAGGER_MS ?? 2;
  }
...
async function dispatchClickAction(action, context, sourceTabId) {
  ...
  const clickResult = await executeBetWithChip(
    ...
    {
      skipBarSettle:
        (action.target === "factor-2" || action.target === "factor-3") &&
...
async function focusMesaTab(tabId) {
  try {
    ...
    // Sem sleep fixo — igual Lotogreen 2F (evita ~1s entre factores).
  } catch {
    /* tab closed */
  }
}
    ...
    if (isIce3fCrossingContext(context ?? {})) {
      const padMs =
        typeof SinglestakeSportingbet3f?.ice3fPadFactorPlacementMs === "function"
          ...
          : Math.max(
              0,
              (ICE3F_GALE3_REFERENCE_UNITS -
                (useDoubleGale ? Math.max(1, units) : chipClicks)) *
                (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150),
            );
