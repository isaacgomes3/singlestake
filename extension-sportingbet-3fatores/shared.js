...
const GOG = {
  ...
  /** Entre fichas do mesmo factor (entrada/gale baixo). */
  CROSSING_FACTOR_CLICK_STAGGER_MS: 150,
  /** Bridge factor-1 → factor-2 → factor-3 (ritmo Lotogreen 2F). */
  ICE2F_FACTOR_BRIDGE_STAGGER_MS: 2,
  /** Intervalo entre fichas — gale 4. */
  ICE2F_GALE4_CHIP_STAGGER_MS: 15,
  /** Intervalo entre fichas — gale 5. */
  ICE2F_GALE5_CHIP_STAGGER_MS: 8,
  /** Intervalo entre cliques em Dobrar (gale). */
  ICE2F_DOUBLE_CLICK_STAGGER_MS: 2,
...
/** Gale 3 (8 unidades) — referência de ritmo de digitação. */
const ICE3F_GALE3_REFERENCE_UNITS = 8;

/** Intervalo entre cliques de ficha — igual Lotogreen 2F. */
...
/** Pausa entre factores no bridge (F1→F2→F3) — igual Lotogreen 2F. */
function ice3fFactorBridgeStaggerMs(_recovery) {
  return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 2;
}
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isIce3fCrossingContext(context) && context?.singleFactorMode !== true) {
    const units =
      typeof context?.units === "number" && Number.isFinite(context.units) && context.units > 0
        ? Math.floor(context.units)
        : ktoStakeUnitsForRecovery(r);
    // Alinha com Lotogreen 2F: gale alto = cliques mais rápidos entre fichas.
    if (units >= 16) return GOG.ICE2F_GALE5_CHIP_STAGGER_MS ?? 8;
    if (units >= 8) return GOG.ICE2F_GALE4_CHIP_STAGGER_MS ?? 15;
    return GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150;
  }
