/**
 * Bundle entry — motor GoldeBet Cruzamento 2F para extensão Chrome.
 */
...
export const GOLDE2F_TABLE_ID = 230;
export const GOLDE2F_MESA_URL =
  "https://goldebet.bet.br/play/pragmatic/roulette-3";
export const GOLDE2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const GOLDE2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const GOLDE2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const GOLDE2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));

export type Golde2fPersistedMachine = {
...
function parsePersistedWatch(
  raw: Golde2fPersistedMachine["watch"],
...
export type CreateGolde2fEngineOptions = {
  maxRecovery?: number;
  initialStats?: RotatingRoomSessionStats | null;
  initialMachine?: Golde2fPersistedMachine | null;
};
...
function pendingRecoveryFromSaved(saved: Golde2fPersistedMachine): number {
  const stored =
...
export type Golde2fEngineSpinResult = {
...
export function createGolde2fEngine(options: CreateGolde2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && GOLDE2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        GOLDE2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && GOLDE2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
      ...(restoredWatch ? { watch: restoredWatch } : {}),
    };
  }
  let pendingRestore: Golde2fPersistedMachine | null = options.initialMachine ?? null;
  let stats =
  ...
  function runTick(): Golde2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && GOLDE2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && GOLDE2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        GOLDE2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        GOLDE2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = GOLDE2F_MESA_URL) {
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `GoldeBet 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `GoldeBet 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `GoldeBet 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "golde2fcruzamento" as const,
...
// named exports; sem isto SinglestakeGolde2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeGolde2f?: typeof api }).SinglestakeGolde2f = api;
}
