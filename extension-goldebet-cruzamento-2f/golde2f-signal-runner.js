/** Autopilot GoldeBet — Cruzamento 2F (230) · cruzamento sequencial 2 fatores. */
const STORAGE_GOLDE2F_AUTOPLAY = "gogGolde2fAutopilotEnabled";
const STORAGE_GOLDE2F_CONFIG = "gogGolde2fConfig";
const STORAGE_GOLDE2F_STATUS = "gogGolde2fAutopilotStatus";
const STORAGE_GOLDE2F_STATS = "gogGolde2fAutopilotSessionStats";
const STORAGE_GOLDE2F_MACHINE = "gogGolde2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeGolde2f['createGolde2fEngine']>|null} */
let engine = null;
...
const GOLDE2F_DEFAULTS = {
  tableId: SinglestakeGolde2f?.GOLDE2F_TABLE_ID ?? 230,
  mesaUrl:
    SinglestakeGolde2f?.GOLDE2F_MESA_URL ??
...
function knownPairIds() {
  const known = SinglestakeGolde2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeGolde2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeGolde2f !== "undefined"
      ? SinglestakeGolde2f.applyIce2fEnabledPairIds
        ? SinglestakeGolde2f
        : SinglestakeGolde2f.default
      : null;
...
async function readGolde2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_AUTOPLAY]);
  return data[STORAGE_GOLDE2F_AUTOPLAY] === true;
}

async function readGolde2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_CONFIG]);
  const stored = data[STORAGE_GOLDE2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...GOLDE2F_DEFAULTS,
      maxRecoveryPreference: GOLDE2F_DEFAULTS.maxRecovery,
...
async function readGolde2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_MACHINE]);
  const raw = data[STORAGE_GOLDE2F_MACHINE];
...
async function persistGolde2fMachineState(machine) {
...
async function clearGolde2fMachineState() {
  await chrome.storage.local.remove([STORAGE_GOLDE2F_MACHINE]);
}

async function readGolde2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_STATS]);
  const raw = data[STORAGE_GOLDE2F_STATS];
...
async function persistGolde2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_GOLDE2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeGolde2fStatus({
    wins: stats.wins ?? 0,
...
async function resetGolde2fStats() {
  const cfg = await readGolde2fConfig();
  const empty =
    globalThis.SinglestakeGolde2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistGolde2fStats(empty, cfg.maxRecovery);
  await clearGolde2fMachineState();
...
async function getGolde2fConfigForPopup() {
  return readGolde2fConfig();
}

async function setGolde2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_CONFIG]);
  const stored =
    data[STORAGE_GOLDE2F_CONFIG] && typeof data[STORAGE_GOLDE2F_CONFIG] === "object"
      ? data[STORAGE_GOLDE2F_CONFIG]
      : {};
  const prev = await readGolde2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_GOLDE2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearGolde2fMachineState();
    await writeGolde2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readGolde2fConfig()) };
}

async function writeGolde2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_STATUS]);
  const prev = data[STORAGE_GOLDE2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_GOLDE2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistGolde2fMachineState(live.machine);
    await writeGolde2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeGolde2f?.ice2fBetDelayMs === "function"
      ? SinglestakeGolde2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeGolde2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeGolde2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeGolde2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `golde2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistGolde2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistGolde2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeGolde2f?.ice2fWatchLabelForMachine ??
      SinglestakeGolde2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeGolde2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeGolde2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startGolde2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeGolde2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readGolde2fAutopilotEnabled();
  if (!enabled) {
    await writeGolde2fStatus({ running: false, reason: "autopilot GoldeBet 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeGolde2f?.createGolde2fEngine) {
    await writeGolde2fStatus({
      running: false,
      reason: "golde2f-engine.js em falta — corra npm run extension:golde2f:build",
    });
    return;
  }

  stopGolde2fAutopilot();

  const cfg = await readGolde2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readGolde2fStats(cfg.maxRecovery);
  const savedMachine = await readGolde2fMachineState();
  engine = SinglestakeGolde2f.createGolde2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeGolde2fStatus({ log: msg }),
    onStatus: (status) => void writeGolde2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readGolde2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readGolde2fConfig().then((cfgNow) =>
...
function stopGolde2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeGolde2fStatus({ running: false, active: false, waitingBet: false });
}

async function setGolde2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_GOLDE2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startGolde2fAutopilot(bridgeHandler ?? globalThis.__singlestakeGolde2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeGolde2fBridgeHandler) {
      await writeGolde2fStatus({
        running: false,
  ...
  return getGolde2fAutopilotStatus();
}

async function getGolde2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE2F_STATUS, STORAGE_GOLDE2F_AUTOPLAY, STORAGE_GOLDE2F_STATS]);
  const statsPack = data[STORAGE_GOLDE2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_GOLDE2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_GOLDE2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_GOLDE2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_GOLDE2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readGolde2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_GOLDE2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readGolde2fConfig();
  const streak =
    globalThis.SinglestakeGolde2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeGolde2f?.ice2fPairLabel === "function"
        ? SinglestakeGolde2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_GOLDE2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_GOLDE2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initGolde2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeGolde2fBridgeHandler = handleBridgePayload;
  void readGolde2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readGolde2fAutopilotEnabled().then((on) => {
    if (on) void startGolde2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_GOLDE2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_GOLDE2F_AUTOPLAY].newValue === true) {
        void startGolde2fAutopilot(bridgeHandler);
      } else {
        stopGolde2fAutopilot();
      }
    }
    if (changes[STORAGE_GOLDE2F_CONFIG] && bridgeHandler) {
      void readGolde2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readGolde2fAutopilotEnabled().then((on) => {
          if (on) void startGolde2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeGolde2fSignalRunner = {
    initGolde2fSignalRunner,
    startGolde2fAutopilot,
    stopGolde2fAutopilot,
    setGolde2fAutopilotEnabled,
    getGolde2fAutopilotStatus,
    getGolde2fConfigForPopup,
    setGolde2fConfigFromPopup,
    resetGolde2fStats,
    STORAGE_GOLDE2F_AUTOPLAY,
    STORAGE_GOLDE2F_CONFIG,
  };
}
