...
const golde2fConfigStatus = document.getElementById("golde2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, golde2fConfig) {
  ...
  const noGale =
    golde2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = golde2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? golde2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.golde2fAutopilot, mode, status?.golde2fConfig);

  const cfg = status?.golde2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "goldebet.bet.br/liveroulettea-pragmaticexternal"}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa GoldeBet 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Roulette 3 no GoldeBet num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa GoldeBet 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-golde2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-golde2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-golde2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.golde2fConfig ?? {};
  await send("set-golde2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-golde2f-config", { config });
  if (golde2fConfigStatus) {
    golde2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
