...
var SinglestakeGolde2f = (() => {
  ...
  // extension-goldebet-cruzamento-2f/golde2f-strategy-entry.ts
  var golde2f_strategy_entry_exports = {};
  __export(golde2f_strategy_entry_exports, {
    GOLDE2F_MAX_GALES: () => GOLDE2F_MAX_GALES,
    GOLDE2F_MESA_URL: () => GOLDE2F_MESA_URL,
    GOLDE2F_TABLE_ID: () => GOLDE2F_TABLE_ID,
    ...
    createGolde2fEngine: () => createGolde2fEngine,
    default: () => golde2f_strategy_entry_default,
  ...
  // extension-goldebet-cruzamento-2f/golde2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var GOLDE2F_TABLE_ID = 230;
  var GOLDE2F_MESA_URL = "https://goldebet.bet.br/play/pragmatic/roulette-3";
  var GOLDE2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var GOLDE2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var GOLDE2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var GOLDE2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of GOLDE2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createGolde2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && GOLDE2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && GOLDE2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && GOLDE2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && GOLDE2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && GOLDE2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && GOLDE2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && GOLDE2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = GOLDE2F_MESA_URL) {
      ...
      const signalId = `golde2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `GoldeBet 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `GoldeBet 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `GoldeBet 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: GOLDE2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? GOLDE2F_MESA_URL,
          mesaProvider: "outro",
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeGolde2f = api;
  }
  var golde2f_strategy_entry_default = api;
  return __toCommonJS(golde2f_strategy_entry_exports);
})();
