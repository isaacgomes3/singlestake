...
/** Gale 4/5 — mesmo tempo de preenchimento que gale 3 (8×150 ms por factor). */
function goldeUnitClickStaggerMs(recovery) {
  const base = GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150;
  const r = Math.max(0, Math.floor(recovery));
  if (r <= 3) return base;
  const gale3Units = 8;
  const gale3FillMs = (gale3Units - 1) * base;
  const units = Math.max(1, 2 ** r);
  if (units <= 1) return base;
  return Math.max(25, Math.round(gale3FillMs / (units - 1)));
}
...
function clickStaggerMsForContext(context, recovery) {
  ...
  if (context?.strategy === "dois2fatores" && context?.singleFactorMode !== true) {
    if (isGoldeCrossingContext(context) && r >= 4) {
      return goldeUnitClickStaggerMs(r);
    }
