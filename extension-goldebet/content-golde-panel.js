/**
 * Painel flutuante na GoldeBet — activar estratégia 2 Fatores · cruzamento sequencial (mesa 28401).
 */
(function () {
  ...
  if (!pageIsGoldeRoulette()) return;
  if (document.getElementById("ss-golde-panel-root")) return;

  const PANEL_ID = "ss-golde-panel-root";
  const POLL_MS = 2000;
  let pollTimer = null;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.kind === "golde-panel-ping") {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .ss-golde-body { padding: 10px 12px 12px; }
    #${PANEL_ID} .ss-golde-sub {
      margin: 0 0 8px;
      font-size: 10px;
      color: #94a3b8;
      line-height: 1.45;
    }
    #${PANEL_ID} .ss-golde-status {
      margin: 0 0 10px;
      font-size: 10px;
      color: #cbd5e1;
      min-height: 2.8em;
    ...
    #${PANEL_ID} .ss-golde-status.err { color: #fca5a5; }
    #${PANEL_ID} .ss-golde-stats {
      display: flex;
      gap: 8px;
      margin-bottom: 10px;
    }
    #${PANEL_ID} .ss-golde-stat {
      flex: 1;
      text-align: center;
      padding: 6px;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-golde-stat strong {
      display: block;
      font-size: 18px;
      color: #f8fafc;
    }
    #${PANEL_ID} .ss-golde-stat small {
      font-size: 8px;
      font-weight: 700;
      text-transform: uppercase;
      color: #94a3b8;
    }
    #${PANEL_ID} .ss-golde-stat.win { border-color: #166534; background: #052e16; }
    #${PANEL_ID} .ss-golde-stat.win strong { color: #86efac; }
    #${PANEL_ID} .ss-golde-stat.loss { border-color: #991b1b; background: #450a0a; }
    #${PANEL_ID} .ss-golde-stat.loss strong { color: #fca5a5; }
    #${PANEL_ID} .ss-golde-actions {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }
    #${PANEL_ID} .ss-golde-btn {
      flex: 1;
      min-width: 72px;
      border: 1px solid #334155;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-golde-btn.on-demo {
      border-color: #2563eb;
      background: #172554;
      color: #bfdbfe;
    }
    #${PANEL_ID} .ss-golde-btn.on-real {
      border-color: #16a34a;
      background: #14532d;
      color: #bbf7d0;
    }
    #${PANEL_ID} .ss-golde-btn.on-active {
      border-color: #059669;
      background: #064e3b;
      color: #a7f3d0;
    }
    #${PANEL_ID} .ss-golde-btn.on-warn {
      border-color: #b45309;
      background: #451a03;
      color: #fcd34d;
    }
    #${PANEL_ID} .ss-golde-hidden { display: none !important; }
    #${PANEL_ID}.ss-golde-collapsed .ss-golde-body { display: none; }
  `;

  const root = el("div");
  root.id = PANEL_ID;

  const card = el("div", "ss-golde-card");
  const head = el("div", "ss-golde-head");
  const title = el("div", "ss-golde-title", "stake37 · 2 Fatores");
  const minBtn = el("button", "ss-golde-min", "−");
  minBtn.type = "button";
  head.append(title, minBtn);

  const body = el("div", "ss-golde-body");
  const sub = el("p", "ss-golde-sub", "French la Partage · mesa 28401 · gales alternados (oposto 1,4,5)");
  const statusEl = el("p", "ss-golde-status", "A ligar à extensão…");

  const stats = el("div", "ss-golde-stats");
  const winStat = el("div", "ss-golde-stat win");
  const winNum = el("strong", null, "0");
  winStat.append(winNum, el("small", null, "Vitórias"));
  const lossStat = el("div", "ss-golde-stat loss");
  const lossNum = el("strong", null, "0");
  lossStat.append(lossNum, el("small", null, "Derrotas"));
  stats.append(winStat, lossStat);

  const actions = el("div", "ss-golde-actions");
  const btnDemo = el("button", "ss-golde-btn on-demo", "Demo");
  const btnReal = el("button", "ss-golde-btn", "Real");
  const btnToggle = el("button", "ss-golde-btn", "Parado");
  const btnReload = el("button", "ss-golde-btn on-warn ss-golde-hidden", "Actualizar página");
  btnDemo.type = "button";
  btnReal.type = "button";
  ...
  minBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    collapsed = !collapsed;
    root.classList.toggle("ss-golde-collapsed", collapsed);
  ...
  btnToggle.addEventListener("click", async () => {
    ...
    const cur = await send("get-golde-autopilot");
    const on = cur?.enabled === true;
    statusEl.textContent = on ? "A parar…" : "A ligar estratégia…";
    const r = await send("set-golde-autopilot", { enabled: !on });
  ...
  function renderStatus(data) {
    ...
    if (!data || data.error) {
      ...
      btnToggle.className = "ss-golde-btn on-warn";
      btnToggle.textContent = "Parado";
      if (contextDead) btnReload.classList.remove("ss-golde-hidden");
      return;
    }

    btnReload.classList.add("ss-golde-hidden");

    const mode = data.mode === "real" ? "real" : "demo";
    const golde = data.goldeAutopilot;
    const st = golde?.status ?? {};
    const on = golde?.enabled === true;
    const running = st.running === true;

    btnDemo.className = `ss-golde-btn ${mode === "demo" ? "on-demo" : ""}`;
    btnReal.className = `ss-golde-btn ${mode === "real" ? "on-real" : ""}`;

    if (on && running) {
      btnToggle.className = "ss-golde-btn on-active";
      btnToggle.textContent = "Activo";
    } else if (on) {
      btnToggle.className = "ss-golde-btn on-warn";
      btnToggle.textContent = "A ligar…";
    } else {
      btnToggle.className = "ss-golde-btn";
      btnToggle.textContent = "Parado";
    }
    ...
    if (on && running) parts.push("DGA mesa 28401 ligada");
    ...
    if (st.recovery > 0 && !st.active) {
      parts.push(`gale pendente ${st.recovery}/${st.maxRecovery ?? 5}`);
    } else if (st.recovery > 0) {
      parts.push(`gale ${st.recovery}/${st.maxRecovery ?? 5}`);
    }
