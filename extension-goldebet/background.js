importScripts("shared.js", "golde-cruzamento-engine.js", "dga-hub.js", "golde-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogGoldeAutopilotEnabled: false });
  }
  void ensureGoldePanelOnOpenTabs();
});
...
const GOLDE_DEFAULT_TABLE_ID = 28401;
...
function isGoldeRoulettePageUrl(url) {
  ...
  try {
    ...
    if (/\/play\/pragmatic\//i.test(path) && /french-roulette|la-partage|28401/i.test(path)) {
      return true;
    }
    return /french-roulette-la-partage/i.test(url);
  } catch {
    return /goldebet\.bet/i.test(url) && /french-roulette-la-partage/i.test(url);
  }
}

async function ensureGoldePanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isGoldeRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "golde-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-golde-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureGoldePanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isGoldeRoulettePageUrl(tab.url)) {
      await ensureGoldePanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isGoldeRoulettePageUrl(url)
  ) {
    registerMesaTab(GOLDE_DEFAULT_TABLE_ID, tabId);
    void ensureGoldePanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-golde-autopilot") {
    void SinglestakeGoldeSignalRunner.setGoldeAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureGoldePanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-golde-autopilot") {
    void SinglestakeGoldeSignalRunner.getGoldeAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-golde-stats") {
    void SinglestakeGoldeSignalRunner.resetGoldeStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-golde-config") {
    void SinglestakeGoldeSignalRunner.getGoldeConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-golde-config") {
    void SinglestakeGoldeSignalRunner.setGoldeConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isGoldeRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(GOLDE_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const goldeAutopilot = await SinglestakeGoldeSignalRunner.getGoldeAutopilotStatus();
  const goldeConfig = await SinglestakeGoldeSignalRunner.getGoldeConfigForPopup();
  return {
    ok: true,
    mode,
    goldeAutopilot,
    ...
    return {
      ...
      mode: "demo",
      goldeAutopilot: { enabled: false, status: { reason: "Erro no service worker" } },
      goldeConfig: {},
    };
  }
}
...
async function handleBridgePayload(payload, sourceTabId) {
  ...
  try {
    ...
    const clickOk = (r) => r?.ok === true && r?.skipped !== true;
    const f1 = results.find((r) => r?.target === "factor-1");
    const f2 = results.find((r) => r?.target === "factor-2");
    const goldeBoth =
      isGoldeCrossingContext(payload.context) && clickOk(f1) && clickOk(f2);
    const placedBet = goldeBoth
      ? true
      : !isGoldeCrossingContext(payload.context) &&
        results.some(
          (r) =>
            (r.target === "factor-1" ||
              r.target === "factor-2" ||
...
async function dispatchClickAction(action, context, sourceTabId) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isGoldeSignal =
    isGoldeCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("golde:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isGoldeSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isGoldeSignal) {
...
function isGoldeCrossingContext(context) {
  return (
...
function isCasinoPlayUrl(url) {
  return isGoldeRoulettePageUrl(url);
}
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail: "Abra French Roulette la Partage no GoldeBet (goldebet.bet.br/play/pragmatic/french-roulette-la-partage) num separador e aguarde carregar.",
    };
  }
...
SinglestakeGoldeSignalRunner.initGoldeSignalRunner(handleBridgePayload);
