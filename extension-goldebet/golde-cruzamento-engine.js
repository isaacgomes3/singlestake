"use strict";
var SinglestakeGoldeCruzamento = (() => {
  ...
  // extension-goldebet/golde-strategy-entry.ts
  var golde_strategy_entry_exports = {};
  __export(golde_strategy_entry_exports, {
    GOLDE_MAX_GALES: () => GOLDE_MAX_GALES,
    GOLDE_MESA_URL: () => GOLDE_MESA_URL,
    GOLDE_TABLE_ID: () => GOLDE_TABLE_ID,
    ...
    createGoldeCruzamentoEngine: () => createGoldeCruzamentoEngine,
    default: () => golde_strategy_entry_default
  });
  ...
  // src/lib/roulette/goldeCruzamentoSequencialStrategy.ts
  var GOLDE_ROULETTE_TABLE_ID = 28401;
  var GOLDE_ROULETTE_MESA_URL = "https://goldebet.bet.br/play/pragmatic/french-roulette-la-partage";
  var GOLDE_CRUZAMENTO_MAX_RECOVERY = 5;
  var GOLDE_CRUZAMENTO_MIN_HISTORY = 2;
  var GOLDE_CRUZAMENTO_FIRST_BET_SETTLE_MS = 6e3;
  var GOLDE_CRUZAMENTO_RECOVERY_BET_DELAY_MS = 6e3;
  var GOLDE_OPPOSITE_RECOVERIES = /* @__PURE__ */ new Set([1, 4, 5]);
  ...
  function goldeCruzamentoUseOppositeFactors(recovery) {
    return GOLDE_OPPOSITE_RECOVERIES.has(recovery);
  }
  function goldeCruzamentoBetFactors(active, recovery) {
  ...
  function detectGoldeCruzamentoTrigger(historyNewestFirst) {
  ...
  function evaluateGoldeCruzamentoRound(num, active, recovery) {
  ...
  function defaultGoldeCruzamentoMachineState() {
    return {
      cycle: null,
      lastSpinHead: null,
      recovery: 0,
  ...
  function goldeCruzamentoBetDelayMs(recovery) {
    return recovery > 0 ? GOLDE_CRUZAMENTO_RECOVERY_BET_DELAY_MS : GOLDE_CRUZAMENTO_FIRST_BET_SETTLE_MS;
  }
  function canPlaceGoldeCruzamentoBet(recovery, lastSpinAtMs, nowMs = Date.now()) {
  ...
  function tickGoldeCruzamentoPlacar(historyNewestFirst, machine, stats, maxRecovery = GOLDE_CRUZAMENTO_MAX_RECOVERY) {
  ...
  function parseGoldeCruzamentoStats(raw, maxRecovery = GOLDE_CRUZAMENTO_MAX_RECOVERY) {
    return parseRotatingRoomSessionStats(raw, maxRecovery);
  }
  function emptyGoldeCruzamentoStats(maxRecovery = GOLDE_CRUZAMENTO_MAX_RECOVERY) {
    return emptyRotatingRoomSessionStats(maxRecovery);
  }

  // extension-goldebet/golde-strategy-entry.ts
  var GOLDE_TABLE_ID = GOLDE_ROULETTE_TABLE_ID;
  var GOLDE_MESA_URL = GOLDE_ROULETTE_MESA_URL;
  var GOLDE_MAX_GALES = GOLDE_CRUZAMENTO_MAX_RECOVERY;
  var ROTATING_ROOM_MESA_FIRST_CLICK_SETTLE_MS = GOLDE_CRUZAMENTO_FIRST_BET_SETTLE_MS;
  var ROTATING_ROOM_CROSSING_BET_DELAY_MS = GOLDE_CRUZAMENTO_RECOVERY_BET_DELAY_MS;
  var BASE_STAKE = 0.5;
  function clampMaxRecovery(value, fallback = GOLDE_CRUZAMENTO_MAX_RECOVERY) {
  ...
  function createGoldeCruzamentoEngine(options = {}) {
    ...
    let stats = options.initialStats != null ? parseGoldeCruzamentoStats(options.initialStats, maxRecovery) : emptyGoldeCruzamentoStats(maxRecovery);
    let history = [];
    let lastGameId = null;
    ...
    function buildBridgePayload(mesaEmbedUrl = GOLDE_MESA_URL) {
      ...
      const betDelayUntilMs = lastLiveSpinAt != null ? lastLiveSpinAt + (recovery > 0 ? GOLDE_CRUZAMENTO_RECOVERY_BET_DELAY_MS : GOLDE_CRUZAMENTO_FIRST_BET_SETTLE_MS) : null;
        ...
        actions: [
          {
            ...
            target: "factor-1",
            label: f1Label,
            reason: `Golde 2F \xB7 ${f1Label}${galeSuffix}${modeSuffix}`
          },
          {
            kind: "click",
            target: "factor-2",
            label: f2Label,
            reason: `Golde 2F \xB7 ${f2Label}${galeSuffix}${modeSuffix}`
          }
        ],
        context: {
          sessionMode: "active",
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeGoldeCruzamento = api;
  }
  var golde_strategy_entry_default = api;
  return __toCommonJS(golde_strategy_entry_exports);
})();
