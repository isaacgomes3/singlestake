/**
 * Calibração — overlay no frame do jogo (incl. iframe Pragmatic no GoldeBet); 1 clique grava coordenadas.
 */
(function () {
  ...
  function shouldActivateInThisFrame() {
    ...
    if (/pragmaticplaylive\.net/i.test(location.hostname)) return true;
    const canvas = document.querySelector("canvas");
    if (canvas) {
      const r = canvas.getBoundingClientRect();
      if (r.width > 200 && r.height > 120) return true;
    }
    if (window === window.top && /goldebet\.bet\.br/i.test(location.hostname)) {
      const iframe = document.querySelector('iframe[src*="pragmatic"], iframe[src*="game"]');
