/** Autopilot Reals — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_REALS24D2F_AUTOPLAY = "gogReals24d2fAutopilotEnabled";
const STORAGE_REALS24D2F_CONFIG = "gogReals24d2fConfig";
const STORAGE_REALS24D2F_STATUS = "gogReals24d2fAutopilotStatus";
const STORAGE_REALS24D2F_STATS = "gogReals24d2fAutopilotSessionStats";
const STORAGE_REALS24D2F_MACHINE = "gogReals24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeReals24d2f['createReals24d2fEngine']>|null} */
let engine = null;
...
function bumpReals24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function reals24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeReals24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeReals24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const REALS24D2F_DEFAULTS = {
  tableId: SinglestakeReals24d2f?.REALS24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeReals24d2f?.REALS24D2F_MESA_URL ??
...
function knownPairIds() {
  const known = SinglestakeReals24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeReals24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeReals24d2f !== "undefined"
      ? SinglestakeReals24d2f.applyIce2fEnabledPairIds
        ? SinglestakeReals24d2f
        : SinglestakeReals24d2f.default
      : null;
...
async function readReals24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_AUTOPLAY]);
  return data[STORAGE_REALS24D2F_AUTOPLAY] === true;
}

async function readReals24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_CONFIG]);
  const stored = data[STORAGE_REALS24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...REALS24D2F_DEFAULTS,
      maxRecoveryPreference: REALS24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? REALS24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? REALS24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : REALS24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? REALS24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : REALS24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : REALS24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : REALS24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : REALS24D2F_DEFAULTS.currency,
    noGale,
...
async function readReals24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_MACHINE]);
  const raw = data[STORAGE_REALS24D2F_MACHINE];
...
async function persistReals24d2fMachineState(machine) {
...
async function clearReals24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_REALS24D2F_MACHINE]);
}

async function readReals24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_STATS]);
  const raw = data[STORAGE_REALS24D2F_STATS];
...
async function persistReals24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_REALS24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, reals24d2fIdleIndicationPatch());
  }
  await writeReals24d2fStatus(patch);
}

async function resetReals24d2fStats() {
  const cfg = await readReals24d2fConfig();
  const empty =
    globalThis.SinglestakeReals24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistReals24d2fStats(empty, cfg.maxRecovery);
  await clearReals24d2fMachineState();
...
async function getReals24d2fConfigForPopup() {
  return readReals24d2fConfig();
}

async function setReals24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_CONFIG]);
  const stored =
    data[STORAGE_REALS24D2F_CONFIG] && typeof data[STORAGE_REALS24D2F_CONFIG] === "object"
      ? data[STORAGE_REALS24D2F_CONFIG]
      : {};
  const prev = await readReals24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_REALS24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearReals24d2fMachineState();
    await writeReals24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readReals24d2fConfig()) };
}

async function writeReals24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_STATUS]);
  const prev = data[STORAGE_REALS24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_REALS24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeReals24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistReals24d2fMachineState(live.machine);
    await writeReals24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readReals24d2fConfig();
    ...
    await writeReals24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readReals24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeReals24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeReals24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeReals24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeReals24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeReals24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `reals24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistReals24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistReals24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpReals24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeReals24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeReals24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeReals24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeReals24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeReals24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startReals24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeReals24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readReals24d2fAutopilotEnabled();
  if (!enabled) {
    await writeReals24d2fStatus({ running: false, reason: "autopilot Reals 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeReals24d2f?.createReals24d2fEngine) {
    await writeReals24d2fStatus({
      running: false,
      reason: "reals24d2f-engine.js em falta — corra npm run extension:reals24d2f:build",
    });
    return;
  }

  stopReals24d2fAutopilot();

  const cfg = await readReals24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readReals24d2fStats(cfg.maxRecovery);
  const savedMachine = await readReals24d2fMachineState();
  engine = SinglestakeReals24d2f.createReals24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeReals24d2fStatus({ log: msg }),
    onStatus: (status) => void writeReals24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readReals24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readReals24d2fConfig().then((cfgNow) =>
...
function stopReals24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpReals24d2fIndicationEpoch();
  void writeReals24d2fStatus(reals24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setReals24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_REALS24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startReals24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeReals24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeReals24d2fBridgeHandler) {
      await writeReals24d2fStatus({
        running: false,
  ...
  const status = await getReals24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getReals24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_REALS24D2F_STATUS, STORAGE_REALS24D2F_AUTOPLAY, STORAGE_REALS24D2F_STATS]);
  const statsPack = data[STORAGE_REALS24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_REALS24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_REALS24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_REALS24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_REALS24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readReals24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_REALS24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readReals24d2fConfig();
  const streak =
    globalThis.SinglestakeReals24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeReals24d2f?.ice2fPairLabel === "function"
        ? SinglestakeReals24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_REALS24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_REALS24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initReals24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeReals24d2fBridgeHandler = handleBridgePayload;
  void readReals24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readReals24d2fAutopilotEnabled().then((on) => {
    if (on) void startReals24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_REALS24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_REALS24D2F_AUTOPLAY].newValue === true) {
        void startReals24d2fAutopilot(bridgeHandler);
      } else {
        stopReals24d2fAutopilot();
      }
    }
    if (changes[STORAGE_REALS24D2F_CONFIG] && bridgeHandler) {
      void readReals24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readReals24d2fAutopilotEnabled().then((on) => {
          if (on) void startReals24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeReals24d2fSignalRunner = {
    initReals24d2fSignalRunner,
    startReals24d2fAutopilot,
    stopReals24d2fAutopilot,
    setReals24d2fAutopilotEnabled,
    getReals24d2fAutopilotStatus,
    getReals24d2fConfigForPopup,
    setReals24d2fConfigFromPopup,
    resetReals24d2fStats,
    STORAGE_REALS24D2F_AUTOPLAY,
    STORAGE_REALS24D2F_CONFIG,
  };
}
