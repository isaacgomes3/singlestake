importScripts("shared.js", "reals24d2f-engine.js", "dga-hub.js", "reals24d2f-signal-runner.js");
...
async function openOrFocusReals24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (reals24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(reals24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      reals24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    reals24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: reals24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogReals24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureReals24d2fPanelOnOpenTabs();
  void openOrFocusReals24d2fPanel();
});
...
/** Janela fixa do painel Reals 24D 2F (não fecha ao clicar na mesa). */
let reals24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === reals24d2fPanelWindowId) reals24d2fPanelWindowId = null;
});

const REALS24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const REALS24D2F_CALIB_SITE_KEY = "reals.bet.br|pragmatic-24d-spin";
...
function isReals24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "reals.bet.br" || host.endsWith(".reals.bet.br"))) return false;
    return /\/play\/liveCassino\/3426/i.test(path) || /24d.?spin|3426/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureReals24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isReals24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "reals24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-reals24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureReals24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isReals24d2fGameUrl(tab.url)) {
      await ensureReals24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isReals24d2fGameUrl(url)
  ) {
    registerMesaTab(REALS24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureReals24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeReals24d2fSignalRunner.setReals24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureReals24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusReals24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusReals24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeReals24d2fSignalRunner.getReals24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeReals24d2fSignalRunner.resetReals24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeReals24d2fSignalRunner.getReals24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeReals24d2fSignalRunner.setReals24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isReals24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(REALS24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isReals24d2fGameUrl(t.url)) ??
  ...
  const reals24d2fAutopilot = await SinglestakeReals24d2fSignalRunner.getReals24d2fAutopilotStatus();
  const reals24d2fConfig = await SinglestakeReals24d2fSignalRunner.getReals24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    reals24d2fAutopilot,
...
function isReals24d2fCrossingContext(context) {
  return context?.strategy === "reals24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isReals24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isReals24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isReals24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "reals24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isReals24d2fSignal =
    isReals24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("reals24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isReals24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isReals24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isReals24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isReals24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isReals24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeReals24d2f?.reals24d2fPadFactorPlacementMs === "function"
          ? SinglestakeReals24d2f.reals24d2fPadFactorPlacementMs(padUnits)
...
function isCasinoPlayUrl(url) {
  return isReals24d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "reals.bet.br" || host.endsWith(".reals.bet.br") || host.endsWith(".reals.bet.br")) {
      return REALS24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[REALS24D2F_CALIB_SITE_KEY]) {
    return { siteKey: REALS24D2F_CALIB_SITE_KEY, site: sites[REALS24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: REALS24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[REALS24D2F_CALIB_SITE_KEY]) return { siteKey: REALS24D2F_CALIB_SITE_KEY, site: sites[REALS24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? REALS24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === REALS24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isReals24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeReals24d2f !== "undefined" && SinglestakeReals24d2f.REALS24D2F_MESA_URL) ||
          "https://reals.bet.br/play/liveCassino/3426?title=24D+Spin&provider=pragmatic&gameId=%223426%22&category=live",
        mesaProvider: null,
        currentTableId: REALS24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Reals 24D encontrada. Abra o 24D Spin (reals.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeReals24d2fSignalRunner.initReals24d2fSignalRunner(handleBridgePayload);
