...
const ice2fStatus = document.getElementById("reals24d2fStatus");
...
const ice2fOnBtn = document.getElementById("reals24d2fOn");
const ice2fOffBtn = document.getElementById("reals24d2fOff");
const ice2fResetBtn = document.getElementById("reals24d2fReset");
const ice2fCasinoId = document.getElementById("reals24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("reals24d2fSave");
const reals24d2fConfigStatus = document.getElementById("reals24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, reals24d2fConfig) {
  ...
  const noGale =
    reals24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = reals24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? reals24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.reals24d2fAutopilot, mode, status?.reals24d2fConfig);

  const cfg = status?.reals24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "reals.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.reals24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (reals24d2fConfigStatus) {
    reals24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
