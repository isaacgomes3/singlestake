...
var SinglestakeReals24d2f = (() => {
  ...
  // extension-reals-24d-cruzamento-2f/reals24d2f-strategy-entry.ts
  var reals24d2f_strategy_entry_exports = {};
  __export(reals24d2f_strategy_entry_exports, {
    ...
    REALS24D2F_MAX_GALES: () => REALS24D2F_MAX_GALES,
    REALS24D2F_MESA_URL: () => REALS24D2F_MESA_URL,
    REALS24D2F_TABLE_ID: () => REALS24D2F_TABLE_ID,
    ...
    createReals24d2fEngine: () => createReals24d2fEngine,
    default: () => reals24d2f_strategy_entry_default,
  ...
  // extension-reals-24d-cruzamento-2f/reals24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var REALS24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var REALS24D2F_MESA_URL = "https://reals.bet.br/play/liveCassino/3426?title=24D+Spin&provider=pragmatic&gameId=%223426%22&category=live";
  var REALS24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var REALS24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var REALS24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var REALS24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of REALS24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createReals24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && REALS24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && REALS24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && REALS24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && REALS24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && REALS24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && REALS24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && REALS24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = REALS24D2F_MESA_URL) {
      ...
      const signalId = `reals24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: REALS24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? REALS24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createReals24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeReals24d2f = api;
  }
  var reals24d2f_strategy_entry_default = api;
  return __toCommonJS(reals24d2f_strategy_entry_exports);
})();
