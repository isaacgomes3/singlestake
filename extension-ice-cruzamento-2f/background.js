...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    ...
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 30;
  }
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "dois2fatores" || context?.strategy === "tres3fatores") return;
    ...
    if (isIce2fCrossingContext(context ?? {})) {
      const padMs =
        typeof SinglestakeIce2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeIce2f.ice2fPadFactorPlacementMs(units)
          : Math.max(0, (ICE2F_GALE3_REFERENCE_UNITS - units) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
