/** Autopilot ICE — Cruzamento 2F (201) · cruzamento sequencial 2 fatores. */
const STORAGE_ICE2F_AUTOPLAY = "gogIce2fAutopilotEnabled";
const STORAGE_ICE2F_CONFIG = "gogIce2fConfig";
const STORAGE_ICE2F_STATUS = "gogIce2fAutopilotStatus";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeIce2f['createIce2fEngine']>|null} */
let engine = null;
...
const ICE2F_DEFAULTS = {
  ...
  mesaUrl:
    SinglestakeIce2f?.ICE2F_MESA_URL ??
...
async function readIce2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_ICE2F_AUTOPLAY]);
  return data[STORAGE_ICE2F_AUTOPLAY] === true;
}
...
async function readIce2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_ICE2F_MACHINE]);
  const raw = data[STORAGE_ICE2F_MACHINE];
...
async function clearIce2fMachineState() {
  await chrome.storage.local.remove([STORAGE_ICE2F_MACHINE]);
}

async function readIce2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_ICE2F_STATS]);
  const raw = data[STORAGE_ICE2F_STATS];
...
async function persistIce2fStats(stats, maxRecovery) {
...
async function writeIce2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_ICE2F_STATUS]);
  const prev = data[STORAGE_ICE2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_ICE2F_STATUS]: next });
  return next;
}
...
async function startIce2fAutopilot(handleBridgePayload) {
  ...
  const enabled = await readIce2fAutopilotEnabled();
  if (!enabled) {
    await writeIce2fStatus({ running: false, reason: "autopilot ICE 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeIce2f?.createIce2fEngine) {
    await writeIce2fStatus({
      running: false,
      reason: "ice2f-engine.js em falta — corra npm run extension:ice2f:build",
    });
    return;
  }
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeIce2fStatus({ log: msg }),
    onStatus: (status) => void writeIce2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readIce2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readIce2fConfig().then((cfgNow) =>
...
function stopIce2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeIce2fStatus({ running: false, active: false, waitingBet: false });
}

async function setIce2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_ICE2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startIce2fAutopilot(bridgeHandler ?? globalThis.__singlestakeIce2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeIce2fBridgeHandler) {
      await writeIce2fStatus({
        running: false,
  ...
  return getIce2fAutopilotStatus();
}

async function getIce2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_ICE2F_STATUS, STORAGE_ICE2F_AUTOPLAY, STORAGE_ICE2F_STATS]);
  const statsPack = data[STORAGE_ICE2F_STATS];
  const live = engine?.getState?.();
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readIce2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
...
function initIce2fSignalRunner(handleBridgePayload) {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes[STORAGE_ICE2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_ICE2F_AUTOPLAY].newValue === true) {
        void startIce2fAutopilot(bridgeHandler);
      } else {
        stopIce2fAutopilot();
      }
    }
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeIce2fSignalRunner = {
    initIce2fSignalRunner,
    startIce2fAutopilot,
    stopIce2fAutopilot,
    setIce2fAutopilotEnabled,
    getIce2fAutopilotStatus,
    getIce2fConfigForPopup,
    setIce2fConfigFromPopup,
    resetIce2fStats,
    STORAGE_ICE2F_AUTOPLAY,
    STORAGE_ICE2F_CONFIG,
  };
}
