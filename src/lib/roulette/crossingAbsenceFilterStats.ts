...
function buildActiveFromBucket(category: string, axis: CrossingAxisKind): DoisFatoresActive | null {
  ...
  return {
    ...
    armingDescription: `Ausência · ${category}`,
  };
}

function bestExactBucketAtSpin(
  historyNow: readonly number[],
  axis: CrossingAxisKind,
  exactAbsence: number,
): { category: string; absence: number } | null {
  let best: { category: string; absence: number } | null = null;
  for (const def of CROSSING_BUCKET_DEFINITIONS) {
    ...
    if (!best || def.category < best.category) {
      best = { category: def.category, absence: absenceNow };
    }
  }
  return best;
}

function winAfterSpinsForCrossingAbsence(
...
function scanCrossingAbsenceTable(
  tableId: number,
  chronological: readonly number[],
  axis: CrossingAxisKind,
  exactAbsence: number,
  ...
  while (cursor < chronological.length) {
    ...
    const match = bestExactBucketAtSpin(now, axis, exactAbsence);
    if (!match) {
      cursor++;
      continue;
    }

    const active = buildActiveFromBucket(match.category, axis);
    if (!active) {
      cursor++;
      continue;
    }
    ...
    const winAfter = winAfterSpinsForCrossingAbsence(chronological, triggerIndex, active);
...
function buildBlockForAxis(
  ...
  for (
    ...
    for (const { tableId, chronological } of tables) {
      events.push(...scanCrossingAbsenceTable(tableId, chronological, axis, filterSpins));
      if (events.length >= ABSENCE_FILTER_STATS_MAX_EVENTS) break;
    }
...
export function buildCrossingAbsenceFilterStats(
  histories: Record<number, readonly number[]>,
): CrossingAbsenceFilterStats {
  return {
