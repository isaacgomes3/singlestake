...
export const Route = createFileRoute("/api/back-office/network/binary/place-direct")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { jsonResponse, requireSessionUser } = await import("@/lib/server/auth/http");
        const { confirmDirectBinaryPlacement } = await import(
        ...
        const directUserId = body?.directUserId?.trim();
        const side = body?.side;
        if (!directUserId || (side !== "left" && side !== "right")) {
          return jsonResponse({ ok: false, error: "Indicado ou perna inválidos." }, { status: 400 });
        }
