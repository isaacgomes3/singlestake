...
export const Route = createFileRoute("/api/back-office/packages/purchase-pix")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const { createPackagePixOrder } = await import("@/lib/server/finance/package-pix");
        ...
        if (!active) {
          return jsonResponse(
            { ok: false, error: "Mensalidade vencida. Regularize para comprar pacotes." },
            { status: 403 },
          );
        }
        ...
        return jsonResponse({ ok: true, order: result.order }, { status: 201 });
      },
    },
  },
});
