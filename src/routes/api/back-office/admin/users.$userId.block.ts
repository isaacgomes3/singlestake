...
export const Route = createFileRoute("/api/back-office/admin/users/$userId/block")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const { jsonResponse, requireSessionUser } = await import("@/lib/server/auth/http");
        const { adminBlockUser } = await import("@/lib/server/admin/users");
        ...
        const result = await adminBlockUser({ userId: params.userId, actorUserId: user.id });
