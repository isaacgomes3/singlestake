...
export const Route = createFileRoute("/api/back-office/admin/users/$userId/unblock")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const { jsonResponse, requireSessionUser } = await import("@/lib/server/auth/http");
        const { adminUnblockUser } = await import("@/lib/server/admin/users");
        ...
        const result = await adminUnblockUser({ userId: params.userId, actorUserId: user.id });
