...
import { Nums28PctStrategyTable } from "@/components/nums-28pct-strategy-table";
...
import { useNums28PctSpinOutcomeFlash } from "@/hooks/useNums28PctSpinOutcomeFlash";
...
import {
  ...
  nums28PctPlacarEvolutionSeries,
...
export const Route = createFileRoute("/numeros-28pct")({
  ...
  head: () => ({
    meta: [
      { title: "Números 2,8% - Roleta" },
      {
        name: "description",
        content: "Números 2,8% — dois giros iguais em sequência; exclusão de 2 opostos dos últimos 12 giros.",
      },
    ],
  }),
  component: Numeros28pctPage,
});

function Numeros28pctPage() {
  ...
  const outcomes = useMemo(() => nums28PctPlacarOutcomes(historyPlacar), [historyPlacar]);
  ...
  const evo = useMemo(() => nums28PctPlacarEvolutionSeries(historyPlacar), [historyPlacar]);
  ...
  const active = useMemo(() => nums28PctActiveFromMirrorSnapshot(historyPlacar), [historyPlacar]);

  useStrategyIndicationActivatedSound(active);

  const mesaLabel = lobbyTableDisplayName(viewTableId);

  const spinFlash = useNums28PctSpinOutcomeFlash(historyPlacar, viewTableId);
    ...
    <div className="min-h-screen bg-[#080d18] text-slate-100">
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <p className="mb-3 rounded-xl border border-cyan-950/25 bg-[#060a14]/90 px-3 py-2 text-center text-sm text-slate-400">
          Números 2,8% · <span className="font-semibold text-white">{mesaLabel}</span>
          <span className="text-slate-500"> · mesa {viewTableId}</span>
        </p>
        {active ? (
          <p className="mb-3 text-center text-xs text-cyan-200/90">
            Oposto total: <span className="font-semibold text-cyan-50">{active.crossingCategoryLabel}</span>
            <span className="text-slate-500"> · exclusão {active.excludedNumbers[0]} e {active.excludedNumbers[1]}</span>
          </p>
        ) : null}
        <RouletteAppTabs />
