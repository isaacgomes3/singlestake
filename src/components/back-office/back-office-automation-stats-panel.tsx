...
export function BackOfficeAutomationStatsPanel() {
  ...
  async function handleToggleTrigger(id: "three", enabled: boolean) {
    setTogglingId(id);
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-border-color">
          {loading ? (
            ...
            <table className="w-full min-w-[480px] text-left text-sm">
              ...
              <tbody>
                {(data?.triggers ?? []).filter((row) => row.id !== "two" && row.id !== "crossing").map((row) => (
                  <tr
                    key={row.id}
                    className={cn(
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          void handleToggleTrigger(row.id as "three", checked)
                        }
                      />
                    </td>
                  </tr>
