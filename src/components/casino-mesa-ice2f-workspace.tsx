...
import { ICE2F_TABLE_MAX_RECOVERY, resetIce2fSession } from "@/lib/roulette/ice2fTableSession";
...
export function CasinoMesaIce2fWorkspace({ tableId }: Props) {
  ...
  const session = useIce2fSession(tableId, history);
  const aproveitamento = rotatingRoomSessionAproveitamentoPct(session.sessionStats);
  const alertLine = session.ice2fActive
    ? session.ice2fActive.armingDescription
    : session.showTapeteSignal
      ? "Indicação activa"
      : "A analisar cruzamento 2F…";
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-slate-500 sm:text-xs">
              Mesa {tableId} · Cruzamento 2F sem gale · {aproveitamento.toFixed(1)}% aproveit.
            </p>
            <p
              className={cn(
      ...
      <div className="relative min-h-0 flex-1">
        ...
        >
          <div className="overflow-hidden rounded-2xl border border-cyan-500/25 bg-[#0d1524]/98 shadow-2xl shadow-black/60 backdrop-blur-md">
            <div
              role="toolbar"
              aria-label="Arrastar painel Cruzamento 2F"
            ...
            <div className="space-y-3 p-3">
              <div className="grid grid-cols-3 gap-1.5 text-center">
                <div className="rounded-lg border border-slate-700/80 bg-slate-950/60 px-2 py-1.5">
                  <p className="text-[9px] uppercase tracking-wide text-slate-500">Vitórias</p>
                  <p className="text-sm font-bold tabular-nums text-emerald-300">{wins}</p>
                </div>
                <div className="rounded-lg border border-slate-700/80 bg-slate-950/60 px-2 py-1.5">
                  <p className="text-[9px] uppercase tracking-wide text-slate-500">Derrotas</p>
                  <p className="text-sm font-bold tabular-nums text-rose-300">{losses}</p>
                </div>
                <div className="rounded-lg border border-slate-700/80 bg-slate-950/60 px-2 py-1.5">
                  <p className="text-[9px] uppercase tracking-wide text-slate-500">Sessão</p>
              ...
              {session.roundFlash ? (
                <div
                  className={cn(
                    ...
                    session.roundFlash.won
                      ? "border-emerald-400/50 bg-emerald-950/40 text-emerald-200"
                      : "border-rose-400/50 bg-rose-950/40 text-rose-200",
                  )}
                >
                  {session.roundFlash.won ? "Vitória" : "Derrota"} · {session.roundFlash.resultNumber}
                </div>
              ) : session.activeCrossing ? (
                <div className="space-y-2 rounded-xl border border-emerald-400/45 bg-emerald-950/25 px-3 py-3 street-indication-pulse-cyan">
                  <p className="text-center text-[10px] font-bold uppercase tracking-[0.14em] text-emerald-300/90">
                    Indicação activa
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <span
                      className={cn(
                        ...
                        doisFatoresFactorButtonClass(session.activeCrossing.factor1),
                      )}
                    >
                    ...
                    <span
                      className={cn(
                        "rounded-lg px-2 py-2 text-center text-sm font-bold",
                        doisFatoresFactorButtonClass(session.activeCrossing.factor2),
                      )}
                    >
                      {doisFatoresFactorLabel(session.activeCrossing.factor2)}
                    </span>
                  </div>
                </div>
              ) : (
                <p className="rounded-xl border border-slate-700/80 bg-slate-950/50 px-3 py-4 text-center text-xs text-slate-400">
                  À espera de cruzamento 2F (sem gale · máx. {ICE2F_TABLE_MAX_RECOVERY} recuperações)
                </p>
              )}
              ...
              {recent.length > 0 ? (
                <div className="flex flex-wrap justify-center gap-1">
                  {recent.map((n, i) => (
                    <span
                      key={`${n}-${i}`}
                      className="inline-flex h-6 min-w-[1.4rem] items-center justify-center rounded border border-slate-700 bg-slate-950 px-1 text-[10px] font-bold tabular-nums text-slate-200"
                    >
                      {n}
                    </span>
                  ))}
              ...
              <button
                type="button"
                onClick={() => resetIce2fSession(tableId, history)}
                className="w-full rounded-lg border border-slate-600/80 bg-slate-900/70 px-3 py-1.5 text-[11px] font-semibold text-slate-300 hover:bg-slate-800"
              >
                Repor placar
              </button>
            </div>
