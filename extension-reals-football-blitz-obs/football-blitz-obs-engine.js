/** Helpers Football Blitz Obs — cartas a partir de winningNumber + scoreDiff (DGA). */
...
function footballBlitzSideLabel(side) {
  ...
  if (side === "away") return "Azul";
  if (side === "draw") return "Empate";
  return "—";
}

/** Deriva as duas cartas da rodada DGA (casa/amarelo vs visitante/azul). */
...
/**
 * Chave exacta do chip: Amarelo·Azul·cor.
 * Q/A amarelo ≠ A/Q azul (mesmo que a carta vencedora seja igual).
 */
...
/** Cartas visíveis no chip (labels Amarelo/Azul). */
...
function findFootballBlitzCardPairSignal(historyNewestFirst, _options = {}) {
  ...
  return {
    signalId: `fb-blitz-coin:${newest.gameId}:${newestKey}:${left.winner}:${coinRight1.winner}+${coinRight2.winner}`,
...
globalThis.SinglestakeFootballBlitzObs = {
