importScripts("shared.js", "bn24d2f-engine.js", "dga-hub.js", "bn24d2f-signal-runner.js");
...
async function openOrFocusBn24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (bn24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(bn24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      bn24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    bn24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: bn24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogBn24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureBn24d2fPanelOnOpenTabs();
  void openOrFocusBn24d2fPanel();
});
...
/** Janela fixa do painel Bet Nacional 24D 2F (não fecha ao clicar na mesa). */
let bn24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === bn24d2fPanelWindowId) bn24d2fPanelWindowId = null;
});

const BN24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const BN24D2F_CALIB_SITE_KEY = "betnacional.bet.br|pragmatic-24d-spin";
...
function isBn24d2fGameUrl(url) {
  ...
  try {
    ...
    if (!(host === "betnacional.bet.br" || host.endsWith(".betnacional.bet.br"))) return false;
    return /\/casino\/game\//i.test(path) && /24d|3426/i.test(path + url);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureBn24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isBn24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "bn24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-bn24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureBn24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isBn24d2fGameUrl(tab.url)) {
      await ensureBn24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isBn24d2fGameUrl(url)
  ) {
    registerMesaTab(BN24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureBn24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeBn24d2fSignalRunner.setBn24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureBn24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusBn24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusBn24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeBn24d2fSignalRunner.getBn24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeBn24d2fSignalRunner.resetBn24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeBn24d2fSignalRunner.getBn24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeBn24d2fSignalRunner.setBn24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isBn24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(BN24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isBn24d2fGameUrl(t.url)) ??
  ...
  const bn24d2fAutopilot = await SinglestakeBn24d2fSignalRunner.getBn24d2fAutopilotStatus();
  const bn24d2fConfig = await SinglestakeBn24d2fSignalRunner.getBn24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    bn24d2fAutopilot,
...
function isBn24d2fCrossingContext(context) {
  return context?.strategy === "bn24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isBn24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isBn24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isBn24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "bn24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isBn24d2fSignal =
    isBn24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("bn24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isBn24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isBn24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isBn24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isBn24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isBn24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeBn24d2f?.bn24d2fPadFactorPlacementMs === "function"
          ? SinglestakeBn24d2f.bn24d2fPadFactorPlacementMs(padUnits)
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "betnacional.bet.br" || host.endsWith(".betnacional.bet.br") || host.endsWith(".betnacional.bet.br")) {
      return BN24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[BN24D2F_CALIB_SITE_KEY]) {
    return { siteKey: BN24D2F_CALIB_SITE_KEY, site: sites[BN24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: BN24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[BN24D2F_CALIB_SITE_KEY]) return { siteKey: BN24D2F_CALIB_SITE_KEY, site: sites[BN24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? BN24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === BN24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isBn24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeBn24d2f !== "undefined" && SinglestakeBn24d2f.BN24D2F_MESA_URL) ||
          "https://betnacional.bet.br/casino/game/",
        mesaProvider: null,
        currentTableId: BN24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Bet Nacional 24D encontrada. Abra o 24D Spin (betnacional.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeBn24d2fSignalRunner.initBn24d2fSignalRunner(handleBridgePayload);
