/** Autopilot Bet Nacional — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_BN24D2F_AUTOPLAY = "gogBn24d2fAutopilotEnabled";
const STORAGE_BN24D2F_CONFIG = "gogBn24d2fConfig";
const STORAGE_BN24D2F_STATUS = "gogBn24d2fAutopilotStatus";
const STORAGE_BN24D2F_STATS = "gogBn24d2fAutopilotSessionStats";
const STORAGE_BN24D2F_MACHINE = "gogBn24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeBn24d2f['createBn24d2fEngine']>|null} */
let engine = null;
...
function bumpBn24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function bn24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeBn24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeBn24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const BN24D2F_DEFAULTS = {
  tableId: SinglestakeBn24d2f?.BN24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeBn24d2f?.BN24D2F_MESA_URL ??
    "https://betnacional.bet.br/casino/game/",
...
function knownPairIds() {
  const known = SinglestakeBn24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeBn24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeBn24d2f !== "undefined"
      ? SinglestakeBn24d2f.applyIce2fEnabledPairIds
        ? SinglestakeBn24d2f
        : SinglestakeBn24d2f.default
      : null;
...
async function readBn24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_AUTOPLAY]);
  return data[STORAGE_BN24D2F_AUTOPLAY] === true;
}

async function readBn24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_CONFIG]);
  const stored = data[STORAGE_BN24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...BN24D2F_DEFAULTS,
      maxRecoveryPreference: BN24D2F_DEFAULTS.maxRecovery,
...
async function readBn24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_MACHINE]);
  const raw = data[STORAGE_BN24D2F_MACHINE];
...
async function persistBn24d2fMachineState(machine) {
...
async function clearBn24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_BN24D2F_MACHINE]);
}

async function readBn24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_STATS]);
  const raw = data[STORAGE_BN24D2F_STATS];
...
async function persistBn24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_BN24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, bn24d2fIdleIndicationPatch());
  }
  await writeBn24d2fStatus(patch);
}

async function resetBn24d2fStats() {
  const cfg = await readBn24d2fConfig();
  const empty =
    globalThis.SinglestakeBn24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistBn24d2fStats(empty, cfg.maxRecovery);
  await clearBn24d2fMachineState();
...
async function getBn24d2fConfigForPopup() {
  return readBn24d2fConfig();
}

async function setBn24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_CONFIG]);
  const stored =
    data[STORAGE_BN24D2F_CONFIG] && typeof data[STORAGE_BN24D2F_CONFIG] === "object"
      ? data[STORAGE_BN24D2F_CONFIG]
      : {};
  const prev = await readBn24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_BN24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearBn24d2fMachineState();
    await writeBn24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readBn24d2fConfig()) };
}

async function writeBn24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_STATUS]);
  const prev = data[STORAGE_BN24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_BN24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeBn24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistBn24d2fMachineState(live.machine);
    await writeBn24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readBn24d2fConfig();
    ...
    await writeBn24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readBn24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeBn24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeBn24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeBn24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeBn24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeBn24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `bn24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistBn24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistBn24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpBn24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeBn24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeBn24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeBn24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeBn24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeBn24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startBn24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeBn24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readBn24d2fAutopilotEnabled();
  if (!enabled) {
    await writeBn24d2fStatus({ running: false, reason: "autopilot Bet Nacional 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeBn24d2f?.createBn24d2fEngine) {
    await writeBn24d2fStatus({
      running: false,
      reason: "bn24d2f-engine.js em falta — corra npm run extension:bn24d2f:build",
    });
    return;
  }

  stopBn24d2fAutopilot();

  const cfg = await readBn24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readBn24d2fStats(cfg.maxRecovery);
  const savedMachine = await readBn24d2fMachineState();
  engine = SinglestakeBn24d2f.createBn24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeBn24d2fStatus({ log: msg }),
    onStatus: (status) => void writeBn24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readBn24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readBn24d2fConfig().then((cfgNow) =>
...
function stopBn24d2fAutopilot() {
  ...
  lastEmittedSignalId = null;
  bumpBn24d2fIndicationEpoch();
  void writeBn24d2fStatus(bn24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setBn24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_BN24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startBn24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeBn24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeBn24d2fBridgeHandler) {
      await writeBn24d2fStatus({
        running: false,
  ...
  const status = await getBn24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getBn24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_BN24D2F_STATUS, STORAGE_BN24D2F_AUTOPLAY, STORAGE_BN24D2F_STATS]);
  const statsPack = data[STORAGE_BN24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_BN24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_BN24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_BN24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_BN24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readBn24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_BN24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readBn24d2fConfig();
  const streak =
    globalThis.SinglestakeBn24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeBn24d2f?.ice2fPairLabel === "function"
        ? SinglestakeBn24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_BN24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_BN24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initBn24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeBn24d2fBridgeHandler = handleBridgePayload;
  void readBn24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readBn24d2fAutopilotEnabled().then((on) => {
    if (on) void startBn24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_BN24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_BN24D2F_AUTOPLAY].newValue === true) {
        void startBn24d2fAutopilot(bridgeHandler);
      } else {
        stopBn24d2fAutopilot();
      }
    }
    if (changes[STORAGE_BN24D2F_CONFIG] && bridgeHandler) {
      void readBn24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readBn24d2fAutopilotEnabled().then((on) => {
          if (on) void startBn24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeBn24d2fSignalRunner = {
    initBn24d2fSignalRunner,
    startBn24d2fAutopilot,
    stopBn24d2fAutopilot,
    setBn24d2fAutopilotEnabled,
    getBn24d2fAutopilotStatus,
    getBn24d2fConfigForPopup,
    setBn24d2fConfigFromPopup,
    resetBn24d2fStats,
    STORAGE_BN24D2F_AUTOPLAY,
    STORAGE_BN24D2F_CONFIG,
  };
}
