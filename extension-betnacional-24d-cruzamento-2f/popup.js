...
const ice2fStatus = document.getElementById("bn24d2fStatus");
...
const ice2fOnBtn = document.getElementById("bn24d2fOn");
const ice2fOffBtn = document.getElementById("bn24d2fOff");
const ice2fResetBtn = document.getElementById("bn24d2fReset");
const ice2fCasinoId = document.getElementById("bn24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("bn24d2fSave");
const bn24d2fConfigStatus = document.getElementById("bn24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, bn24d2fConfig) {
  ...
  const noGale =
    bn24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = bn24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? bn24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.bn24d2fAutopilot, mode, status?.bn24d2fConfig);

  const cfg = status?.bn24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "betnacional.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.bn24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (bn24d2fConfigStatus) {
    bn24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
