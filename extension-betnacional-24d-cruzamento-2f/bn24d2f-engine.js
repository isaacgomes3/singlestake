...
var SinglestakeBn24d2f = (() => {
  ...
  // extension-betnacional-24d-cruzamento-2f/bn24d2f-strategy-entry.ts
  var bn24d2f_strategy_entry_exports = {};
  __export(bn24d2f_strategy_entry_exports, {
    BN24D2F_MAX_GALES: () => BN24D2F_MAX_GALES,
    BN24D2F_MESA_URL: () => BN24D2F_MESA_URL,
    BN24D2F_TABLE_ID: () => BN24D2F_TABLE_ID,
    ...
    createBn24d2fEngine: () => createBn24d2fEngine,
    default: () => bn24d2f_strategy_entry_default,
  ...
  // extension-betnacional-24d-cruzamento-2f/bn24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var BN24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var BN24D2F_MESA_URL = "https://betnacional.bet.br/casino/game/";
  var BN24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var BN24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var BN24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var BN24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of BN24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createBn24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && BN24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && BN24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && BN24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && BN24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && BN24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && BN24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && BN24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = BN24D2F_MESA_URL) {
      ...
      const signalId = `bn24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: BN24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? BN24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeBn24d2f = api;
  }
  var bn24d2f_strategy_entry_default = api;
  return __toCommonJS(bn24d2f_strategy_entry_exports);
})();
