...
const GOG = {
  ...
  /** 2F cruzamento — pausa entre factor-1 e factor-2 após fichas. */
  ICE2F_FACTOR_CLICK_STAGGER_MS: 80,
  /** Bridge factor-1 → factor-2 (KTO/ICE 2F). */
...
/** Gales altos — ritmo de clique baseado nas unidades (ref. gale 3 = 8×150 ms). */
function kto2fUnitClickStaggerMs(units, recovery = 0) {
  const u = Math.max(1, Math.floor(units));
  const r = Math.max(0, Math.floor(recovery));
  const base = GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150;
  if (u <= KTO2F_GALE3_REFERENCE_UNITS) return base;
  const refFillMs = (KTO2F_GALE3_REFERENCE_UNITS - 1) * base;
  let stagger = Math.max(25, Math.round(refFillMs / (u - 1)));
  if (r >= 5) stagger = Math.max(10, Math.round(stagger / 3));
  else if (r >= 4) stagger = Math.max(12, Math.round(stagger / 2));
  return stagger;
}
  ...
  if (isKto2fCrossingContext(context) && context?.singleFactorMode !== true) {
    const units =
      typeof context?.units === "number" && Number.isFinite(context.units) && context.units > 0
        ...
        : 1;
    if (units > KTO2F_GALE3_REFERENCE_UNITS) return kto2fUnitClickStaggerMs(units, r);
    return GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? CLICK_STAGGER_BASE_MS / 3;
  }
  if (isDois2FatoresCrossingContext(context)) {
    if (r >= 4) {
      return kto2fUnitClickStaggerMs(ktoStakeUnitsForRecovery(r), r);
    }
