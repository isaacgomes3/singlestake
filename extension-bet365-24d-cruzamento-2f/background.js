importScripts("shared.js", "bet36524d2f-engine.js", "dga-hub.js", "bet36524d2f-signal-runner.js");
...
async function openOrFocusBet36524d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (bet36524d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(bet36524d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      bet36524d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    bet36524d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: bet36524d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogBet36524d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureBet36524d2fPanelOnOpenTabs();
  void openOrFocusBet36524d2fPanel();
});
...
/** Janela fixa do painel Bet365 24D 2F (não fecha ao clicar na mesa). */
let bet36524d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === bet36524d2fPanelWindowId) bet36524d2fPanelWindowId = null;
});

const BET36524D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const BET36524D_CALIB_SITE_KEY = "bet365.bet.br|pragmatic-24d-spin";
...
function isBet36524d2fGameUrl(url) {
  ...
  try {
    ...
    if (/pragmaticplaylive\.net|pragmaticplay\.net/i.test(host)) {
      return /24d|3426/i.test(u.pathname + u.search + u.hash) || true;
    }
    return false;
  } catch {
    return /bet365|24d.?spin/i.test(url);
  }
}

async function ensureBet36524d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isBet36524d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "bet36524d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-bet36524d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureBet36524d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isBet36524d2fGameUrl(tab.url)) {
      await ensureBet36524d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isBet36524d2fGameUrl(url)
  ) {
    registerMesaTab(BET36524D2F_DEFAULT_TABLE_ID, tabId);
    void ensureBet36524d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeBet36524d2fSignalRunner.setBet36524d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureBet36524d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusBet36524d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusBet36524d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeBet36524d2fSignalRunner.getBet36524d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeBet36524d2fSignalRunner.resetBet36524d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeBet36524d2fSignalRunner.getBet36524d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeBet36524d2fSignalRunner.setBet36524d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isBet36524d2fGameUrl(sender.tab.url)) {
      registerMesaTab(BET36524D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isBet36524d2fGameUrl(t.url)) ??
  ...
  const bet36524d2fAutopilot = await SinglestakeBet36524d2fSignalRunner.getBet36524d2fAutopilotStatus();
  const bet36524d2fConfig = await SinglestakeBet36524d2fSignalRunner.getBet36524d2fConfigForPopup();
  return {
    ok: true,
    mode,
    bet36524d2fAutopilot,
    bet36524d2fConfig,
...
function isBet36524d2fCrossingContext(context) {
  return context?.strategy === "bet36524d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isBet36524d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isBet36524d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isBet36524d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "bet36524d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isBet36524d2fSignal =
    isBet36524d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("bet36524d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isBet36524d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isBet36524d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isBet36524d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isBet36524d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isBet36524d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeBet36524d2f?.bet36524d2fPadFactorPlacementMs === "function"
          ? SinglestakeBet36524d2f.bet36524d2fPadFactorPlacementMs(padUnits)
...
function isCasinoPlayUrl(url) {
  return isBet36524d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "casino.bet365.bet.br" || host.endsWith(".bet365.bet.br") || host.endsWith(".bet365.com")) {
      return BET36524D_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[BET36524D_CALIB_SITE_KEY]) {
    return { siteKey: BET36524D_CALIB_SITE_KEY, site: sites[BET36524D_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: BET36524D_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[BET36524D_CALIB_SITE_KEY]) return { siteKey: BET36524D_CALIB_SITE_KEY, site: sites[BET36524D_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? BET36524D_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === BET36524D_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isBet36524d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeBet36524d2f !== "undefined" && SinglestakeBet36524d2f.BET36524D2F_MESA_URL) ||
          "https://casino.bet365.bet.br/play/24DSpin",
        mesaProvider: null,
        currentTableId: BET36524D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
...
SinglestakeBet36524d2fSignalRunner.initBet36524d2fSignalRunner(handleBridgePayload);
