...
const ice2fStatus = document.getElementById("bet36524d2fStatus");
...
const ice2fOnBtn = document.getElementById("bet36524d2fOn");
const ice2fOffBtn = document.getElementById("bet36524d2fOff");
const ice2fResetBtn = document.getElementById("bet36524d2fReset");
const ice2fCasinoId = document.getElementById("bet36524d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("bet36524d2fSave");
const bet36524d2fConfigStatus = document.getElementById("bet36524d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, bet36524d2fConfig) {
  ...
  const noGale =
    bet36524d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = bet36524d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? bet36524d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.bet36524d2fAutopilot, mode, status?.bet36524d2fConfig);

  const cfg = status?.bet36524d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "casino.bet365.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.bet36524d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (bet36524d2fConfigStatus) {
    bet36524d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
