...
var SinglestakeBet36524d2f = (() => {
  ...
  // extension-bet365-24d-cruzamento-2f/bet36524d2f-strategy-entry.ts
  var bet36524d2f_strategy_entry_exports = {};
  __export(bet36524d2f_strategy_entry_exports, {
    BET36524D2F_MAX_GALES: () => BET36524D2F_MAX_GALES,
    BET36524D2F_MESA_URL: () => BET36524D2F_MESA_URL,
    BET36524D2F_TABLE_ID: () => BET36524D2F_TABLE_ID,
    ...
    createBet36524d2fEngine: () => createBet36524d2fEngine,
    default: () => bet36524d2f_strategy_entry_default,
  ...
  var ICE_2F_ROULETTE_MESA_URL = "https://casino.bet365.bet.br/play/24DSpin";
  ...
  // extension-bet365-24d-cruzamento-2f/bet36524d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var BET36524D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var BET36524D2F_MESA_URL = ICE_2F_ROULETTE_MESA_URL;
  var BET36524D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var BET36524D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var BET36524D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var BET36524D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of BET36524D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createBet36524d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && BET36524D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && BET36524D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && BET36524D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && BET36524D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && BET36524D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && BET36524D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && BET36524D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = BET36524D2F_MESA_URL) {
      ...
      const signalId = `bet36524d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: BET36524D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? BET36524D2F_MESA_URL,
          mesaProvider: "outro",
          ...
          maxRecovery,
          executionMode: null,
          strategy: "bet36524d2fcruzamento",
  ...
  var api = {
    BET36524D2F_TABLE_ID,
    BET36524D2F_MESA_URL,
    BET36524D2F_MAX_GALES,
    ICE_2F_BET_DELAY_MS,
    ...
    createBet36524d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeBet36524d2f = api;
  }
  var bet36524d2f_strategy_entry_default = api;
  return __toCommonJS(bet36524d2f_strategy_entry_exports);
})();
