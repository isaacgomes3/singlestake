...
export const BET36524D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
export const BET36524D2F_MESA_URL = ICE_2F_ROULETTE_MESA_URL;
export const BET36524D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const BET36524D2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const BET36524D2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const BET36524D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
...
export function createBet36524d2fEngine(options: CreateIce2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && BET36524D2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        BET36524D2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && BET36524D2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && BET36524D2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && BET36524D2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        BET36524D2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        BET36524D2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = BET36524D2F_MESA_URL) {
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "bet36524d2fcruzamento" as const,
...
// named exports; sem isto SinglestakeBet36524d2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeBet36524d2f?: typeof api }).SinglestakeBet36524d2f = api;
}
