/** Autopilot Reals — Cruzamento 2F (237) · cruzamento sequencial 2 fatores. */
const STORAGE_REALS2F_AUTOPLAY = "gogReals2fAutopilotEnabled";
const STORAGE_REALS2F_CONFIG = "gogReals2fConfig";
const STORAGE_REALS2F_STATUS = "gogReals2fAutopilotStatus";
const STORAGE_REALS2F_STATS = "gogReals2fAutopilotSessionStats";
const STORAGE_REALS2F_MACHINE = "gogReals2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeReals2f['createReals2fEngine']>|null} */
let engine = null;
...
const REALS2F_DEFAULTS = {
  tableId: SinglestakeReals2f?.REALS2F_TABLE_ID ?? 237,
  mesaUrl:
    SinglestakeReals2f?.REALS2F_MESA_URL ??
...
function knownPairIds() {
  const known = SinglestakeReals2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeReals2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeReals2f !== "undefined"
      ? SinglestakeReals2f.applyIce2fEnabledPairIds
        ? SinglestakeReals2f
        : SinglestakeReals2f.default
      : null;
...
async function readReals2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_AUTOPLAY]);
  return data[STORAGE_REALS2F_AUTOPLAY] === true;
}

async function readReals2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_CONFIG]);
  const stored = data[STORAGE_REALS2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...REALS2F_DEFAULTS,
      maxRecoveryPreference: REALS2F_DEFAULTS.maxRecovery,
...
async function readReals2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_MACHINE]);
  const raw = data[STORAGE_REALS2F_MACHINE];
...
async function persistReals2fMachineState(machine) {
...
async function clearReals2fMachineState() {
  await chrome.storage.local.remove([STORAGE_REALS2F_MACHINE]);
}

async function readReals2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_STATS]);
  const raw = data[STORAGE_REALS2F_STATS];
...
async function persistReals2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_REALS2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeReals2fStatus({
    wins: stats.wins ?? 0,
...
async function resetReals2fStats() {
  const cfg = await readReals2fConfig();
  const empty =
    globalThis.SinglestakeReals2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistReals2fStats(empty, cfg.maxRecovery);
  await clearReals2fMachineState();
...
async function getReals2fConfigForPopup() {
  return readReals2fConfig();
}

async function setReals2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_CONFIG]);
  const stored =
    data[STORAGE_REALS2F_CONFIG] && typeof data[STORAGE_REALS2F_CONFIG] === "object"
      ? data[STORAGE_REALS2F_CONFIG]
      : {};
  const prev = await readReals2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_REALS2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearReals2fMachineState();
    await writeReals2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readReals2fConfig()) };
}

async function writeReals2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_STATUS]);
  const prev = data[STORAGE_REALS2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_REALS2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistReals2fMachineState(live.machine);
    await writeReals2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeReals2f?.ice2fBetDelayMs === "function"
      ? SinglestakeReals2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeReals2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeReals2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeReals2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `reals2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistReals2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistReals2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeReals2f?.ice2fWatchLabelForMachine ??
      SinglestakeReals2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeReals2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeReals2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startReals2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeReals2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readReals2fAutopilotEnabled();
  if (!enabled) {
    await writeReals2fStatus({ running: false, reason: "autopilot Reals 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeReals2f?.createReals2fEngine) {
    await writeReals2fStatus({
      running: false,
      reason: "reals2f-engine.js em falta — corra npm run extension:reals2f:build",
    });
    return;
  }

  stopReals2fAutopilot();

  const cfg = await readReals2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readReals2fStats(cfg.maxRecovery);
  const savedMachine = await readReals2fMachineState();
  engine = SinglestakeReals2f.createReals2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeReals2fStatus({ log: msg }),
    onStatus: (status) => void writeReals2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readReals2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readReals2fConfig().then((cfgNow) =>
...
function stopReals2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeReals2fStatus({ running: false, active: false, waitingBet: false });
}

async function setReals2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_REALS2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startReals2fAutopilot(bridgeHandler ?? globalThis.__singlestakeReals2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeReals2fBridgeHandler) {
      await writeReals2fStatus({
        running: false,
  ...
  return getReals2fAutopilotStatus();
}

async function getReals2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_REALS2F_STATUS, STORAGE_REALS2F_AUTOPLAY, STORAGE_REALS2F_STATS]);
  const statsPack = data[STORAGE_REALS2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_REALS2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_REALS2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_REALS2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_REALS2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readReals2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_REALS2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readReals2fConfig();
  const streak =
    globalThis.SinglestakeReals2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeReals2f?.ice2fPairLabel === "function"
        ? SinglestakeReals2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_REALS2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_REALS2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initReals2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeReals2fBridgeHandler = handleBridgePayload;
  void readReals2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readReals2fAutopilotEnabled().then((on) => {
    if (on) void startReals2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_REALS2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_REALS2F_AUTOPLAY].newValue === true) {
        void startReals2fAutopilot(bridgeHandler);
      } else {
        stopReals2fAutopilot();
      }
    }
    if (changes[STORAGE_REALS2F_CONFIG] && bridgeHandler) {
      void readReals2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readReals2fAutopilotEnabled().then((on) => {
          if (on) void startReals2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeReals2fSignalRunner = {
    initReals2fSignalRunner,
    startReals2fAutopilot,
    stopReals2fAutopilot,
    setReals2fAutopilotEnabled,
    getReals2fAutopilotStatus,
    getReals2fConfigForPopup,
    setReals2fConfigFromPopup,
    resetReals2fStats,
    STORAGE_REALS2F_AUTOPLAY,
    STORAGE_REALS2F_CONFIG,
  };
}
