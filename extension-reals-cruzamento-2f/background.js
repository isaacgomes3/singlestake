importScripts("shared.js", "reals2f-engine.js", "dga-hub.js", "reals2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogReals2fAutopilotEnabled: false });
  }
  void ensureReals2fPanelOnOpenTabs();
});
...
let reals2fPanelWindowId = null;

async function openOrFocusReals2fPanel() {
  if (reals2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(reals2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      reals2fPanelWindowId = null;
    }
  }
  ...
  reals2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusReals2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === reals2fPanelWindowId) reals2fPanelWindowId = null;
});

const REALS2F_DEFAULT_TABLE_ID = 237;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const REALS2F_CALIB_SITE_KEY = "reals.bet.br|pragmatic-roulette";

function isReals2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "reals.bet.br" || host.endsWith(".reals.bet.br");
  } catch {
    return /reals\.bet\.br/i.test(url);
  }
}

function isReals2fRoulettePageUrl(url) {
  if (!isReals2fHost(url)) return false;
  try {
    const u = new URL(url);
    const path = `${u.pathname}${u.hash}${u.search}`.toLowerCase();
    return /\/play\/liveCassino\/237/i.test(path) || /brazilian|roulette/i.test(path);
  } catch {
    return /reals\.bet\.br/i.test(url);
  }
}

async function ensureReals2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isReals2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "reals2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-reals2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureReals2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isReals2fRoulettePageUrl(tab.url)) {
      await ensureReals2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isReals2fRoulettePageUrl(url)
  ) {
    registerMesaTab(REALS2F_DEFAULT_TABLE_ID, tabId);
    void ensureReals2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-reals2f-autopilot") {
    void SinglestakeReals2fSignalRunner.setReals2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureReals2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-reals2f-autopilot") {
    void SinglestakeReals2fSignalRunner.getReals2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-reals2f-stats") {
    void SinglestakeReals2fSignalRunner.resetReals2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-reals2f-config") {
    void SinglestakeReals2fSignalRunner.getReals2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-reals2f-config") {
    void SinglestakeReals2fSignalRunner.setReals2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isReals2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(REALS2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isReals2fRoulettePageUrl(t.url)) ??
  ...
  const reals2fAutopilot = await SinglestakeReals2fSignalRunner.getReals2fAutopilotStatus();
  const reals2fConfig = await SinglestakeReals2fSignalRunner.getReals2fConfigForPopup();
  return {
    ok: true,
    mode,
    reals2fAutopilot,
...
function isReals2fCrossingContext(context) {
  return context?.strategy === "reals2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isReals2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.REALS2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isReals2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.REALS2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isReals2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "reals2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isReals2fSignal =
    isReals2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("reals2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isReals2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isReals2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isReals2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isReals2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isReals2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeReals2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeReals2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (REALS2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isReals2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Brazilian Roulette no Reals num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "reals.bet.br" || host.endsWith(".reals.bet.br")) {
      return REALS2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[REALS2F_CALIB_SITE_KEY]) {
    return { siteKey: REALS2F_CALIB_SITE_KEY, site: sites[REALS2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: REALS2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isReals2fHost(url)) {
    for (const [key, site] of Object.entries(sites)) {
      if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
    }
    if (sites[REALS2F_CALIB_SITE_KEY]) return { siteKey: REALS2F_CALIB_SITE_KEY, site: sites[REALS2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isReals2fHost(tab.url) ? REALS2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === REALS2F_CALIB_SITE_KEY && isReals2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isReals2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isReals2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isReals2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Reals encontrada. Abra Brazilian Roulette no Reals num separador normal do Chrome (não neste painel).",
    };
  }
...
SinglestakeReals2fSignalRunner.initReals2fSignalRunner(handleBridgePayload);
