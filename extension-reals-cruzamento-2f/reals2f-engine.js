...
var SinglestakeReals2f = (() => {
  ...
  // extension-reals-cruzamento-2f/reals2f-strategy-entry.ts
  var reals2f_strategy_entry_exports = {};
  __export(reals2f_strategy_entry_exports, {
    ...
    REALS2F_MAX_GALES: () => REALS2F_MAX_GALES,
    REALS2F_MESA_URL: () => REALS2F_MESA_URL,
    REALS2F_TABLE_ID: () => REALS2F_TABLE_ID,
    ...
    createReals2fEngine: () => createReals2fEngine,
    default: () => reals2f_strategy_entry_default,
  ...
  // extension-reals-cruzamento-2f/reals2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var REALS2F_TABLE_ID = 237;
  var REALS2F_MESA_URL = "https://reals.bet.br/play/liveCassino/237?title=Brazilian+Roulette&provider=pragmatic&gameId=%22237%22&category=live";
  var REALS2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var REALS2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var REALS2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var REALS2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of REALS2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createReals2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && REALS2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && REALS2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && REALS2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && REALS2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && REALS2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && REALS2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && REALS2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = REALS2F_MESA_URL) {
      ...
      const signalId = `reals2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `Reals 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `Reals 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `Reals 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: REALS2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? REALS2F_MESA_URL,
          mesaProvider: "outro",
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeReals2f = api;
  }
  var reals2f_strategy_entry_default = api;
  return __toCommonJS(reals2f_strategy_entry_exports);
})();
