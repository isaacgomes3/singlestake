/**
 * Painel flutuante na Reals 2F — activar estratégia 2 Fatores · cruzamento sequencial (mesa 237).
 */
(function () {
 ...
 * Painel flutuante na Reals 2F — activar estratégia 2 Fatores · cruzamento sequencial (mesa 237).
 */
function pageIsReals2fRoulette() {
    if (!/reals\.bet\.br/i.test(location.hostname)) return false;
    const path = `${location.pathname}${location.hash}${location.search}`.toLowerCase();
    return /\/play\/liveCassino\/237/i.test(path) || /roulette|pragmatic|casino|live/i.test(path);
  }

  if (!pageIsReals2fRoulette()) return;
  if (document.getElementById("ss-reals2f-panel-root")) return;

  const PANEL_ID = "ss-reals2f-panel-root";
  const POLL_MS = 2000;
  let pollTimer = null;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.kind === "reals2f-panel-ping") {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .ss-reals2f-head:active { cursor: grabbing; }
    #${PANEL_ID} .ss-reals2f-title {
      font-size: 11px;
      font-weight: 800;
    ...
    #${PANEL_ID} .ss-reals2f-body {
      padding: 10px 12px 12px;
      overflow: auto;
      flex: 1;
      min-height: 0;
    }
    #${PANEL_ID} .ss-reals2f-sub {
      margin: 0 0 8px;
      font-size: 11px;
      color: #94a3b8;
      line-height: 1.45;
    }
    #${PANEL_ID} .ss-reals2f-signal {
      margin: 0 0 10px;
      padding: 12px;
      border-radius: 10px;
    ...
    #${PANEL_ID} .ss-reals2f-signal-idle {
      font-size: 14px;
      font-weight: 600;
      color: #94a3b8;
      line-height: 1.4;
    }
    #${PANEL_ID} .ss-reals2f-watch {
      margin-top: 8px;
      font-size: 10px;
      font-weight: 500;
      color: #64748b;
    ...
    #${PANEL_ID} .ss-reals2f-indication {
      font-size: 20px;
      font-weight: 800;
      color: #f8fafc;
      letter-spacing: 0.04em;
    ...
    #${PANEL_ID} .ss-reals2f-gale.entry {
      background: #172554;
      color: #bfdbfe;
      border: 1px solid #2563eb;
    }
    #${PANEL_ID} .ss-reals2f-gale.recovery {
      background: #451a03;
      color: #fcd34d;
      border: 1px solid #b45309;
    }
    #${PANEL_ID} .ss-reals2f-gale.wait {
      background: #1e293b;
      color: #cbd5e1;
      border: 1px solid #475569;
    }
    #${PANEL_ID} .ss-reals2f-status {
      margin: 0 0 10px;
      font-size: 11px;
      color: #cbd5e1;
      min-height: 2.2em;
    ...
    #${PANEL_ID} .ss-reals2f-status.err { color: #fca5a5; }
    #${PANEL_ID} .ss-reals2f-stats {
      display: flex;
      gap: 8px;
      margin-bottom: 10px;
    }
    #${PANEL_ID} .ss-reals2f-stat {
      flex: 1;
      text-align: center;
      padding: 6px;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-reals2f-stat strong {
      display: block;
      font-size: 22px;
      color: #f8fafc;
    }
    #${PANEL_ID} .ss-reals2f-stat small {
      font-size: 9px;
      font-weight: 700;
      text-transform: uppercase;
      color: #94a3b8;
    }
    #${PANEL_ID} .ss-reals2f-stat.win { border-color: #166534; background: #052e16; }
    #${PANEL_ID} .ss-reals2f-stat.win strong { color: #86efac; }
    #${PANEL_ID} .ss-reals2f-stat.loss { border-color: #991b1b; background: #450a0a; }
    #${PANEL_ID} .ss-reals2f-stat.loss strong { color: #fca5a5; }
    #${PANEL_ID} .ss-reals2f-nogale {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 10px;
    ...
    #${PANEL_ID} .ss-reals2f-nogale.on {
      border-color: #b45309;
      background: #1c1410;
    }
    #${PANEL_ID} .ss-reals2f-nogale-copy {
      display: flex;
      flex-direction: column;
      gap: 2px;
      min-width: 0;
    }
    #${PANEL_ID} .ss-reals2f-nogale-label {
      font-size: 11px;
      font-weight: 700;
      color: #e2e8f0;
    }
    #${PANEL_ID} .ss-reals2f-nogale-hint {
      font-size: 9px;
      color: #94a3b8;
      line-height: 1.35;
    }
    #${PANEL_ID} .ss-reals2f-switch {
      position: relative;
      width: 40px;
      height: 22px;
      flex-shrink: 0;
    }
    #${PANEL_ID} .ss-reals2f-switch input {
      opacity: 0;
      width: 0;
      height: 0;
    }
    #${PANEL_ID} .ss-reals2f-slider {
      position: absolute;
      inset: 0;
      cursor: pointer;
      background: #334155;
    ...
    #${PANEL_ID} .ss-reals2f-slider::before {
      content: "";
      position: absolute;
      height: 16px;
      width: 16px;
    ...
    #${PANEL_ID} .ss-reals2f-switch input:checked + .ss-reals2f-slider {
      background: #b45309;
    }
    #${PANEL_ID} .ss-reals2f-switch input:checked + .ss-reals2f-slider::before {
      transform: translateX(18px);
    }
    #${PANEL_ID} .ss-reals2f-chart-wrap {
      margin: 0 0 10px;
      padding: 8px;
      border-radius: 10px;
    ...
    #${PANEL_ID} .ss-reals2f-chart-title {
      margin: 0 0 6px;
      font-size: 10px;
      font-weight: 700;
      color: #94a3b8;
    ...
    #${PANEL_ID} .ss-reals2f-chart-metrics {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 6px;
      margin-bottom: 8px;
    }
    #${PANEL_ID} .ss-reals2f-chart-metric {
      display: flex;
      justify-content: space-between;
      gap: 6px;
      padding: 6px 8px;
    ...
    #${PANEL_ID} .ss-reals2f-chart-metric span { color: #94a3b8; }
    #${PANEL_ID} .ss-reals2f-chart-metric strong { font-variant-numeric: tabular-nums; }
    #${PANEL_ID} .ss-reals2f-chart-metric strong.up { color: #86efac; }
    #${PANEL_ID} .ss-reals2f-chart-metric strong.down { color: #fca5a5; }
    #${PANEL_ID} .ss-reals2f-chart-row {
      display: block;
    }
    #${PANEL_ID} .ss-reals2f-chart-canvas-box {
      position: relative;
      height: 140px;
      border-radius: 8px;
      border: 1px solid #1e293b;
    ...
    #${PANEL_ID} .ss-reals2f-chart-canvas-box canvas {
      width: 100%;
      height: 100%;
      display: block;
    }
    #${PANEL_ID} .ss-reals2f-pair-title {
      margin: 0 0 6px;
      font-size: 10px;
      font-weight: 700;
      color: #94a3b8;
    ...
    #${PANEL_ID} .ss-reals2f-pair-head,
    #${PANEL_ID} .ss-reals2f-pair-row {
      display: grid;
    ...
    #${PANEL_ID} .ss-reals2f-pair-id { font-weight: 700; color: #cbd5e1; }
    #${PANEL_ID} .ss-reals2f-pair-row.off .ss-reals2f-pair-id { color: #64748b; }
    #${PANEL_ID} .ss-reals2f-pair-enable {
      position: relative;
      width: 34px;
      height: 18px;
      flex-shrink: 0;
    }
    #${PANEL_ID} .ss-reals2f-pair-enable input {
      opacity: 0;
      width: 0;
      height: 0;
    }
    #${PANEL_ID} .ss-reals2f-pair-enable .ss-reals2f-slider {
      position: absolute;
      inset: 0;
      background: #334155;
      border-radius: 999px;
    ...
    #${PANEL_ID} .ss-reals2f-pair-enable .ss-reals2f-slider::before {
      content: "";
      position: absolute;
      width: 14px;
      height: 14px;
    ...
    #${PANEL_ID} .ss-reals2f-pair-enable input:checked + .ss-reals2f-slider {
      background: #15803d;
    }
    #${PANEL_ID} .ss-reals2f-pair-enable input:checked + .ss-reals2f-slider::before {
      transform: translateX(16px);
    }
    #${PANEL_ID} .ss-reals2f-pair-ok,
    #${PANEL_ID} .ss-reals2f-pair-bad {
      text-align: center;
      font-weight: 700;
      font-variant-numeric: tabular-nums;
    }
    #${PANEL_ID} .ss-reals2f-pair-ok { color: #86efac; }
    #${PANEL_ID} .ss-reals2f-pair-bad { color: #fca5a5; }
    #${PANEL_ID} .ss-reals2f-pair-head .ss-reals2f-pair-ok,
    #${PANEL_ID} .ss-reals2f-pair-head .ss-reals2f-pair-bad,
    #${PANEL_ID} .ss-reals2f-pair-head .ss-reals2f-pair-en {
      color: #64748b;
      text-align: center;
    }
    #${PANEL_ID} .ss-reals2f-actions {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }
    #${PANEL_ID} .ss-reals2f-btn {
      flex: 1;
      min-width: 72px;
      border: 1px solid #334155;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-reals2f-btn.on-demo {
      border-color: #2563eb;
      background: #172554;
      color: #bfdbfe;
    }
    #${PANEL_ID} .ss-reals2f-btn.on-real {
      border-color: #16a34a;
      background: #14532d;
      color: #bbf7d0;
    }
    #${PANEL_ID} .ss-reals2f-btn.on-active {
      border-color: #059669;
      background: #064e3b;
      color: #a7f3d0;
    }
    #${PANEL_ID} .ss-reals2f-btn.on-warn {
      border-color: #b45309;
      background: #451a03;
      color: #fcd34d;
    }
    #${PANEL_ID} .ss-reals2f-hidden { display: none !important; }
    #${PANEL_ID}.ss-reals2f-collapsed .ss-reals2f-body { display: none; }
  `;

  const root = el("div");
  root.id = PANEL_ID;

  const card = el("div", "ss-reals2f-card");
  const head = el("div", "ss-reals2f-head");
  const title = el("div", "ss-reals2f-title", "stake37 · 2 Fatores");
  const minBtn = el("button", "ss-reals2f-min", "−");
  minBtn.type = "button";
  head.append(title, minBtn);

  const body = el("div", "ss-reals2f-body");
  const sub = el("p", "ss-reals2f-sub", "Brazilian Roulette · mesa 237 · stake 2·4·8·16·32·64");

  const signalBox = el("div", "ss-reals2f-signal");
  const signalIdle = el("div", "ss-reals2f-signal-idle", "Sem sinal — aguarda pares 2F");
  const signalWatch = el("div", "ss-reals2f-watch", "");
  const signalPos = el("div", "ss-reals2f-pos", "—");
  const signalAxis = el("div", "ss-reals2f-axis", "");
  const signalIndication = el("div", "ss-reals2f-indication", "—");
  const signalGale = el("div", "ss-reals2f-gale entry", "ENTRADA");
  signalBox.append(signalIdle, signalWatch, signalPos, signalAxis, signalIndication, signalGale);
  signalIdle.classList.add("ss-reals2f-hidden");
  signalWatch.classList.add("ss-reals2f-hidden");
  signalPos.classList.add("ss-reals2f-hidden");
  signalAxis.classList.add("ss-reals2f-hidden");
  signalIndication.classList.add("ss-reals2f-hidden");
  signalGale.classList.add("ss-reals2f-hidden");

  const statusEl = el("p", "ss-reals2f-status", "A ligar à extensão…");

  const stats = el("div", "ss-reals2f-stats");
  const winStat = el("div", "ss-reals2f-stat win");
  const winNum = el("strong", null, "0");
  winStat.append(winNum, el("small", null, "Vitórias"));
  const lossStat = el("div", "ss-reals2f-stat loss");
  const lossNum = el("strong", null, "0");
  lossStat.append(lossNum, el("small", null, "Derrotas"));
  stats.append(winStat, lossStat);

  const noGaleRow = el("div", "ss-reals2f-nogale");
  const noGaleCopy = el("div", "ss-reals2f-nogale-copy");
  noGaleCopy.append(
    el("span", "ss-reals2f-nogale-label", "Modo sem gale"),
    el("span", "ss-reals2f-nogale-hint", "Stake única · W/L sem recuperação"),
  );
  const noGaleSwitch = el("label", "ss-reals2f-switch");
  const noGaleInput = document.createElement("input");
  noGaleInput.type = "checkbox";
  noGaleInput.setAttribute("aria-label", "Modo sem gale");
  const noGaleSlider = el("span", "ss-reals2f-slider");
  noGaleSwitch.append(noGaleInput, noGaleSlider);
  noGaleRow.append(noGaleCopy, noGaleSwitch);

  const observeRow = el("div", "ss-reals2f-nogale");
  const observeCopy = el("div", "ss-reals2f-nogale-copy");
  observeCopy.append(
    el("span", "ss-reals2f-nogale-label", "Sem clique"),
    el("span", "ss-reals2f-nogale-hint", "Só observação — sem apostar na mesa"),
  );
  const observeSwitch = el("label", "ss-reals2f-switch");
  const observeInput = document.createElement("input");
  observeInput.type = "checkbox";
  observeInput.setAttribute("aria-label", "Sem clique");
  const observeSlider = el("span", "ss-reals2f-slider");
  observeSwitch.append(observeInput, observeSlider);
  observeRow.append(observeCopy, observeSwitch);

  const chartWrap = el("div", "ss-reals2f-chart-wrap");
  chartWrap.append(el("div", "ss-reals2f-chart-title", "Run-up & Drawdown"));
  const chartMetrics = el("div", "ss-reals2f-chart-metrics");
  const mConsec = el("div", "ss-reals2f-chart-metric");
  const mConsecVal = el("strong", "up", "0");
  mConsec.append(el("span", null, "Consecutivas"), mConsecVal);
  const mMaxUp = el("div", "ss-reals2f-chart-metric");
  const mMaxUpVal = el("strong", "up", "0");
  mMaxUp.append(el("span", null, "Máx. run-up"), mMaxUpVal);
  const mDdNow = el("div", "ss-reals2f-chart-metric");
  const mDdNowVal = el("strong", "down", "0");
  mDdNow.append(el("span", null, "Drawdown atual"), mDdNowVal);
  const mDdMax = el("div", "ss-reals2f-chart-metric");
  ...
  const chartRow = el("div", "ss-reals2f-chart-row");
  const canvasBox = el("div", "ss-reals2f-chart-canvas-box");
  ...
  const pairTitle = el("div", "ss-reals2f-pair-title", "Gatilho (OK/ERR · activar)");
  const pairBoard = el("div", "ss-reals2f-pairs");
  const pairHead = el("div", "ss-reals2f-pair-head");
  pairHead.append(
    el("span"),
    el("span", "ss-reals2f-pair-en", "ON"),
    el("span", "ss-reals2f-pair-ok", "OK"),
    el("span", "ss-reals2f-pair-bad", "ERR"),
  );
  ...
  function ensurePairRow(id, label) {
    if (pairCells[id]) {
      const idEl = pairCells[id].row.querySelector(".ss-reals2f-pair-id");
      if (idEl && label) idEl.textContent = label;
      return pairCells[id];
    }
    const row = el("div", "ss-reals2f-pair-row");
    const enable = el("label", "ss-reals2f-pair-enable");
    const input = document.createElement("input");
    input.type = "checkbox";
    input.setAttribute("aria-label", `Activar ${label || id}`);
    const slider = el("span", "ss-reals2f-slider");
    enable.append(input, slider);
    const okEl = el("span", "ss-reals2f-pair-ok", "0");
    const badEl = el("span", "ss-reals2f-pair-bad", "0");
    row.append(el("span", "ss-reals2f-pair-id", label || id), enable, okEl, badEl);
    ...
    input.addEventListener("change", async () => {
      ...
      input.disabled = true;
      const r = await send("set-reals2f-config", { config: { enabledPairIds: enabled } });
      if (r?.ok === false) {
        input.checked = !want;
  ...
  const actions = el("div", "ss-reals2f-actions");
  const btnDemo = el("button", "ss-reals2f-btn on-demo", "Demo");
  const btnReal = el("button", "ss-reals2f-btn", "Real");
  const btnToggle = el("button", "ss-reals2f-btn", "Parado");
  const btnReload = el("button", "ss-reals2f-btn on-warn ss-reals2f-hidden", "Actualizar página");
  btnDemo.type = "button";
  btnReal.type = "button";
  ...
  minBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    collapsed = !collapsed;
    root.classList.toggle("ss-reals2f-collapsed", collapsed);
  ...
  btnToggle.addEventListener("click", async () => {
    ...
    const cur = await send("get-reals2f-autopilot");
    const on = cur?.enabled === true;
    statusEl.textContent = on ? "A parar…" : "A ligar estratégia…";
    const r = await send("set-reals2f-autopilot", { enabled: !on });
  ...
  noGaleInput.addEventListener("change", async () => {
    ...
    const cfg = await send("get-reals2f-config");
    const r = await send("set-reals2f-config", {
      config: {
  ...
  const LAYOUT_KEY = "ssReals2fPanelLayout";
  function loadLayout() {
    try {
  ...
  function renderSignalBlock(st) {
    ...
    if (!hasActive) {
      signalIdle.classList.remove("ss-reals2f-hidden");
      ...
      if (st.watchLabel) {
        signalWatch.classList.remove("ss-reals2f-hidden");
        signalWatch.textContent = `Placar: ${st.watchLabel}`;
      }
      return;
    }

    signalGale.classList.remove("ss-reals2f-hidden");

    if (parsed.isPause) {
      signalIndication.classList.remove("ss-reals2f-hidden");
      signalIndication.textContent = "ZERO — PAUSA";
      signalGale.className = "ss-reals2f-gale wait";
      signalGale.textContent = parsed.recovery > 0 ? `GALE ${parsed.recovery} MANTIDO` : "AGUARDA REF.";
    } else {
      if (parsed.position) {
        signalPos.classList.remove("ss-reals2f-hidden");
      ...
      if (parsed.axis) {
        ...
        if (ax) {
          signalAxis.classList.remove("ss-reals2f-hidden");
    ...
    if (st.waitingBet && !parsed.isPause) {
      ...
      signalGale.className = "ss-reals2f-gale recovery";
      signalGale.textContent = `GALE ${parsed.recovery}`;
    } else {
      signalGale.className = "ss-reals2f-gale entry";
      signalGale.textContent =
        st.observeOnly === true
          ? "OBSERVAÇÃO"
  ...
  function renderStatus(data) {
    ...
    if (!data || data.error) {
      ...
      btnToggle.className = "ss-reals2f-btn on-warn";
      btnToggle.textContent = "Parado";
      if (contextDead) btnReload.classList.remove("ss-reals2f-hidden");
      return;
    }

    btnReload.classList.add("ss-reals2f-hidden");

    const mode = data.mode === "real" ? "real" : "demo";
    const ice2f = data.reals2fAutopilot;
    const st = ice2f?.status ?? {};
    const on = ice2f?.enabled === true;
    const running = st.running === true;
    const engaged = on && (running || st.active === true || st.waitingBet === true);

    btnDemo.className = `ss-reals2f-btn ${mode === "demo" ? "on-demo" : ""}`;
    btnReal.className = `ss-reals2f-btn ${mode === "real" ? "on-real" : ""}`;

    if (engaged) {
      btnToggle.className = "ss-reals2f-btn on-active";
      btnToggle.textContent = running ? "Activo" : "A ligar…";
    } else if (on) {
      btnToggle.className = "ss-reals2f-btn on-warn";
      btnToggle.textContent = "A ligar…";
    } else {
      btnToggle.className = "ss-reals2f-btn";
      btnToggle.textContent = "Parado";
    }
    ...
    const noGale =
      st.noGale === true ||
      data.reals2fConfig?.noGale === true ||
      st.maxRecovery === 0;
    noGaleInput.checked = noGale;
    noGaleRow.classList.toggle("on", noGale);

    const observeOnly =
      st.observeOnly === true || data.reals2fConfig?.observeOnly === true;
    ...
    const enabledSet = new Set(
      Array.isArray(st.enabledPairIds) && st.enabledPairIds.length > 0
        ? st.enabledPairIds
        : Array.isArray(data.reals2fConfig?.enabledPairIds)
          ? data.reals2fConfig.enabledPairIds
          : ["2x4"],
    );
