/**
 * Bundle entry — motor Reals Cruzamento 2F para extensão Chrome.
 */
...
const REALS2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const REALS2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const REALS2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));

export type Reals2fPersistedMachine = {
...
function parsePersistedWatch(
  raw: Reals2fPersistedMachine["watch"],
...
export type CreateReals2fEngineOptions = {
  maxRecovery?: number;
  initialStats?: RotatingRoomSessionStats | null;
  initialMachine?: Reals2fPersistedMachine | null;
};
...
function pendingRecoveryFromSaved(saved: Reals2fPersistedMachine): number {
  const stored =
...
export type Reals2fEngineSpinResult = {
...
export function createReals2fEngine(options: CreateReals2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && REALS2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        REALS2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && REALS2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
      ...(restoredWatch ? { watch: restoredWatch } : {}),
    };
  }
  let pendingRestore: Reals2fPersistedMachine | null = options.initialMachine ?? null;
  let stats =
  ...
  function runTick(): Reals2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && REALS2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && REALS2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        REALS2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        REALS2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = REALS2F_MESA_URL) {
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `Reals 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `Reals 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `Reals 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "reals2fcruzamento" as const,
...
// named exports; sem isto SinglestakeReals2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeReals2f?: typeof api }).SinglestakeReals2f = api;
}
