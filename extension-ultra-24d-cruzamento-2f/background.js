importScripts("shared.js", "ultra24d2f-engine.js", "dga-hub.js", "ultra24d2f-signal-runner.js");
...
async function openOrFocusUltra24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (ultra24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(ultra24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      ultra24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    ultra24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: ultra24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogUltra24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureUltra24d2fPanelOnOpenTabs();
  void openOrFocusUltra24d2fPanel();
});
...
/** Janela fixa do painel Ultra 24D 2F (não fecha ao clicar na mesa). */
let ultra24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === ultra24d2fPanelWindowId) ultra24d2fPanelWindowId = null;
});

const ULTRA24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const ULTRA24D2F_CALIB_SITE_KEY = "ultra.bet.br|pragmatic-24d-spin";
...
function isUltra24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "ultra.bet.br" || host.endsWith(".ultra.bet.br"))) return false;
    return /\/jogo\/\d+\/pragmatic-play-live\/24d[-_]?spin/i.test(path) || /24d[-_]?spin|24dspin|3426/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureUltra24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isUltra24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "ultra24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-ultra24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureUltra24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isUltra24d2fGameUrl(tab.url)) {
      await ensureUltra24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isUltra24d2fGameUrl(url)
  ) {
    registerMesaTab(ULTRA24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureUltra24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeUltra24d2fSignalRunner.setUltra24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureUltra24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusUltra24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusUltra24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeUltra24d2fSignalRunner.getUltra24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeUltra24d2fSignalRunner.resetUltra24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeUltra24d2fSignalRunner.getUltra24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeUltra24d2fSignalRunner.setUltra24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isUltra24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(ULTRA24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isUltra24d2fGameUrl(t.url)) ??
  ...
  const ultra24d2fAutopilot = await SinglestakeUltra24d2fSignalRunner.getUltra24d2fAutopilotStatus();
  const ultra24d2fConfig = await SinglestakeUltra24d2fSignalRunner.getUltra24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    ultra24d2fAutopilot,
...
function isUltra24d2fCrossingContext(context) {
  return context?.strategy === "ultra24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isUltra24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isUltra24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isUltra24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "ultra24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isUltra24d2fSignal =
    isUltra24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("ultra24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isUltra24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isUltra24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isUltra24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isUltra24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isUltra24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeUltra24d2f?.ultra24d2fPadFactorPlacementMs === "function"
          ? SinglestakeUltra24d2f.ultra24d2fPadFactorPlacementMs(padUnits)
...
function isCasinoPlayUrl(url) {
  return isUltra24d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "ultra.bet.br" || host.endsWith(".ultra.bet.br") || host.endsWith(".ultra.bet.br")) {
      return ULTRA24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[ULTRA24D2F_CALIB_SITE_KEY]) {
    return { siteKey: ULTRA24D2F_CALIB_SITE_KEY, site: sites[ULTRA24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: ULTRA24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[ULTRA24D2F_CALIB_SITE_KEY]) return { siteKey: ULTRA24D2F_CALIB_SITE_KEY, site: sites[ULTRA24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? ULTRA24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === ULTRA24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isUltra24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeUltra24d2f !== "undefined" && SinglestakeUltra24d2f.ULTRA24D2F_MESA_URL) ||
          "https://ultra.bet.br/",
        mesaProvider: null,
        currentTableId: ULTRA24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Ultra 24D encontrada. Abra o 24D Spin (ultra.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeUltra24d2fSignalRunner.initUltra24d2fSignalRunner(handleBridgePayload);
