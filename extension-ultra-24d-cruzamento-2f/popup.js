...
const ice2fStatus = document.getElementById("ultra24d2fStatus");
...
const ice2fOnBtn = document.getElementById("ultra24d2fOn");
const ice2fOffBtn = document.getElementById("ultra24d2fOff");
const ice2fResetBtn = document.getElementById("ultra24d2fReset");
const ice2fCasinoId = document.getElementById("ultra24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("ultra24d2fSave");
const ultra24d2fConfigStatus = document.getElementById("ultra24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, ultra24d2fConfig) {
  ...
  const noGale =
    ultra24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = ultra24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? ultra24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.ultra24d2fAutopilot, mode, status?.ultra24d2fConfig);

  const cfg = status?.ultra24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "ultra.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.ultra24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (ultra24d2fConfigStatus) {
    ultra24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
