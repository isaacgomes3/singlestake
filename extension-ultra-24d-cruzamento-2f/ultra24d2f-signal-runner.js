/** Autopilot Ultra — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_ULTRA24D2F_AUTOPLAY = "gogUltra24d2fAutopilotEnabled";
const STORAGE_ULTRA24D2F_CONFIG = "gogUltra24d2fConfig";
const STORAGE_ULTRA24D2F_STATUS = "gogUltra24d2fAutopilotStatus";
const STORAGE_ULTRA24D2F_STATS = "gogUltra24d2fAutopilotSessionStats";
const STORAGE_ULTRA24D2F_MACHINE = "gogUltra24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeUltra24d2f['createUltra24d2fEngine']>|null} */
let engine = null;
...
function bumpUltra24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function ultra24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeUltra24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeUltra24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const ULTRA24D2F_DEFAULTS = {
  tableId: SinglestakeUltra24d2f?.ULTRA24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeUltra24d2f?.ULTRA24D2F_MESA_URL ??
    "https://ultra.bet.br/",
...
function knownPairIds() {
  const known = SinglestakeUltra24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeUltra24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeUltra24d2f !== "undefined"
      ? SinglestakeUltra24d2f.applyIce2fEnabledPairIds
        ? SinglestakeUltra24d2f
        : SinglestakeUltra24d2f.default
      : null;
...
async function readUltra24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_AUTOPLAY]);
  return data[STORAGE_ULTRA24D2F_AUTOPLAY] === true;
}

async function readUltra24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_CONFIG]);
  const stored = data[STORAGE_ULTRA24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...ULTRA24D2F_DEFAULTS,
      maxRecoveryPreference: ULTRA24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? ULTRA24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? ULTRA24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : ULTRA24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? ULTRA24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : ULTRA24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : ULTRA24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : ULTRA24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : ULTRA24D2F_DEFAULTS.currency,
    noGale,
...
async function readUltra24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_MACHINE]);
  const raw = data[STORAGE_ULTRA24D2F_MACHINE];
...
async function persistUltra24d2fMachineState(machine) {
...
async function clearUltra24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_ULTRA24D2F_MACHINE]);
}

async function readUltra24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_STATS]);
  const raw = data[STORAGE_ULTRA24D2F_STATS];
...
async function persistUltra24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_ULTRA24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, ultra24d2fIdleIndicationPatch());
  }
  await writeUltra24d2fStatus(patch);
}

async function resetUltra24d2fStats() {
  const cfg = await readUltra24d2fConfig();
  const empty =
    globalThis.SinglestakeUltra24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistUltra24d2fStats(empty, cfg.maxRecovery);
  await clearUltra24d2fMachineState();
...
async function getUltra24d2fConfigForPopup() {
  return readUltra24d2fConfig();
}

async function setUltra24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_CONFIG]);
  const stored =
    data[STORAGE_ULTRA24D2F_CONFIG] && typeof data[STORAGE_ULTRA24D2F_CONFIG] === "object"
      ? data[STORAGE_ULTRA24D2F_CONFIG]
      : {};
  const prev = await readUltra24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_ULTRA24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearUltra24d2fMachineState();
    await writeUltra24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readUltra24d2fConfig()) };
}

async function writeUltra24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_STATUS]);
  const prev = data[STORAGE_ULTRA24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_ULTRA24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeUltra24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistUltra24d2fMachineState(live.machine);
    await writeUltra24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readUltra24d2fConfig();
    ...
    await writeUltra24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readUltra24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeUltra24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeUltra24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeUltra24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeUltra24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeUltra24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `ultra24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistUltra24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistUltra24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpUltra24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeUltra24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeUltra24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeUltra24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeUltra24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeUltra24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startUltra24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeUltra24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readUltra24d2fAutopilotEnabled();
  if (!enabled) {
    await writeUltra24d2fStatus({ running: false, reason: "autopilot Ultra 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeUltra24d2f?.createUltra24d2fEngine) {
    await writeUltra24d2fStatus({
      running: false,
      reason: "ultra24d2f-engine.js em falta — corra npm run extension:ultra24d2f:build",
    });
    return;
  }

  stopUltra24d2fAutopilot();

  const cfg = await readUltra24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readUltra24d2fStats(cfg.maxRecovery);
  const savedMachine = await readUltra24d2fMachineState();
  engine = SinglestakeUltra24d2f.createUltra24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeUltra24d2fStatus({ log: msg }),
    onStatus: (status) => void writeUltra24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readUltra24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readUltra24d2fConfig().then((cfgNow) =>
...
function stopUltra24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpUltra24d2fIndicationEpoch();
  void writeUltra24d2fStatus(ultra24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setUltra24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_ULTRA24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startUltra24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeUltra24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeUltra24d2fBridgeHandler) {
      await writeUltra24d2fStatus({
        running: false,
  ...
  const status = await getUltra24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getUltra24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA24D2F_STATUS, STORAGE_ULTRA24D2F_AUTOPLAY, STORAGE_ULTRA24D2F_STATS]);
  const statsPack = data[STORAGE_ULTRA24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_ULTRA24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_ULTRA24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_ULTRA24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_ULTRA24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readUltra24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_ULTRA24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readUltra24d2fConfig();
  const streak =
    globalThis.SinglestakeUltra24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeUltra24d2f?.ice2fPairLabel === "function"
        ? SinglestakeUltra24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_ULTRA24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_ULTRA24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initUltra24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeUltra24d2fBridgeHandler = handleBridgePayload;
  void readUltra24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readUltra24d2fAutopilotEnabled().then((on) => {
    if (on) void startUltra24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_ULTRA24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_ULTRA24D2F_AUTOPLAY].newValue === true) {
        void startUltra24d2fAutopilot(bridgeHandler);
      } else {
        stopUltra24d2fAutopilot();
      }
    }
    if (changes[STORAGE_ULTRA24D2F_CONFIG] && bridgeHandler) {
      void readUltra24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readUltra24d2fAutopilotEnabled().then((on) => {
          if (on) void startUltra24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeUltra24d2fSignalRunner = {
    initUltra24d2fSignalRunner,
    startUltra24d2fAutopilot,
    stopUltra24d2fAutopilot,
    setUltra24d2fAutopilotEnabled,
    getUltra24d2fAutopilotStatus,
    getUltra24d2fConfigForPopup,
    setUltra24d2fConfigFromPopup,
    resetUltra24d2fStats,
    STORAGE_ULTRA24D2F_AUTOPLAY,
    STORAGE_ULTRA24D2F_CONFIG,
  };
}
