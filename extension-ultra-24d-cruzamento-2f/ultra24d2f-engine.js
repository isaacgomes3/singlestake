...
var SinglestakeUltra24d2f = (() => {
  ...
  // extension-ultra-24d-cruzamento-2f/ultra24d2f-strategy-entry.ts
  var ultra24d2f_strategy_entry_exports = {};
  __export(ultra24d2f_strategy_entry_exports, {
    ...
    ULTRA24D2F_MAX_GALES: () => ULTRA24D2F_MAX_GALES,
    ULTRA24D2F_MESA_URL: () => ULTRA24D2F_MESA_URL,
    ULTRA24D2F_TABLE_ID: () => ULTRA24D2F_TABLE_ID,
    ...
    createUltra24d2fEngine: () => createUltra24d2fEngine,
    default: () => ultra24d2f_strategy_entry_default,
  ...
  // extension-ultra-24d-cruzamento-2f/ultra24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var ULTRA24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var ULTRA24D2F_MESA_URL = "https://ultra.bet.br/";
  var ULTRA24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var ULTRA24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var ULTRA24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var ULTRA24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of ULTRA24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createUltra24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && ULTRA24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && ULTRA24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && ULTRA24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && ULTRA24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && ULTRA24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && ULTRA24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && ULTRA24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = ULTRA24D2F_MESA_URL) {
      ...
      const signalId = `ultra24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: ULTRA24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? ULTRA24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createUltra24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeUltra24d2f = api;
  }
  var ultra24d2f_strategy_entry_default = api;
  return __toCommonJS(ultra24d2f_strategy_entry_exports);
})();
