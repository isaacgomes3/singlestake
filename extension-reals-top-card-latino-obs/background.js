/* global SinglestakeTopCardLatinoObs, SinglestakeFootballBlitzDga */

importScripts("top-card-latino-obs-engine.js", "dga-top-card-latino.js");

const STORAGE = {
  config: "tclObsRealsConfig",
  history: "tclObsRealsHistory",
  active: "tclObsRealsActive",
  status: "tclObsRealsStatus",
  suppressedIds: "tclObsRealsSuppressed",
  clearedAt: "tclObsRealsClearedAt",
  seedSuppress: "tclObsRealsSeedSuppress",
  sessionOutcomes: "tclObsRealsSessionOutcomes",
  deckOutcomeSynced: "tclObsRealsDeckOutcomeSynced",
};
...
function expandRound(round) {
  return SinglestakeTopCardLatinoObs.expandFootballBlitzRound(round);
}
...
function publicStatus() {
  const Api = SinglestakeTopCardLatinoObs;
  ...
  return {
    ...
    channel: `pragmatic.top-card-latino.${state.config.tableKey}`,
...
function clearHistoryOnShuffle(event = {}) {
  ...
  syncSessionOutcomes(
    SinglestakeTopCardLatinoObs.buildFootballBlitzCardPairTrack?.(state.history, {
      signalMinSamples: state.config.signalMinSamples ?? 1,
    }) ?? { outcomeHistory: [] },
  );
  state.history = [];
...
async function openOrFocusGameTab() {
  ...
  const match =
    tabs.find((tab) => {
      ...
      return /reals\.bet\.br/i.test(href) && /football.?blitz|top.?card|blitz|4022|super.?trunfo|liveCassino/i.test(href);
    }) ||
  ...
  state.config = {
    ...
    mesaUrl:
      ...
      (/reals\.bet\.br/i.test(stored[STORAGE.config].mesaUrl) ||
        /football.?blitz|top.?card|4022|super.?trunfo/i.test(stored[STORAGE.config].mesaUrl))
        ? stored[STORAGE.config].mesaUrl.trim()
        : DEFAULT_CONFIG.mesaUrl,
    tableKey: 4022,
  };
  // Só reutiliza histórico se já era desta mesa (4022).
  state.history =
    prevTableKey === 4022 && Array.isArray(stored[STORAGE.history])
...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name !== "tcl-obs-ui") return;
  uiPorts.add(port);
  try {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  const respond = async () => {
    if (message?.kind === "tcl-obs-get-status") return publicStatus();
    if (message?.kind === "tcl-obs-set-active") {
    ...
    if (message?.kind === "tcl-obs-refresh") {
      startDga();
      return publicStatus();
    }
    if (message?.kind === "tcl-obs-clear-history") {
      state.history = [];
      state.lastResultId = null;
      state.clearedAt = Date.now();
    ...
    if (message?.kind === "tcl-obs-open-panel") {
      await openOrFocusObsPanel();
      return { ok: true };
    }
    if (message?.kind === "tcl-obs-open-game") {
