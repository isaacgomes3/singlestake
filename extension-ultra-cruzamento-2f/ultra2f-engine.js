...
var SinglestakeUltra2f = (() => {
  ...
  // extension-ultra-cruzamento-2f/ultra2f-strategy-entry.ts
  var ultra2f_strategy_entry_exports = {};
  __export(ultra2f_strategy_entry_exports, {
    ...
    ULTRA2F_MAX_GALES: () => ULTRA2F_MAX_GALES,
    ULTRA2F_MESA_URL: () => ULTRA2F_MESA_URL,
    ULTRA2F_TABLE_ID: () => ULTRA2F_TABLE_ID,
    ...
    createUltra2fEngine: () => createUltra2fEngine,
    default: () => ultra2f_strategy_entry_default,
  ...
  // extension-ultra-cruzamento-2f/ultra2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var ULTRA2F_TABLE_ID = 227;
  var ULTRA2F_MESA_URL = "https://ultra.bet.br/jogo/3330041/pragmatic-play-live/roulette-1";
  var ULTRA2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var ULTRA2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var ULTRA2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var ULTRA2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of ULTRA2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createUltra2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && ULTRA2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && ULTRA2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && ULTRA2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && ULTRA2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && ULTRA2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && ULTRA2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && ULTRA2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = ULTRA2F_MESA_URL) {
      ...
      const signalId = `ultra2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `Ultra 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `Ultra 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `Ultra 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: ULTRA2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? ULTRA2F_MESA_URL,
          mesaProvider: "outro",
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeUltra2f = api;
  }
  var ultra2f_strategy_entry_default = api;
  return __toCommonJS(ultra2f_strategy_entry_exports);
})();
