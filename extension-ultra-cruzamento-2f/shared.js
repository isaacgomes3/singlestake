...
const GOG = {
  ...
  ULTRA2F_FACTOR_BRIDGE_STAGGER_MS: 5,
  ...
  ULTRA2F_DOUBLE_CLICK_STAGGER_MS: 20,
...
/** Unidades Ultra 2F: 1 · 2 · 4 · 8 · 16 · 32 · 64 · 128 · 256 (entrada + 8 gales). */
const ULTRA2F_STAKE_UNITS = [1, 2, 4, 8, 16, 32, 64, 128, 256];

function ultra2fStakeUnitsForRecovery(recovery) {
  const r = Math.max(0, Math.floor(recovery));
  return ULTRA2F_STAKE_UNITS[Math.min(r, ULTRA2F_STAKE_UNITS.length - 1)] ?? 1;
}

/** Gale 3 (8 unidades) — referência de ritmo de digitação ICE 3F. */
const ULTRA2F_GALE3_REFERENCE_UNITS = 8;

/** Intervalo entre cliques de ficha — 2F (gale 4/5 fixos). */
function ultra2fChipClickStaggerMs(recovery) {
  const r = Math.max(0, Math.floor(recovery));
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isUltra2fCrossingContext(context) && context?.singleFactorMode !== true) {
    return ultra2fChipClickStaggerMs(r);
  }
...
function panelSignalToBridge(data) {
  ...
  return {
    ...
    context: {
      ...
      signalId,
      stakeAmount: 0.5 * ultra2fStakeUnitsForRecovery(recovery),
