importScripts("shared.js", "ultra2f-engine.js", "dga-hub.js", "ultra2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogUltra2fAutopilotEnabled: false });
  }
  void ensureUltra2fPanelOnOpenTabs();
});
...
let ultra2fPanelWindowId = null;

async function openOrFocusUltra2fPanel() {
  if (ultra2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(ultra2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      ultra2fPanelWindowId = null;
    }
  }
  ...
  ultra2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusUltra2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === ultra2fPanelWindowId) ultra2fPanelWindowId = null;
});

const ULTRA2F_DEFAULT_TABLE_ID = 227;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const ULTRA2F_CALIB_SITE_KEY = "ultra.bet.br|pragmatic-roulette";

function isUltra2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "ultra.bet.br" || host.endsWith(".ultra.bet.br");
  } catch {
    return /ultra\.bet\.br/i.test(url);
  }
}

function isUltra2fRoulettePageUrl(url) {
  if (!isUltra2fHost(url)) return false;
  try {
    const u = new URL(url);
    const path = `${u.pathname}${u.hash}${u.search}`.toLowerCase();
    return /\/jogo\/3330041\/pragmatic-play-live\/roulette-1/i.test(path) || /roulette-1|pragmatic-play-live/i.test(path);
  } catch {
    return /ultra\.bet\.br/i.test(url);
  }
}

async function ensureUltra2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isUltra2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "ultra2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-ultra2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureUltra2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isUltra2fRoulettePageUrl(tab.url)) {
      await ensureUltra2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isUltra2fRoulettePageUrl(url)
  ) {
    registerMesaTab(ULTRA2F_DEFAULT_TABLE_ID, tabId);
    void ensureUltra2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ultra2f-autopilot") {
    void SinglestakeUltra2fSignalRunner.setUltra2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureUltra2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-ultra2f-autopilot") {
    void SinglestakeUltra2fSignalRunner.getUltra2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ultra2f-stats") {
    void SinglestakeUltra2fSignalRunner.resetUltra2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ultra2f-config") {
    void SinglestakeUltra2fSignalRunner.getUltra2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ultra2f-config") {
    void SinglestakeUltra2fSignalRunner.setUltra2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isUltra2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(ULTRA2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isUltra2fRoulettePageUrl(t.url)) ??
  ...
  const ultra2fAutopilot = await SinglestakeUltra2fSignalRunner.getUltra2fAutopilotStatus();
  const ultra2fConfig = await SinglestakeUltra2fSignalRunner.getUltra2fConfigForPopup();
  return {
    ok: true,
    mode,
    ultra2fAutopilot,
...
function isUltra2fCrossingContext(context) {
  return context?.strategy === "ultra2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isUltra2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ULTRA2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isUltra2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.ULTRA2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isUltra2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "ultra2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isUltra2fSignal =
    isUltra2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("ultra2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isUltra2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isUltra2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isUltra2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isUltra2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isUltra2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeUltra2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeUltra2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (ULTRA2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isUltra2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Roulette 1 no Ultra num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "ultra.bet.br" || host.endsWith(".ultra.bet.br")) {
      return ULTRA2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[ULTRA2F_CALIB_SITE_KEY]) {
    return { siteKey: ULTRA2F_CALIB_SITE_KEY, site: sites[ULTRA2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: ULTRA2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isUltra2fHost(url)) {
    for (const [key, site] of Object.entries(sites)) {
      if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
    }
    if (sites[ULTRA2F_CALIB_SITE_KEY]) return { siteKey: ULTRA2F_CALIB_SITE_KEY, site: sites[ULTRA2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isUltra2fHost(tab.url) ? ULTRA2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === ULTRA2F_CALIB_SITE_KEY && isUltra2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isUltra2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isUltra2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isUltra2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeUltra2f !== "undefined" && SinglestakeUltra2f.ULTRA2F_MESA_URL) ||
          "https://ultra.bet.br/jogo/3330041/pragmatic-play-live/roulette-1",
        mesaProvider: null,
        currentTableId: ULTRA2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Ultra encontrada. Abra Roulette 1 no Ultra num separador normal do Chrome (não neste painel).",
    };
  }
...
SinglestakeUltra2fSignalRunner.initUltra2fSignalRunner(handleBridgePayload);
