/** Autopilot Ultra — Cruzamento 2F (227) · cruzamento sequencial 2 fatores. */
const STORAGE_ULTRA2F_AUTOPLAY = "gogUltra2fAutopilotEnabled";
const STORAGE_ULTRA2F_CONFIG = "gogUltra2fConfig";
const STORAGE_ULTRA2F_STATUS = "gogUltra2fAutopilotStatus";
const STORAGE_ULTRA2F_STATS = "gogUltra2fAutopilotSessionStats";
const STORAGE_ULTRA2F_MACHINE = "gogUltra2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeUltra2f['createUltra2fEngine']>|null} */
let engine = null;
...
const ULTRA2F_DEFAULTS = {
  tableId: SinglestakeUltra2f?.ULTRA2F_TABLE_ID ?? 227,
  mesaUrl:
    SinglestakeUltra2f?.ULTRA2F_MESA_URL ??
    "https://ultra.bet.br/jogo/3330041/pragmatic-play-live/roulette-1",
...
function knownPairIds() {
  const known = SinglestakeUltra2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeUltra2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeUltra2f !== "undefined"
      ? SinglestakeUltra2f.applyIce2fEnabledPairIds
        ? SinglestakeUltra2f
        : SinglestakeUltra2f.default
      : null;
...
async function readUltra2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_AUTOPLAY]);
  return data[STORAGE_ULTRA2F_AUTOPLAY] === true;
}

async function readUltra2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_CONFIG]);
  const stored = data[STORAGE_ULTRA2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...ULTRA2F_DEFAULTS,
      maxRecoveryPreference: ULTRA2F_DEFAULTS.maxRecovery,
...
async function readUltra2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_MACHINE]);
  const raw = data[STORAGE_ULTRA2F_MACHINE];
...
async function persistUltra2fMachineState(machine) {
...
async function clearUltra2fMachineState() {
  await chrome.storage.local.remove([STORAGE_ULTRA2F_MACHINE]);
}

async function readUltra2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_STATS]);
  const raw = data[STORAGE_ULTRA2F_STATS];
...
async function persistUltra2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_ULTRA2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeUltra2fStatus({
    wins: stats.wins ?? 0,
...
async function resetUltra2fStats() {
  const cfg = await readUltra2fConfig();
  const empty =
    globalThis.SinglestakeUltra2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistUltra2fStats(empty, cfg.maxRecovery);
  await clearUltra2fMachineState();
...
async function getUltra2fConfigForPopup() {
  return readUltra2fConfig();
}

async function setUltra2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_CONFIG]);
  const stored =
    data[STORAGE_ULTRA2F_CONFIG] && typeof data[STORAGE_ULTRA2F_CONFIG] === "object"
      ? data[STORAGE_ULTRA2F_CONFIG]
      : {};
  const prev = await readUltra2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_ULTRA2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearUltra2fMachineState();
    await writeUltra2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readUltra2fConfig()) };
}

async function writeUltra2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_STATUS]);
  const prev = data[STORAGE_ULTRA2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_ULTRA2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistUltra2fMachineState(live.machine);
    await writeUltra2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeUltra2f?.ice2fBetDelayMs === "function"
      ? SinglestakeUltra2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeUltra2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeUltra2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeUltra2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `ultra2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistUltra2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistUltra2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeUltra2f?.ice2fWatchLabelForMachine ??
      SinglestakeUltra2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeUltra2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeUltra2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startUltra2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeUltra2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readUltra2fAutopilotEnabled();
  if (!enabled) {
    await writeUltra2fStatus({ running: false, reason: "autopilot Ultra 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeUltra2f?.createUltra2fEngine) {
    await writeUltra2fStatus({
      running: false,
      reason: "ultra2f-engine.js em falta — corra npm run extension:ultra2f:build",
    });
    return;
  }

  stopUltra2fAutopilot();

  const cfg = await readUltra2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readUltra2fStats(cfg.maxRecovery);
  const savedMachine = await readUltra2fMachineState();
  engine = SinglestakeUltra2f.createUltra2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeUltra2fStatus({ log: msg }),
    onStatus: (status) => void writeUltra2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readUltra2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readUltra2fConfig().then((cfgNow) =>
...
function stopUltra2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeUltra2fStatus({ running: false, active: false, waitingBet: false });
}

async function setUltra2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_ULTRA2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startUltra2fAutopilot(bridgeHandler ?? globalThis.__singlestakeUltra2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeUltra2fBridgeHandler) {
      await writeUltra2fStatus({
        running: false,
  ...
  return getUltra2fAutopilotStatus();
}

async function getUltra2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_ULTRA2F_STATUS, STORAGE_ULTRA2F_AUTOPLAY, STORAGE_ULTRA2F_STATS]);
  const statsPack = data[STORAGE_ULTRA2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_ULTRA2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_ULTRA2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_ULTRA2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_ULTRA2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readUltra2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_ULTRA2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readUltra2fConfig();
  const streak =
    globalThis.SinglestakeUltra2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeUltra2f?.ice2fPairLabel === "function"
        ? SinglestakeUltra2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_ULTRA2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_ULTRA2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initUltra2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeUltra2fBridgeHandler = handleBridgePayload;
  void readUltra2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readUltra2fAutopilotEnabled().then((on) => {
    if (on) void startUltra2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_ULTRA2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_ULTRA2F_AUTOPLAY].newValue === true) {
        void startUltra2fAutopilot(bridgeHandler);
      } else {
        stopUltra2fAutopilot();
      }
    }
    if (changes[STORAGE_ULTRA2F_CONFIG] && bridgeHandler) {
      void readUltra2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readUltra2fAutopilotEnabled().then((on) => {
          if (on) void startUltra2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeUltra2fSignalRunner = {
    initUltra2fSignalRunner,
    startUltra2fAutopilot,
    stopUltra2fAutopilot,
    setUltra2fAutopilotEnabled,
    getUltra2fAutopilotStatus,
    getUltra2fConfigForPopup,
    setUltra2fConfigFromPopup,
    resetUltra2fStats,
    STORAGE_ULTRA2F_AUTOPLAY,
    STORAGE_ULTRA2F_CONFIG,
  };
}
