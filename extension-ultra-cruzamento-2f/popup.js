...
const ultra2fConfigStatus = document.getElementById("ultra2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, ultra2fConfig) {
  ...
  const noGale =
    ultra2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = ultra2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? ultra2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.ultra2fAutopilot, mode, status?.ultra2fConfig);

  const cfg = status?.ultra2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "ultra.bet.br/liveroulettea-pragmaticexternal"}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa Ultra 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Roulette 1 no Ultra num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa Ultra 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-ultra2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-ultra2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-ultra2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.ultra2fConfig ?? {};
  await send("set-ultra2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-ultra2f-config", { config });
  if (ultra2fConfigStatus) {
    ultra2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
