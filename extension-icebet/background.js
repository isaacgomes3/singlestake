importScripts("operator-config.js", "shared.js", "um-fator-engine.js", "dga-hub.js", "signal-runner.js");
  ...
  if (targetTabId == null) {
    ...
    return {
      ...
      detail: label
        ...
        : "Mesa não encontrada — abra ice.bet.br/games/… num separador (mesma janela do Chrome)",
    };
  }
...
function pickCasinoTabForNavigation(tabs, mesaUrl) {
  ...
  if (isIceBetUrl(mesaUrl)) {
    const sameOrigin = tabs.filter((t) => {
      if (!t.url) return false;
      try {
        const u = new URL(t.url);
        return u.origin === mesa.origin && u.pathname.toLowerCase().includes("/games/");
      } catch {
        return false;
      }
    });
  ...
  if (tabId != null && url) {
    try {
      ...
      if (tab.url && !casinoPathsMatch(tab.url, url)) {
        ...
        await settleMesaTabAfterNavigation(tabId, url);
      ...
      await chrome.tabs.update(tabId, { active: true });
      if (url && isIceCasinoGameUrl(url)) {
        await waitForIceLobbyGate(tabId, url);
      } else {
        await sleep(1100);
      }
      return tabId;
...
function isCasinoPlayUrl(url) {
  if (!url) return false;
  if (/\/play\/(playtech|pragmatic)\//i.test(url)) return true;
  return isIceCasinoGameUrl(url);
}

async function injectIceLobbyHelpers(tabId) {
  await chrome.scripting.executeScript({
    target: { tabId, allFrames: false },
    files: ["operator-config.js", "operator-lobby.js"],
  });
}

async function waitForIceLobbyGate(tabId, pageUrl) {
  if (!isIceCasinoGameUrl(pageUrl)) {
    return { ok: true, skipped: true, detail: "não é página ice.bet" };
  }
  const deadline = Date.now() + ICE_OPERATOR.lobbyGateTimeoutMs;
  let lastDetail = "à espera do lobby";
  while (Date.now() < deadline) {
    try {
      await injectIceLobbyHelpers(tabId);
      const frameResults = await chrome.scripting.executeScript({
        target: { tabId, allFrames: false },
        func: () => {
          const gate = globalThis.__iceTryLobbyGate?.();
          const readyNow = globalThis.__iceIsGameReady?.() === true;
          return {
            ...gate,
            ready: readyNow || gate?.ready === true,
          };
        },
      });
      const result = frameResults[0]?.result;
      if (result?.ready) {
        await sleep(ICE_OPERATOR.postNavigateSettleMs);
        return { ok: true, ready: true, detail: result.detail ?? "jogo pronto" };
      }
      lastDetail = result?.detail ?? lastDetail;
    } catch (err) {
      lastDetail = err instanceof Error ? err.message : String(err);
    }
    await sleep(ICE_OPERATOR.lobbyPollMs);
  }
  return { ok: false, ready: false, detail: `timeout lobby ice.bet: ${lastDetail}` };
}

async function settleMesaTabAfterNavigation(tabId, url) {
  if (isIceCasinoGameUrl(url)) {
    await sleep(ICE_OPERATOR.initialDomWaitMs);
    return waitForIceLobbyGate(tabId, url);
  }
  await sleep(6500);
  return { ok: true };
}
...
/** Prefer iframe Pragmatic/Playtech sobre a shell do operador (ice.bet / br4). */
function rankFrameResult(result) {
  ...
  else if (href.includes("ice.bet.br")) bonus -= 50;
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail: "Abra a mesa Pragmatic no ice.bet num separador (games/tag/roulette/…).",
    };
  }
