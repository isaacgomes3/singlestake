...
var SinglestakeBrazino2f = (() => {
  ...
  // extension-brazino777-cruzamento-2f/brazino2f-strategy-entry.ts
  var brazino2f_strategy_entry_exports = {};
  __export(brazino2f_strategy_entry_exports, {
    BRAZINO2F_MAX_GALES: () => BRAZINO2F_MAX_GALES,
    BRAZINO2F_MESA_URL: () => BRAZINO2F_MESA_URL,
    BRAZINO2F_TABLE_ID: () => BRAZINO2F_TABLE_ID,
    ...
    createBrazino2fEngine: () => createBrazino2fEngine,
    default: () => brazino2f_strategy_entry_default,
  ...
  // extension-brazino777-cruzamento-2f/brazino2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var BRAZINO2F_TABLE_ID = 237;
  var BRAZINO2F_MESA_URL = "https://www.brazino777.bet.br/game/brazilian_roulette_pr";
  var BRAZINO2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var BRAZINO2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var BRAZINO2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var BRAZINO2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of BRAZINO2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createBrazino2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && BRAZINO2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && BRAZINO2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && BRAZINO2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && BRAZINO2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && BRAZINO2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && BRAZINO2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && BRAZINO2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = BRAZINO2F_MESA_URL) {
      ...
      const signalId = `brazino2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `Brazino777 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `Brazino777 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `Brazino777 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: BRAZINO2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? BRAZINO2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createBrazino2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeBrazino2f = api;
  }
  var brazino2f_strategy_entry_default = api;
  return __toCommonJS(brazino2f_strategy_entry_exports);
})();
