...
const brazino2fConfigStatus = document.getElementById("brazino2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, brazino2fConfig) {
  ...
  const noGale =
    brazino2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = brazino2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? brazino2fConfig?.enabledPairIds,
  );
  ...
  if (!on) {
    ice2fStatus.textContent = "Parada — clique Ligar para seguir a mesa 237 via DGA.";
    return;
  }
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.brazino2fAutopilot, mode, status?.brazino2fConfig);

  const cfg = status?.brazino2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "brazino777.bet.br/liveroulettea-pragmaticexternal"}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa Brazino777 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Brazilian Roulette no Brazino777 num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa Brazino777 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-brazino2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-brazino2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-brazino2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.brazino2fConfig ?? {};
  await send("set-brazino2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-brazino2f-config", { config });
  if (brazino2fConfigStatus) {
    brazino2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
