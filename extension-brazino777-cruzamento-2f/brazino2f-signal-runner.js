/** Autopilot Brazino777 — Cruzamento 2F (237) · cruzamento sequencial 2 fatores. */
const STORAGE_BRAZINO2F_AUTOPLAY = "gogBrazino2fAutopilotEnabled";
const STORAGE_BRAZINO2F_CONFIG = "gogBrazino2fConfig";
const STORAGE_BRAZINO2F_STATUS = "gogBrazino2fAutopilotStatus";
const STORAGE_BRAZINO2F_STATS = "gogBrazino2fAutopilotSessionStats";
const STORAGE_BRAZINO2F_MACHINE = "gogBrazino2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeBrazino2f['createBrazino2fEngine']>|null} */
let engine = null;
...
const BRAZINO2F_DEFAULTS = {
  tableId: SinglestakeBrazino2f?.BRAZINO2F_TABLE_ID ?? 237,
  mesaUrl:
    SinglestakeBrazino2f?.BRAZINO2F_MESA_URL ??
    "https://www.brazino777.bet.br/game/brazilian_roulette_pr",
...
function knownPairIds() {
  const known = SinglestakeBrazino2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeBrazino2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeBrazino2f !== "undefined"
      ? SinglestakeBrazino2f.applyIce2fEnabledPairIds
        ? SinglestakeBrazino2f
        : SinglestakeBrazino2f.default
      : null;
...
async function readBrazino2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_AUTOPLAY]);
  return data[STORAGE_BRAZINO2F_AUTOPLAY] === true;
}

async function readBrazino2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_CONFIG]);
  const stored = data[STORAGE_BRAZINO2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...BRAZINO2F_DEFAULTS,
      maxRecoveryPreference: BRAZINO2F_DEFAULTS.maxRecovery,
      enabledPairIds: defaultEnabledPairIds(),
    };
  }
  const legacyWrong =
    (typeof stored.tableId === "number" && stored.tableId !== 237) ||
    (typeof stored.mesaUrl === "string" &&
      stored.mesaUrl.trim() !== "" &&
      !stored.mesaUrl.toLowerCase().includes("brazino777.bet.br"));
  const noGale = stored.noGale === true;
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? BRAZINO2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? BRAZINO2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : BRAZINO2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? BRAZINO2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : BRAZINO2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : BRAZINO2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : BRAZINO2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : BRAZINO2F_DEFAULTS.currency,
    noGale,
...
async function readBrazino2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_MACHINE]);
  const raw = data[STORAGE_BRAZINO2F_MACHINE];
...
async function persistBrazino2fMachineState(machine) {
...
async function clearBrazino2fMachineState() {
  await chrome.storage.local.remove([STORAGE_BRAZINO2F_MACHINE]);
}

async function readBrazino2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_STATS]);
  const raw = data[STORAGE_BRAZINO2F_STATS];
...
async function persistBrazino2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_BRAZINO2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeBrazino2fStatus({
    wins: stats.wins ?? 0,
...
async function resetBrazino2fStats() {
  const cfg = await readBrazino2fConfig();
  const empty =
    globalThis.SinglestakeBrazino2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistBrazino2fStats(empty, cfg.maxRecovery);
  await clearBrazino2fMachineState();
...
async function getBrazino2fConfigForPopup() {
  return readBrazino2fConfig();
}

async function setBrazino2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_CONFIG]);
  const stored =
    data[STORAGE_BRAZINO2F_CONFIG] && typeof data[STORAGE_BRAZINO2F_CONFIG] === "object"
      ? data[STORAGE_BRAZINO2F_CONFIG]
      : {};
  const prev = await readBrazino2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_BRAZINO2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearBrazino2fMachineState();
    await writeBrazino2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readBrazino2fConfig()) };
}

async function writeBrazino2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_STATUS]);
  const prev = data[STORAGE_BRAZINO2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_BRAZINO2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistBrazino2fMachineState(live.machine);
    await writeBrazino2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeBrazino2f?.ice2fBetDelayMs === "function"
      ? SinglestakeBrazino2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeBrazino2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeBrazino2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeBrazino2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `brazino2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistBrazino2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistBrazino2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeBrazino2f?.ice2fWatchLabelForMachine ??
      SinglestakeBrazino2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeBrazino2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeBrazino2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startBrazino2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeBrazino2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readBrazino2fAutopilotEnabled();
  if (!enabled) {
    await writeBrazino2fStatus({ running: false, reason: "autopilot Brazino777 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeBrazino2f?.createBrazino2fEngine) {
    await writeBrazino2fStatus({
      running: false,
      reason: "brazino2f-engine.js em falta — corra npm run extension:brazino2f:build",
    });
    return;
  }

  stopBrazino2fAutopilot();

  const cfg = await readBrazino2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readBrazino2fStats(cfg.maxRecovery);
  const savedMachine = await readBrazino2fMachineState();
  engine = SinglestakeBrazino2f.createBrazino2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeBrazino2fStatus({ log: msg }),
    onStatus: (status) => void writeBrazino2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readBrazino2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readBrazino2fConfig().then((cfgNow) =>
...
function stopBrazino2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeBrazino2fStatus({ running: false, active: false, waitingBet: false });
}

async function setBrazino2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_BRAZINO2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startBrazino2fAutopilot(bridgeHandler ?? globalThis.__singlestakeBrazino2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeBrazino2fBridgeHandler) {
      await writeBrazino2fStatus({
        running: false,
  ...
  return getBrazino2fAutopilotStatus();
}

async function getBrazino2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO2F_STATUS, STORAGE_BRAZINO2F_AUTOPLAY, STORAGE_BRAZINO2F_STATS]);
  const statsPack = data[STORAGE_BRAZINO2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_BRAZINO2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_BRAZINO2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_BRAZINO2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_BRAZINO2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readBrazino2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_BRAZINO2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readBrazino2fConfig();
  const streak =
    globalThis.SinglestakeBrazino2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeBrazino2f?.ice2fPairLabel === "function"
        ? SinglestakeBrazino2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_BRAZINO2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_BRAZINO2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initBrazino2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeBrazino2fBridgeHandler = handleBridgePayload;
  void readBrazino2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readBrazino2fAutopilotEnabled().then((on) => {
    if (on) void startBrazino2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_BRAZINO2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_BRAZINO2F_AUTOPLAY].newValue === true) {
        void startBrazino2fAutopilot(bridgeHandler);
      } else {
        stopBrazino2fAutopilot();
      }
    }
    if (changes[STORAGE_BRAZINO2F_CONFIG] && bridgeHandler) {
      void readBrazino2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readBrazino2fAutopilotEnabled().then((on) => {
          if (on) void startBrazino2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeBrazino2fSignalRunner = {
    initBrazino2fSignalRunner,
    startBrazino2fAutopilot,
    stopBrazino2fAutopilot,
    setBrazino2fAutopilotEnabled,
    getBrazino2fAutopilotStatus,
    getBrazino2fConfigForPopup,
    setBrazino2fConfigFromPopup,
    resetBrazino2fStats,
    STORAGE_BRAZINO2F_AUTOPLAY,
    STORAGE_BRAZINO2F_CONFIG,
  };
}
