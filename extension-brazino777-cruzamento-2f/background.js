importScripts("shared.js", "brazino2f-engine.js", "dga-hub.js", "brazino2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogBrazino2fAutopilotEnabled: false });
  }
  void ensureBrazino2fPanelOnOpenTabs();
});
...
let brazino2fPanelWindowId = null;

async function openOrFocusBrazino2fPanel() {
  if (brazino2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(brazino2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      brazino2fPanelWindowId = null;
    }
  }
  ...
  brazino2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusBrazino2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === brazino2fPanelWindowId) brazino2fPanelWindowId = null;
});

const BRAZINO2F_DEFAULT_TABLE_ID = 237;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const BRAZINO2F_CALIB_SITE_KEY = "brazino777.bet.br|pragmatic-roulette";

function isBrazino2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "brazino777.bet.br" || host.endsWith(".brazino777.bet.br");
  } catch {
    return /brazino777\.bet\.br/i.test(url);
  }
}

function isBrazino2fRoulettePageUrl(url) {
  if (!isBrazino2fHost(url)) return false;
  try {
    const u = new URL(url);
    const path = `${u.pathname}${u.hash}${u.search}`.toLowerCase();
    return /\/game\/brazilian_roulette_pr/i.test(path) || /brazilian_roulette/i.test(path);
  } catch {
    return /brazino777\.bet\.br/i.test(url);
  }
}

async function ensureBrazino2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isBrazino2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "brazino2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-brazino2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureBrazino2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isBrazino2fRoulettePageUrl(tab.url)) {
      await ensureBrazino2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isBrazino2fRoulettePageUrl(url)
  ) {
    registerMesaTab(BRAZINO2F_DEFAULT_TABLE_ID, tabId);
    void ensureBrazino2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-brazino2f-autopilot") {
    void SinglestakeBrazino2fSignalRunner.setBrazino2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureBrazino2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-brazino2f-autopilot") {
    void SinglestakeBrazino2fSignalRunner.getBrazino2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-brazino2f-stats") {
    void SinglestakeBrazino2fSignalRunner.resetBrazino2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-brazino2f-config") {
    void SinglestakeBrazino2fSignalRunner.getBrazino2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-brazino2f-config") {
    void SinglestakeBrazino2fSignalRunner.setBrazino2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isBrazino2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(BRAZINO2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isBrazino2fRoulettePageUrl(t.url)) ??
  ...
  const brazino2fAutopilot = await SinglestakeBrazino2fSignalRunner.getBrazino2fAutopilotStatus();
  const brazino2fConfig = await SinglestakeBrazino2fSignalRunner.getBrazino2fConfigForPopup();
  return {
    ok: true,
    mode,
    brazino2fAutopilot,
...
function isBrazino2fCrossingContext(context) {
  return context?.strategy === "brazino2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isBrazino2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.BRAZINO2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isBrazino2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.BRAZINO2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isBrazino2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "brazino2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isBrazino2fSignal =
    isBrazino2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("brazino2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isBrazino2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isBrazino2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isBrazino2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isBrazino2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isBrazino2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeBrazino2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeBrazino2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (BRAZINO2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isBrazino2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Brazilian Roulette no Brazino777 num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "brazino777.bet.br" || host.endsWith(".brazino777.bet.br")) {
      return BRAZINO2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[BRAZINO2F_CALIB_SITE_KEY]) {
    return { siteKey: BRAZINO2F_CALIB_SITE_KEY, site: sites[BRAZINO2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: BRAZINO2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isBrazino2fHost(url)) {
    for (const [key, site] of Object.entries(sites)) {
      if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
    }
    if (sites[BRAZINO2F_CALIB_SITE_KEY]) return { siteKey: BRAZINO2F_CALIB_SITE_KEY, site: sites[BRAZINO2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isBrazino2fHost(tab.url) ? BRAZINO2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === BRAZINO2F_CALIB_SITE_KEY && isBrazino2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isBrazino2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isBrazino2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isBrazino2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeBrazino2f !== "undefined" && SinglestakeBrazino2f.BRAZINO2F_MESA_URL) ||
          "https://www.brazino777.bet.br/game/brazilian_roulette_pr",
        mesaProvider: null,
        currentTableId: BRAZINO2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Brazino777 encontrada. Abra Brazilian Roulette no Brazino777 num separador normal do Chrome (não neste painel).",
    };
  }
...
SinglestakeBrazino2fSignalRunner.initBrazino2fSignalRunner(handleBridgePayload);
