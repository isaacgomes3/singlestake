/**
 * Bundle entry — motor Brazino777 Cruzamento 2F para extensão Chrome.
 */
...
export const BRAZINO2F_TABLE_ID = 237;
export const BRAZINO2F_MESA_URL =
  "https://www.brazino777.bet.br/game/brazilian_roulette_pr";
export const BRAZINO2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const BRAZINO2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const BRAZINO2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const BRAZINO2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));

export type Brazino2fPersistedMachine = {
...
function parsePersistedWatch(
  raw: Brazino2fPersistedMachine["watch"],
...
export type CreateBrazino2fEngineOptions = {
  maxRecovery?: number;
  initialStats?: RotatingRoomSessionStats | null;
  initialMachine?: Brazino2fPersistedMachine | null;
};
...
function pendingRecoveryFromSaved(saved: Brazino2fPersistedMachine): number {
  const stored =
...
export type Brazino2fEngineSpinResult = {
...
export function createBrazino2fEngine(options: CreateBrazino2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && BRAZINO2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        BRAZINO2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && BRAZINO2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
      ...(restoredWatch ? { watch: restoredWatch } : {}),
    };
  }
  let pendingRestore: Brazino2fPersistedMachine | null = options.initialMachine ?? null;
  let stats =
  ...
  function runTick(): Brazino2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && BRAZINO2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && BRAZINO2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        BRAZINO2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        BRAZINO2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = BRAZINO2F_MESA_URL) {
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `Brazino777 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `Brazino777 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `Brazino777 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "brazino2fcruzamento" as const,
...
// named exports; sem isto SinglestakeBrazino2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeBrazino2f?: typeof api }).SinglestakeBrazino2f = api;
}
