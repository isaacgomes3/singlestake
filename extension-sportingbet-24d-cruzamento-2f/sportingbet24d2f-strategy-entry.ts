/**
 * Bundle entry — motor Sportingbet 24D Spin Cruzamento 2F (mesa 3426).
 */
...
export const SPORTINGBET24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
export const SPORTINGBET24D2F_MESA_URL = "https://br4.bet.br/play/pragmatic/24d-spin";
export const SPORTINGBET24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const SPORTINGBET24D2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const SPORTINGBET24D2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const SPORTINGBET24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
...
export function createSportingbet24d2fEngine(options: CreateIce2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && SPORTINGBET24D2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        SPORTINGBET24D2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && SPORTINGBET24D2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
  ...
  function runTick(): Ice2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && SPORTINGBET24D2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && SPORTINGBET24D2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        SPORTINGBET24D2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        SPORTINGBET24D2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = SPORTINGBET24D2F_MESA_URL) {
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `24D 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `24D 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `24D 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "sportingbet24d2fcruzamento" as const,
...
// named exports; sem isto SinglestakeSportingbet24d2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeSportingbet24d2f?: typeof api }).SinglestakeSportingbet24d2f = api;
}
