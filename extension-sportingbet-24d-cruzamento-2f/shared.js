...
const GOG = {
  ...
  SPORTINGBET24D2F_FACTOR_BRIDGE_STAGGER_MS: 5,
  /** Intervalo entre fichas — gale 4 (recovery 4). */
  SPORTINGBET24D2F_GALE4_CHIP_STAGGER_MS: 15,
  /** Intervalo entre fichas — gale 5 (recovery 5). */
  SPORTINGBET24D2F_GALE5_CHIP_STAGGER_MS: 8,
  /** Intervalo entre cliques em Dobrar (gale). */
  SPORTINGBET24D2F_DOUBLE_CLICK_STAGGER_MS: 20,
  ...
  MESA_FIRST_CLICK_SETTLE_MS: 9500,
};
...
/** Unidades 24D 2F: 1 · 2 · 4 · 8 · 16 · 32 · 64 · 128 · 256 (entrada + 8 gales). */
const ICE2F_STAKE_UNITS = [1, 2, 4, 8, 16, 32, 64, 128, 256];
...
function ice2fChipClickStaggerMs(recovery) {
  const r = Math.max(0, Math.floor(recovery));
  if (r >= 5) return GOG.SPORTINGBET24D2F_GALE5_CHIP_STAGGER_MS ?? 8;
  if (r >= 4) return GOG.SPORTINGBET24D2F_GALE4_CHIP_STAGGER_MS ?? 15;
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isSportingbet24d2fCrossingContext(context) && context?.singleFactorMode !== true) {
