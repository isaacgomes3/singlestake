"use strict";
var SinglestakeSportingbet24d2f = (() => {
  ...
  // extension-sportingbet-24d-cruzamento-2f/sportingbet24d2f-strategy-entry.ts
  var sportingbet24d2f_strategy_entry_exports = {};
  __export(sportingbet24d2f_strategy_entry_exports, {
    ...
    SPORTINGBET24D2F_MAX_GALES: () => SPORTINGBET24D2F_MAX_GALES,
    SPORTINGBET24D2F_MESA_URL: () => SPORTINGBET24D2F_MESA_URL,
    SPORTINGBET24D2F_TABLE_ID: () => SPORTINGBET24D2F_TABLE_ID,
    ...
    createSportingbet24d2fEngine: () => createSportingbet24d2fEngine,
    default: () => sportingbet24d2f_strategy_entry_default,
  ...
  // src/lib/roulette/spin24dFactors.ts
  var RED_24D = /* @__PURE__ */ new Set([
    1,
    2,
    3,
    4,
  ...
  function isSpin24dNumber(n) {
    return Number.isInteger(n) && n >= 1 && n <= 24;
  }
  function spin24dColorOf(n) {
    if (!isSpin24dNumber(n)) return null;
    return RED_24D.has(n) ? "Vermelho" : "Preto";
  }
  function spin24dHeightOf(n) {
    if (!isSpin24dNumber(n)) return null;
    return n <= 12 ? "Baixo" : "Alto";
  }
  function spin24dParityOf(n) {
    if (!isSpin24dNumber(n)) return null;
    return n % 2 === 0 ? "Par" : "Impar";
  }
  function factorsFor24dNumberOnAxis(n, axis) {
  ...
  function spin24dFactorWins(num, factor) {
    switch (factor.kind) {
      case "cor":
  ...
  function spin24dTripleFactors(n) {
    const col = spin24dColorOf(n);
    const alt = spin24dHeightOf(n);
    const par = spin24dParityOf(n);
    if (!col || !alt || !par) return null;
  ...
  function spin24dSharedFactorsBetween(a, b) {
    const triple = spin24dTripleFactors(a);
    if (!triple) return [];
    return triple.filter((f) => spin24dFactorWins(b, f));
  }
  function spin24dTriggerMatchCount(a, b) {
    return spin24dSharedFactorsBetween(a, b).length;
  }
  function spin24dEvaluateRound(num, factor1, factor2) {
  ...
  // src/lib/roulette/spin24dCruzamento2fStrategy.ts
  var ICE_2F_ROULETTE_TABLE_ID = 3426;
  ...
  var ICE_2F_RECOVERY_BET_DELAY_MS = 9500;
  ...
  function ice2fFindHitForPair(historyNewestFirst, posA, posB, pairMeta) {
    ...
    if (numberA < 1 || numberA > 24 || numberB < 1 || numberB > 24) return null;
    const sharedCount = spin24dTriggerMatchCount(numberA, numberB);
    ...
    if (sharedCount >= 3) {
      axis = "cor-paridade";
      const factors = factorsFor24dNumberOnAxis(numberA, axis);
      if (!factors) return null;
      factor1 = factors[0];
      factor2 = factors[1];
    } else {
      const shared = spin24dSharedFactorsBetween(numberA, numberB);
  ...
  function ice2fClassifyBetRound(result, active) {
    if (result === 0 || result < 1 || result > 24) return "L";
    return spin24dEvaluateRound(result, active.factor1, active.factor2);
  }
  function ice2fBuildActiveFromHit(hit) {
    const labels = [hit.factor1, hit.factor2].map((f) => spin24dFactorLabel(f)).join(" \xB7 ");
  ...
  // extension-sportingbet-24d-cruzamento-2f/sportingbet24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var SPORTINGBET24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var SPORTINGBET24D2F_MESA_URL = "https://br4.bet.br/play/pragmatic/24d-spin";
  var SPORTINGBET24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var SPORTINGBET24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var SPORTINGBET24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var SPORTINGBET24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of SPORTINGBET24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createSportingbet24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && SPORTINGBET24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && SPORTINGBET24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && SPORTINGBET24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && SPORTINGBET24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && SPORTINGBET24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && SPORTINGBET24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && SPORTINGBET24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = SPORTINGBET24D2F_MESA_URL) {
      ...
      const signalId = `sportingbet24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      const f1Key = pragmaticExteriorBetKeyFromFactor(active.factor1);
      const f2Key = pragmaticExteriorBetKeyFromFactor(active.factor2);
      const f1Label = spin24dFactorLabel(active.factor1);
      const f2Label = spin24dFactorLabel(active.factor2);
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `24D 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `24D 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `24D 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: SPORTINGBET24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? SPORTINGBET24D2F_MESA_URL,
          mesaProvider: "outro",
    ...
    return {
      tableId: SPORTINGBET24D2F_TABLE_ID,
  ...
  var api = {
    SPORTINGBET24D2F_TABLE_ID,
    SPORTINGBET24D2F_MESA_URL,
    SPORTINGBET24D2F_MAX_GALES,
    ICE_2F_BET_DELAY_MS,
    ...
    createSportingbet24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeSportingbet24d2f = api;
  }
  var sportingbet24d2f_strategy_entry_default = api;
  return __toCommonJS(sportingbet24d2f_strategy_entry_exports);
})();
