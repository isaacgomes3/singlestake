importScripts("shared.js", "sportingbet24d2f-engine.js", "dga-hub.js", "sportingbet24d2f-signal-runner.js");
...
async function openOrFocusSportingbet24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (sportingbet24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(sportingbet24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      sportingbet24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    sportingbet24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: sportingbet24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogSportingbet24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureSportingbet24d2fPanelOnOpenTabs();
  void openOrFocusSportingbet24d2fPanel();
});
...
/** Janela fixa do painel Sportingbet 24D 2F (não fecha ao clicar na mesa). */
let sportingbet24d2fPanelWindowId = null;

chrome.action.onClicked.addListener(() => {
  void openOrFocusSportingbet24d2fPanel();
});

// Garante que o clique no ícone abre a janela (não o popup embutido).
void chrome.action.setPopup({ popup: "" });

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === sportingbet24d2fPanelWindowId) sportingbet24d2fPanelWindowId = null;
});

const SPORTINGBET24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const SPORTINGBET24D2F_CALIB_SITE_KEY = "sportingbet.bet.br|pragmatic-24d-spin";
...
function isSportingbet24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "sportingbet.bet.br" || host.endsWith(".sportingbet.bet.br") || host.endsWith(".br4.bet.br"))) return false;
    return /24d[-_]?spin|24dspin|3426/i.test(path) || /\/play\/pragmatic\/24d/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureSportingbet24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isSportingbet24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "sportingbet24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-sportingbet24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureSportingbet24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isSportingbet24d2fGameUrl(tab.url)) {
      await ensureSportingbet24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isSportingbet24d2fGameUrl(url)
  ) {
    registerMesaTab(SPORTINGBET24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureSportingbet24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeSportingbet24d2fSignalRunner.setSportingbet24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureSportingbet24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusSportingbet24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusSportingbet24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeSportingbet24d2fSignalRunner.getSportingbet24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeSportingbet24d2fSignalRunner.resetSportingbet24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeSportingbet24d2fSignalRunner.getSportingbet24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeSportingbet24d2fSignalRunner.setSportingbet24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isSportingbet24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(SPORTINGBET24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
const DEFAULT_BRIDGE_PREFS = { maxRecovery: 8, wins: 0, losses: 0, closeMesaOnFinish: false };
...
async function readBridgePrefs() {
  ...
  return {
    ...
    closeMesaOnFinish: raw.closeMesaOnFinish === true,
  };
}
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isSportingbet24d2fGameUrl(t.url)) ??
  ...
  const sportingbet24d2fAutopilot = await SinglestakeSportingbet24d2fSignalRunner.getSportingbet24d2fAutopilotStatus();
  const sportingbet24d2fConfig = await SinglestakeSportingbet24d2fSignalRunner.getSportingbet24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    sportingbet24d2fAutopilot,
    sportingbet24d2fConfig,
...
function isSportingbet24d2fCrossingContext(context) {
  return context?.strategy === "sportingbet24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isSportingbet24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isSportingbet24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isSportingbet24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "sportingbet24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isSportingbet24d2fSignal =
    isSportingbet24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("sportingbet24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isSportingbet24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isSportingbet24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isSportingbet24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isSportingbet24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isSportingbet24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeSportingbet24d2f?.sportingbet24d2fPadFactorPlacementMs === "function"
          ? SinglestakeSportingbet24d2f.sportingbet24d2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (ICE2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isSportingbet24d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "sportingbet.bet.br" || host.endsWith(".sportingbet.bet.br") || host.endsWith(".sportingbet.bet.br")) {
      return SPORTINGBET24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[SPORTINGBET24D2F_CALIB_SITE_KEY]) {
    return { siteKey: SPORTINGBET24D2F_CALIB_SITE_KEY, site: sites[SPORTINGBET24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: SPORTINGBET24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[SPORTINGBET24D2F_CALIB_SITE_KEY]) return { siteKey: SPORTINGBET24D2F_CALIB_SITE_KEY, site: sites[SPORTINGBET24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? SPORTINGBET24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === SPORTINGBET24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isSportingbet24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isIceHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isIceHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeSportingbet24d2f !== "undefined" && SinglestakeSportingbet24d2f.SPORTINGBET24D2F_MESA_URL) ||
          "https://br4.bet.br/play/pragmatic/24d-spin",
        mesaProvider: null,
        currentTableId: SPORTINGBET24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Sportingbet 24D encontrada. Abra o 24D Spin (sportingbet.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeSportingbet24d2fSignalRunner.initSportingbet24d2fSignalRunner(handleBridgePayload);
