...
const ice2fStatus = document.getElementById("sportingbet24d2fStatus");
...
const ice2fOnBtn = document.getElementById("sportingbet24d2fOn");
const ice2fOffBtn = document.getElementById("sportingbet24d2fOff");
const ice2fResetBtn = document.getElementById("sportingbet24d2fReset");
const ice2fCasinoId = document.getElementById("sportingbet24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("sportingbet24d2fSave");
const sportingbet24d2fConfigStatus = document.getElementById("sportingbet24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, sportingbet24d2fConfig) {
  ...
  const noGale =
    sportingbet24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = sportingbet24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? sportingbet24d2fConfig?.enabledPairIds,
  );
  ...
  if (!on) {
    ice2fStatus.textContent = "Parada — clique Ligar para seguir a mesa 3426 via DGA.";
    return;
  }
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.sportingbet24d2fAutopilot, mode, status?.sportingbet24d2fConfig);

  const cfg = status?.sportingbet24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "sportingbet.bet.br/play/24DSpin"}`);
  lines.push(`Table ID: ${cfg.tableId ?? 3426}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa 24D 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra a roleta 24D 2F, aguarde carregar, clique na aba e use 📍.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa 24D 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  const r = await send("set-ice2f-autopilot", { enabled: true });
  if (r?.ok === false && ice2fStatus) {
    ice2fStatus.textContent = r.error ?? "Falha ao ligar";
  }
  await send("open-panel");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.sportingbet24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (sportingbet24d2fConfigStatus) {
    sportingbet24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
