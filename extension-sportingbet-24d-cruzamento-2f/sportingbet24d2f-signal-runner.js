/** Autopilot Sportingbet — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_SPORTINGBET24D2F_AUTOPLAY = "gogSportingbet24d2fAutopilotEnabled";
const STORAGE_SPORTINGBET24D2F_CONFIG = "gogSportingbet24d2fConfig";
const STORAGE_SPORTINGBET24D2F_STATUS = "gogSportingbet24d2fAutopilotStatus";
const STORAGE_SPORTINGBET24D2F_STATS = "gogSportingbet24d2fAutopilotSessionStats";
const STORAGE_SPORTINGBET24D2F_MACHINE = "gogSportingbet24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeSportingbet24d2f['createSportingbet24d2fEngine']>|null} */
let engine = null;
...
function bumpSportingbet24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function sportingbet24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeSportingbet24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeSportingbet24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const SPORTINGBET24D2F_DEFAULTS = {
  tableId: SinglestakeSportingbet24d2f?.SPORTINGBET24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeSportingbet24d2f?.SPORTINGBET24D2F_MESA_URL ??
    "https://br4.bet.br/play/pragmatic/24d-spin",
...
function knownPairIds() {
  const known = SinglestakeSportingbet24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeSportingbet24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeSportingbet24d2f !== "undefined"
      ? SinglestakeSportingbet24d2f.applyIce2fEnabledPairIds
        ? SinglestakeSportingbet24d2f
        : SinglestakeSportingbet24d2f.default
      : null;
  ...
  if (typeof engine?.dropCycleIfPairDisabled === "function") {
    ...
    if (dropped) {
      ...
      void writeSportingbet24d2fStatus({
        active: false,
        label: null,
        waitingBet: false,
        waitingReference: false,
...
async function readSportingbet24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_AUTOPLAY]);
  return data[STORAGE_SPORTINGBET24D2F_AUTOPLAY] === true;
}

async function readSportingbet24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_CONFIG]);
  const stored = data[STORAGE_SPORTINGBET24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...SPORTINGBET24D2F_DEFAULTS,
      maxRecoveryPreference: SPORTINGBET24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? SPORTINGBET24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? SPORTINGBET24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : SPORTINGBET24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? SPORTINGBET24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : SPORTINGBET24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : SPORTINGBET24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : SPORTINGBET24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : SPORTINGBET24D2F_DEFAULTS.currency,
    noGale,
...
async function readSportingbet24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_MACHINE]);
  const raw = data[STORAGE_SPORTINGBET24D2F_MACHINE];
...
async function persistSportingbet24d2fMachineState(machine) {
  ...
  await chrome.storage.local.set({
    [STORAGE_SPORTINGBET24D2F_MACHINE]: {
      recovery:
...
async function clearSportingbet24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_SPORTINGBET24D2F_MACHINE]);
}

async function readSportingbet24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_STATS]);
  const raw = data[STORAGE_SPORTINGBET24D2F_STATS];
...
async function persistSportingbet24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_SPORTINGBET24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, sportingbet24d2fIdleIndicationPatch());
  }
  await writeSportingbet24d2fStatus(patch);
}

async function resetSportingbet24d2fStats() {
  const cfg = await readSportingbet24d2fConfig();
  const empty =
    globalThis.SinglestakeSportingbet24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistSportingbet24d2fStats(empty, cfg.maxRecovery);
  await clearSportingbet24d2fMachineState();
...
async function getSportingbet24d2fConfigForPopup() {
  return readSportingbet24d2fConfig();
}

async function setSportingbet24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_CONFIG]);
  const stored =
    data[STORAGE_SPORTINGBET24D2F_CONFIG] && typeof data[STORAGE_SPORTINGBET24D2F_CONFIG] === "object"
      ? data[STORAGE_SPORTINGBET24D2F_CONFIG]
      : {};
  const prev = await readSportingbet24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_SPORTINGBET24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearSportingbet24d2fMachineState();
    await writeSportingbet24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readSportingbet24d2fConfig()) };
}

async function writeSportingbet24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_STATUS]);
  const prev = data[STORAGE_SPORTINGBET24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_SPORTINGBET24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeSportingbet24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistSportingbet24d2fMachineState(live.machine);
    await writeSportingbet24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readSportingbet24d2fConfig();
    ...
    await writeSportingbet24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readSportingbet24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeSportingbet24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeSportingbet24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeSportingbet24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeSportingbet24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeSportingbet24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function syncReferencePauseStatus(result, cfg) {
  ...
  lastEmittedSignalId = null;
  engine?.abortBetCommit?.();
  await writeSportingbet24d2fStatus({
    active: true,
    tableId: cfg.tableId,
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!active) {
    ...
    lastEmittedSignalId = null;
    await writeSportingbet24d2fIndicationStatus(
      sportingbet24d2fIdleIndicationPatch({
        tableId: null,
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `sportingbet24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistSportingbet24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistSportingbet24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpSportingbet24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeSportingbet24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeSportingbet24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeSportingbet24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeSportingbet24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeSportingbet24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startSportingbet24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeSportingbet24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readSportingbet24d2fAutopilotEnabled();
  if (!enabled) {
    await writeSportingbet24d2fStatus({ running: false, reason: "autopilot Sportingbet 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeSportingbet24d2f?.createSportingbet24d2fEngine) {
    await writeSportingbet24d2fStatus({
      running: false,
      reason: "sportingbet24d2f-engine.js em falta — corra npm run extension:sportingbet24d2f:build",
    });
    return;
  }

  stopSportingbet24d2fAutopilot();

  const cfg = await readSportingbet24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readSportingbet24d2fStats(cfg.maxRecovery);
  const savedMachine = await readSportingbet24d2fMachineState();
  engine = SinglestakeSportingbet24d2f.createSportingbet24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  lastEmittedSignalId = null;
  clearPendingBetTimers();
  bumpSportingbet24d2fIndicationEpoch();
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeSportingbet24d2fStatus({ log: msg }),
    onStatus: (status) => void writeSportingbet24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readSportingbet24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readSportingbet24d2fConfig().then((cfgNow) =>
...
function stopSportingbet24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpSportingbet24d2fIndicationEpoch();
  void writeSportingbet24d2fStatus(sportingbet24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setSportingbet24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_SPORTINGBET24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startSportingbet24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeSportingbet24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeSportingbet24d2fBridgeHandler) {
      await writeSportingbet24d2fStatus({
        running: false,
  ...
  const status = await getSportingbet24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getSportingbet24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_SPORTINGBET24D2F_STATUS, STORAGE_SPORTINGBET24D2F_AUTOPLAY, STORAGE_SPORTINGBET24D2F_STATS]);
  const statsPack = data[STORAGE_SPORTINGBET24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_SPORTINGBET24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_SPORTINGBET24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_SPORTINGBET24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_SPORTINGBET24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readSportingbet24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_SPORTINGBET24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readSportingbet24d2fConfig();
  const streak =
    globalThis.SinglestakeSportingbet24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeSportingbet24d2f?.ice2fPairLabel === "function"
        ? SinglestakeSportingbet24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_SPORTINGBET24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_SPORTINGBET24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initSportingbet24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeSportingbet24d2fBridgeHandler = handleBridgePayload;
  void readSportingbet24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readSportingbet24d2fAutopilotEnabled().then((on) => {
    if (on) void startSportingbet24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_SPORTINGBET24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_SPORTINGBET24D2F_AUTOPLAY].newValue === true) {
        void startSportingbet24d2fAutopilot(bridgeHandler);
      } else {
        stopSportingbet24d2fAutopilot();
      }
    }
    if (changes[STORAGE_SPORTINGBET24D2F_CONFIG] && bridgeHandler) {
      void readSportingbet24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readSportingbet24d2fAutopilotEnabled().then((on) => {
          if (on) void startSportingbet24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeSportingbet24d2fSignalRunner = {
    initSportingbet24d2fSignalRunner,
    startSportingbet24d2fAutopilot,
    stopSportingbet24d2fAutopilot,
    setSportingbet24d2fAutopilotEnabled,
    getSportingbet24d2fAutopilotStatus,
    getSportingbet24d2fConfigForPopup,
    setSportingbet24d2fConfigFromPopup,
    resetSportingbet24d2fStats,
    STORAGE_SPORTINGBET24D2F_AUTOPLAY,
    STORAGE_SPORTINGBET24D2F_CONFIG,
  };
}
