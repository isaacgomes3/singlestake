...
function ice3fUnitClickStaggerMs(recovery) {
  const base = GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150;
  const units = ktoStakeUnitsForRecovery(recovery);
  if (units <= ICE3F_GALE3_REFERENCE_UNITS) return base;
  const refUnits = ICE3F_GALE3_REFERENCE_UNITS;
