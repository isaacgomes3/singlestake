/**
 * Painel flutuante na ICE 3F — activar estratégia 2 Fatores · cruzamento sequencial (mesa 201).
 */
(function () {
  ...
  function pageIsKtoRoulette() {
    ...
    return (
      /ice3f\.bet\.br/i.test(location.hostname) &&
      /\/app\/cassino\/game\/(liveroulettea-pragmaticexternal|roleta-ao-vivo)/i.test(path)
    );
  }

  if (!pageIsKtoRoulette()) return;
  if (document.getElementById("ss-ice3f-panel-root")) return;

  const PANEL_ID = "ss-ice3f-panel-root";
  const POLL_MS = 2000;
  let pollTimer = null;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.kind === "ice3f-panel-ping") {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .ss-ice3f-body { padding: 10px 12px 12px; }
    #${PANEL_ID} .ss-ice3f-sub {
      margin: 0 0 8px;
      font-size: 10px;
      color: #94a3b8;
      line-height: 1.45;
    }
    #${PANEL_ID} .ss-ice3f-status {
      margin: 0 0 10px;
      font-size: 10px;
      color: #cbd5e1;
      min-height: 2.8em;
    ...
    #${PANEL_ID} .ss-ice3f-status.err { color: #fca5a5; }
    #${PANEL_ID} .ss-ice3f-stats {
      display: flex;
      gap: 8px;
      margin-bottom: 10px;
    }
    #${PANEL_ID} .ss-ice3f-stat {
      flex: 1;
      text-align: center;
      padding: 6px;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-ice3f-stat strong {
      display: block;
      font-size: 18px;
      color: #f8fafc;
    }
    #${PANEL_ID} .ss-ice3f-stat small {
      font-size: 8px;
      font-weight: 700;
      text-transform: uppercase;
      color: #94a3b8;
    }
    #${PANEL_ID} .ss-ice3f-stat.win { border-color: #166534; background: #052e16; }
    #${PANEL_ID} .ss-ice3f-stat.win strong { color: #86efac; }
    #${PANEL_ID} .ss-ice3f-stat.loss { border-color: #991b1b; background: #450a0a; }
    #${PANEL_ID} .ss-ice3f-stat.loss strong { color: #fca5a5; }
    #${PANEL_ID} .ss-ice3f-actions {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }
    #${PANEL_ID} .ss-ice3f-btn {
      flex: 1;
      min-width: 72px;
      border: 1px solid #334155;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-ice3f-btn.on-demo {
      border-color: #2563eb;
      background: #172554;
      color: #bfdbfe;
    }
    #${PANEL_ID} .ss-ice3f-btn.on-real {
      border-color: #16a34a;
      background: #14532d;
      color: #bbf7d0;
    }
    #${PANEL_ID} .ss-ice3f-btn.on-active {
      border-color: #059669;
      background: #064e3b;
      color: #a7f3d0;
    }
    #${PANEL_ID} .ss-ice3f-btn.on-warn {
      border-color: #b45309;
      background: #451a03;
      color: #fcd34d;
    }
    #${PANEL_ID} .ss-ice3f-hidden { display: none !important; }
    #${PANEL_ID}.ss-ice3f-collapsed .ss-ice3f-body { display: none; }
  `;

  const root = el("div");
  root.id = PANEL_ID;

  const card = el("div", "ss-ice3f-card");
  const head = el("div", "ss-ice3f-head");
  const title = el("div", "ss-ice3f-title", "stake37 · 2 Fatores");
  const minBtn = el("button", "ss-ice3f-min", "−");
  minBtn.type = "button";
  ...
  const winStat = el("div", "ss-ice3f-stat win");
  const winNum = el("strong", null, "0");
  winStat.append(winNum, el("small", null, "Vitórias"));
  const lossStat = el("div", "ss-ice3f-stat loss");
  const lossNum = el("strong", null, "0");
  lossStat.append(lossNum, el("small", null, "Derrotas"));
  stats.append(winStat, lossStat);

  const actions = el("div", "ss-ice3f-actions");
  const btnDemo = el("button", "ss-ice3f-btn on-demo", "Demo");
  const btnReal = el("button", "ss-ice3f-btn", "Real");
  const btnToggle = el("button", "ss-ice3f-btn", "Parado");
  const btnReload = el("button", "ss-ice3f-btn on-warn ss-ice3f-hidden", "Actualizar página");
  btnDemo.type = "button";
  btnReal.type = "button";
  ...
  minBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    collapsed = !collapsed;
    root.classList.toggle("ss-ice3f-collapsed", collapsed);
  ...
  btnToggle.addEventListener("click", async () => {
    ...
    const cur = await send("get-ice3f-autopilot");
    const on = cur?.enabled === true;
    statusEl.textContent = on ? "A parar…" : "A ligar estratégia…";
    const r = await send("set-ice3f-autopilot", { enabled: !on });
  ...
  function renderStatus(data) {
    ...
    if (!data || data.error) {
      ...
      btnToggle.className = "ss-ice3f-btn on-warn";
      btnToggle.textContent = "Parado";
      if (contextDead) btnReload.classList.remove("ss-ice3f-hidden");
      return;
    }

    btnReload.classList.add("ss-ice3f-hidden");

    const mode = data.mode === "real" ? "real" : "demo";
    const ice3f = data.ice3fAutopilot;
    const st = ice3f?.status ?? {};
    const on = ice3f?.enabled === true;
    const running = st.running === true;

    btnDemo.className = `ss-ice3f-btn ${mode === "demo" ? "on-demo" : ""}`;
    btnReal.className = `ss-ice3f-btn ${mode === "real" ? "on-real" : ""}`;

    if (on && running) {
      btnToggle.className = "ss-ice3f-btn on-active";
      btnToggle.textContent = "Activo";
    } else if (on) {
      btnToggle.className = "ss-ice3f-btn on-warn";
      btnToggle.textContent = "A ligar…";
    } else {
      btnToggle.className = "ss-ice3f-btn";
      btnToggle.textContent = "Parado";
    }
