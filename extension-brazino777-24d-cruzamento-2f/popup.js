...
const ice2fStatus = document.getElementById("brazino24d2fStatus");
...
const ice2fOnBtn = document.getElementById("brazino24d2fOn");
const ice2fOffBtn = document.getElementById("brazino24d2fOff");
const ice2fResetBtn = document.getElementById("brazino24d2fReset");
const ice2fCasinoId = document.getElementById("brazino24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("brazino24d2fSave");
const brazino24d2fConfigStatus = document.getElementById("brazino24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, brazino24d2fConfig) {
  ...
  const noGale =
    brazino24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = brazino24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? brazino24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.brazino24d2fAutopilot, mode, status?.brazino24d2fConfig);

  const cfg = status?.brazino24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "brazino777.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.brazino24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (brazino24d2fConfigStatus) {
    brazino24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
