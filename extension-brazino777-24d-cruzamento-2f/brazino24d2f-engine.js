...
var SinglestakeBrazino24d2f = (() => {
  ...
  // extension-brazino777-24d-cruzamento-2f/brazino24d2f-strategy-entry.ts
  var brazino24d2f_strategy_entry_exports = {};
  __export(brazino24d2f_strategy_entry_exports, {
    BRAZINO24D2F_MAX_GALES: () => BRAZINO24D2F_MAX_GALES,
    BRAZINO24D2F_MESA_URL: () => BRAZINO24D2F_MESA_URL,
    BRAZINO24D2F_TABLE_ID: () => BRAZINO24D2F_TABLE_ID,
    ...
    createBrazino24d2fEngine: () => createBrazino24d2fEngine,
    default: () => brazino24d2f_strategy_entry_default,
  ...
  // extension-brazino777-24d-cruzamento-2f/brazino24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var BRAZINO24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var BRAZINO24D2F_MESA_URL = "https://www.brazino777.bet.br/game/24d_spin_pr";
  var BRAZINO24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var BRAZINO24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var BRAZINO24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var BRAZINO24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of BRAZINO24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createBrazino24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && BRAZINO24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && BRAZINO24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && BRAZINO24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && BRAZINO24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && BRAZINO24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && BRAZINO24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && BRAZINO24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = BRAZINO24D2F_MESA_URL) {
      ...
      const signalId = `brazino24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: BRAZINO24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? BRAZINO24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    BRAZINO24D2F_TABLE_ID,
    BRAZINO24D2F_MESA_URL,
    BRAZINO24D2F_MAX_GALES,
    ICE_2F_BET_DELAY_MS,
    ...
    createBrazino24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeBrazino24d2f = api;
  }
  var brazino24d2f_strategy_entry_default = api;
  return __toCommonJS(brazino24d2f_strategy_entry_exports);
})();
