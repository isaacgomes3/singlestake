/** Autopilot Brazino777 — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_BRAZINO24D2F_AUTOPLAY = "gogBrazino24d2fAutopilotEnabled";
const STORAGE_BRAZINO24D2F_CONFIG = "gogBrazino24d2fConfig";
const STORAGE_BRAZINO24D2F_STATUS = "gogBrazino24d2fAutopilotStatus";
const STORAGE_BRAZINO24D2F_STATS = "gogBrazino24d2fAutopilotSessionStats";
const STORAGE_BRAZINO24D2F_MACHINE = "gogBrazino24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeBrazino24d2f['createBrazino24d2fEngine']>|null} */
let engine = null;
...
function bumpBrazino24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function brazino24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeBrazino24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeBrazino24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const BRAZINO24D2F_DEFAULTS = {
  tableId: SinglestakeBrazino24d2f?.BRAZINO24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeBrazino24d2f?.BRAZINO24D2F_MESA_URL ??
    "https://www.brazino777.bet.br/game/24d_spin_pr",
...
function knownPairIds() {
  const known = SinglestakeBrazino24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeBrazino24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeBrazino24d2f !== "undefined"
      ? SinglestakeBrazino24d2f.applyIce2fEnabledPairIds
        ? SinglestakeBrazino24d2f
        : SinglestakeBrazino24d2f.default
      : null;
...
async function readBrazino24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_AUTOPLAY]);
  return data[STORAGE_BRAZINO24D2F_AUTOPLAY] === true;
}

async function readBrazino24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_CONFIG]);
  const stored = data[STORAGE_BRAZINO24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...BRAZINO24D2F_DEFAULTS,
      maxRecoveryPreference: BRAZINO24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? BRAZINO24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? BRAZINO24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : BRAZINO24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? BRAZINO24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : BRAZINO24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : BRAZINO24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : BRAZINO24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : BRAZINO24D2F_DEFAULTS.currency,
    noGale,
...
async function readBrazino24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_MACHINE]);
  const raw = data[STORAGE_BRAZINO24D2F_MACHINE];
...
async function persistBrazino24d2fMachineState(machine) {
...
async function clearBrazino24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_BRAZINO24D2F_MACHINE]);
}

async function readBrazino24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_STATS]);
  const raw = data[STORAGE_BRAZINO24D2F_STATS];
...
async function persistBrazino24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_BRAZINO24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, brazino24d2fIdleIndicationPatch());
  }
  await writeBrazino24d2fStatus(patch);
}

async function resetBrazino24d2fStats() {
  const cfg = await readBrazino24d2fConfig();
  const empty =
    globalThis.SinglestakeBrazino24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistBrazino24d2fStats(empty, cfg.maxRecovery);
  await clearBrazino24d2fMachineState();
...
async function getBrazino24d2fConfigForPopup() {
  return readBrazino24d2fConfig();
}

async function setBrazino24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_CONFIG]);
  const stored =
    data[STORAGE_BRAZINO24D2F_CONFIG] && typeof data[STORAGE_BRAZINO24D2F_CONFIG] === "object"
      ? data[STORAGE_BRAZINO24D2F_CONFIG]
      : {};
  const prev = await readBrazino24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_BRAZINO24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearBrazino24d2fMachineState();
    await writeBrazino24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readBrazino24d2fConfig()) };
}

async function writeBrazino24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_STATUS]);
  const prev = data[STORAGE_BRAZINO24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_BRAZINO24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeBrazino24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistBrazino24d2fMachineState(live.machine);
    await writeBrazino24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readBrazino24d2fConfig();
    ...
    await writeBrazino24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readBrazino24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeBrazino24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeBrazino24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeBrazino24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeBrazino24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeBrazino24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!active) {
    ...
    lastEmittedSignalId = null;
    await writeBrazino24d2fIndicationStatus(
      brazino24d2fIdleIndicationPatch({
        tableId: null,
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `brazino24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistBrazino24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistBrazino24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpBrazino24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeBrazino24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeBrazino24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeBrazino24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeBrazino24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeBrazino24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startBrazino24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeBrazino24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readBrazino24d2fAutopilotEnabled();
  if (!enabled) {
    await writeBrazino24d2fStatus({ running: false, reason: "autopilot Brazino777 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeBrazino24d2f?.createBrazino24d2fEngine) {
    await writeBrazino24d2fStatus({
      running: false,
      reason: "brazino24d2f-engine.js em falta — corra npm run extension:brazino24d2f:build",
    });
    return;
  }

  stopBrazino24d2fAutopilot();

  const cfg = await readBrazino24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readBrazino24d2fStats(cfg.maxRecovery);
  const savedMachine = await readBrazino24d2fMachineState();
  engine = SinglestakeBrazino24d2f.createBrazino24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  lastEmittedSignalId = null;
  clearPendingBetTimers();
  bumpBrazino24d2fIndicationEpoch();
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeBrazino24d2fStatus({ log: msg }),
    onStatus: (status) => void writeBrazino24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readBrazino24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readBrazino24d2fConfig().then((cfgNow) =>
...
function stopBrazino24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpBrazino24d2fIndicationEpoch();
  void writeBrazino24d2fStatus(brazino24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setBrazino24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_BRAZINO24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startBrazino24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeBrazino24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeBrazino24d2fBridgeHandler) {
      await writeBrazino24d2fStatus({
        running: false,
  ...
  const status = await getBrazino24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getBrazino24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_BRAZINO24D2F_STATUS, STORAGE_BRAZINO24D2F_AUTOPLAY, STORAGE_BRAZINO24D2F_STATS]);
  const statsPack = data[STORAGE_BRAZINO24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_BRAZINO24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_BRAZINO24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_BRAZINO24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_BRAZINO24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readBrazino24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_BRAZINO24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readBrazino24d2fConfig();
  const streak =
    globalThis.SinglestakeBrazino24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeBrazino24d2f?.ice2fPairLabel === "function"
        ? SinglestakeBrazino24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_BRAZINO24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_BRAZINO24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initBrazino24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeBrazino24d2fBridgeHandler = handleBridgePayload;
  void readBrazino24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readBrazino24d2fAutopilotEnabled().then((on) => {
    if (on) void startBrazino24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_BRAZINO24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_BRAZINO24D2F_AUTOPLAY].newValue === true) {
        void startBrazino24d2fAutopilot(bridgeHandler);
      } else {
        stopBrazino24d2fAutopilot();
      }
    }
    if (changes[STORAGE_BRAZINO24D2F_CONFIG] && bridgeHandler) {
      void readBrazino24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readBrazino24d2fAutopilotEnabled().then((on) => {
          if (on) void startBrazino24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeBrazino24d2fSignalRunner = {
    initBrazino24d2fSignalRunner,
    startBrazino24d2fAutopilot,
    stopBrazino24d2fAutopilot,
    setBrazino24d2fAutopilotEnabled,
    getBrazino24d2fAutopilotStatus,
    getBrazino24d2fConfigForPopup,
    setBrazino24d2fConfigFromPopup,
    resetBrazino24d2fStats,
    STORAGE_BRAZINO24D2F_AUTOPLAY,
    STORAGE_BRAZINO24D2F_CONFIG,
  };
}
