importScripts("shared.js", "brazino24d2f-engine.js", "dga-hub.js", "brazino24d2f-signal-runner.js");
...
async function openOrFocusBrazino24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (brazino24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(brazino24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      brazino24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    brazino24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: brazino24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogBrazino24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureBrazino24d2fPanelOnOpenTabs();
  void openOrFocusBrazino24d2fPanel();
});
...
/** Janela fixa do painel Brazino777 24D 2F (não fecha ao clicar na mesa). */
let brazino24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === brazino24d2fPanelWindowId) brazino24d2fPanelWindowId = null;
});

const BRAZINO24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const BRAZINO24D2F_CALIB_SITE_KEY = "brazino777.bet.br|pragmatic-24d-spin";
...
function isBrazino24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "brazino777.bet.br" || host.endsWith(".brazino777.bet.br"))) return false;
    return /\/game\/24d[_-]?spin/i.test(path) || /24d[-_]?spin|24dspin|3426/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureBrazino24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isBrazino24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "brazino24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-brazino24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureBrazino24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isBrazino24d2fGameUrl(tab.url)) {
      await ensureBrazino24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isBrazino24d2fGameUrl(url)
  ) {
    registerMesaTab(BRAZINO24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureBrazino24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeBrazino24d2fSignalRunner.setBrazino24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureBrazino24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusBrazino24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusBrazino24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeBrazino24d2fSignalRunner.getBrazino24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeBrazino24d2fSignalRunner.resetBrazino24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeBrazino24d2fSignalRunner.getBrazino24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeBrazino24d2fSignalRunner.setBrazino24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isBrazino24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(BRAZINO24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isBrazino24d2fGameUrl(t.url)) ??
  ...
  const brazino24d2fAutopilot = await SinglestakeBrazino24d2fSignalRunner.getBrazino24d2fAutopilotStatus();
  const brazino24d2fConfig = await SinglestakeBrazino24d2fSignalRunner.getBrazino24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    brazino24d2fAutopilot,
    brazino24d2fConfig,
...
function isBrazino24d2fCrossingContext(context) {
  return context?.strategy === "brazino24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isBrazino24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isBrazino24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isBrazino24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "brazino24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isBrazino24d2fSignal =
    isBrazino24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("brazino24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isBrazino24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isBrazino24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isBrazino24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isBrazino24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isBrazino24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeBrazino24d2f?.brazino24d2fPadFactorPlacementMs === "function"
          ? SinglestakeBrazino24d2f.brazino24d2fPadFactorPlacementMs(padUnits)
...
function isCasinoPlayUrl(url) {
  return isBrazino24d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "brazino777.bet.br" || host.endsWith(".brazino777.bet.br") || host.endsWith(".brazino777.bet.br")) {
      return BRAZINO24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[BRAZINO24D2F_CALIB_SITE_KEY]) {
    return { siteKey: BRAZINO24D2F_CALIB_SITE_KEY, site: sites[BRAZINO24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: BRAZINO24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[BRAZINO24D2F_CALIB_SITE_KEY]) return { siteKey: BRAZINO24D2F_CALIB_SITE_KEY, site: sites[BRAZINO24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? BRAZINO24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === BRAZINO24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isBrazino24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeBrazino24d2f !== "undefined" && SinglestakeBrazino24d2f.BRAZINO24D2F_MESA_URL) ||
          "https://www.brazino777.bet.br/game/24d_spin_pr",
        mesaProvider: null,
        currentTableId: BRAZINO24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Brazino777 24D encontrada. Abra o 24D Spin (brazino777.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeBrazino24d2fSignalRunner.initBrazino24d2fSignalRunner(handleBridgePayload);
