importScripts("shared.js", "kto-cruzamento-engine.js", "dga-hub.js", "kto-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogKtoAutopilotEnabled: false });
  }
  void ensureKtoPanelOnOpenTabs();
});
...
const KTO_DEFAULT_TABLE_ID = 230;

function isKtoRoulettePageUrl(url) {
  if (!url) return false;
  try {
    const u = new URL(url);
...
async function ensureKtoPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isKtoRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "kto-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-kto-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureKtoPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isKtoRoulettePageUrl(tab.url)) {
      await ensureKtoPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isKtoRoulettePageUrl(url)
  ) {
    registerMesaTab(KTO_DEFAULT_TABLE_ID, tabId);
    void ensureKtoPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-kto-autopilot") {
    void SinglestakeKtoSignalRunner.setKtoAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureKtoPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-kto-autopilot") {
    void SinglestakeKtoSignalRunner.getKtoAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-kto-stats") {
    void SinglestakeKtoSignalRunner.resetKtoStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-kto-config") {
    void SinglestakeKtoSignalRunner.getKtoConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-kto-config") {
    void SinglestakeKtoSignalRunner.setKtoConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isKtoRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(KTO_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const ktoAutopilot = await SinglestakeKtoSignalRunner.getKtoAutopilotStatus();
  const ktoConfig = await SinglestakeKtoSignalRunner.getKtoConfigForPopup();
  return {
    ok: true,
    mode,
    ktoAutopilot,
...
async function dispatchClickAction(action, context, sourceTabId) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isKtoSignal =
    isKtoCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("kto:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isKtoSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isKtoSignal) {
...
function isCasinoPlayUrl(url) {
  return isKtoRoulettePageUrl(url);
}
...
SinglestakeKtoSignalRunner.initKtoSignalRunner(handleBridgePayload);
