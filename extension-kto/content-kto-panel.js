/**
 * Painel flutuante na KTO — activar estratégia 2 Fatores · cruzamento sequencial (mesa 230).
 */
(function () {
  ...
  if (document.getElementById("ss-kto-panel-root")) return;

  const PANEL_ID = "ss-kto-panel-root";
  const POLL_MS = 2000;
  let pollTimer = null;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.kind === "kto-panel-ping") {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .ss-kto-body { padding: 10px 12px 12px; }
    #${PANEL_ID} .ss-kto-sub {
      margin: 0 0 8px;
      font-size: 10px;
      color: #94a3b8;
    ...
    #${PANEL_ID} .ss-kto-status.err { color: #fca5a5; }
    #${PANEL_ID} .ss-kto-stats {
      display: flex;
      gap: 8px;
      margin-bottom: 10px;
    }
    #${PANEL_ID} .ss-kto-stat {
      flex: 1;
      text-align: center;
      padding: 6px;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-kto-stat strong {
      display: block;
      font-size: 18px;
      color: #f8fafc;
    }
    #${PANEL_ID} .ss-kto-stat small {
      font-size: 8px;
      font-weight: 700;
      text-transform: uppercase;
      color: #94a3b8;
    }
    #${PANEL_ID} .ss-kto-stat.win { border-color: #166534; background: #052e16; }
    #${PANEL_ID} .ss-kto-stat.win strong { color: #86efac; }
    #${PANEL_ID} .ss-kto-stat.loss { border-color: #991b1b; background: #450a0a; }
    #${PANEL_ID} .ss-kto-stat.loss strong { color: #fca5a5; }
    #${PANEL_ID} .ss-kto-actions {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }
    ...
    #${PANEL_ID} .ss-kto-btn.on-demo {
      border-color: #2563eb;
      background: #172554;
      color: #bfdbfe;
    }
    #${PANEL_ID} .ss-kto-btn.on-real {
      border-color: #16a34a;
      background: #14532d;
      color: #bbf7d0;
    }
    #${PANEL_ID} .ss-kto-btn.on-active {
      border-color: #059669;
      background: #064e3b;
      color: #a7f3d0;
    }
    #${PANEL_ID} .ss-kto-btn.on-warn {
      border-color: #b45309;
      background: #451a03;
      color: #fcd34d;
    }
    #${PANEL_ID} .ss-kto-hidden { display: none !important; }
    #${PANEL_ID}.ss-kto-collapsed .ss-kto-body { display: none; }
  `;

  const root = el("div");
  root.id = PANEL_ID;

  const card = el("div", "ss-kto-card");
  const head = el("div", "ss-kto-head");
  const title = el("div", "ss-kto-title", "stake37 · 2 Fatores");
  const minBtn = el("button", "ss-kto-min", "−");
  minBtn.type = "button";
  head.append(title, minBtn);

  const body = el("div", "ss-kto-body");
  const sub = el("p", "ss-kto-sub", "Roulette 3 · mesa 230 · stake 1·1·2·4·8·16·32");
  const statusEl = el("p", "ss-kto-status", "A ligar à extensão…");

  const stats = el("div", "ss-kto-stats");
  const winStat = el("div", "ss-kto-stat win");
  const winNum = el("strong", null, "0");
  winStat.append(winNum, el("small", null, "Vitórias"));
  const lossStat = el("div", "ss-kto-stat loss");
  const lossNum = el("strong", null, "0");
  lossStat.append(lossNum, el("small", null, "Derrotas"));
  stats.append(winStat, lossStat);

  const actions = el("div", "ss-kto-actions");
  const btnDemo = el("button", "ss-kto-btn on-demo", "Demo");
  const btnReal = el("button", "ss-kto-btn", "Real");
  const btnToggle = el("button", "ss-kto-btn", "Parado");
  const btnReload = el("button", "ss-kto-btn on-warn ss-kto-hidden", "Actualizar página");
  btnDemo.type = "button";
  btnReal.type = "button";
  ...
  minBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    collapsed = !collapsed;
    root.classList.toggle("ss-kto-collapsed", collapsed);
  ...
  btnToggle.addEventListener("click", async () => {
    ...
    const cur = await send("get-kto-autopilot");
    const on = cur?.enabled === true;
    statusEl.textContent = on ? "A parar…" : "A ligar estratégia…";
    const r = await send("set-kto-autopilot", { enabled: !on });
  ...
  function renderStatus(data) {
    ...
    if (!data || data.error) {
      ...
      btnToggle.className = "ss-kto-btn on-warn";
      btnToggle.textContent = "Parado";
      if (contextDead) btnReload.classList.remove("ss-kto-hidden");
      return;
    }

    btnReload.classList.add("ss-kto-hidden");

    const mode = data.mode === "real" ? "real" : "demo";
    const kto = data.ktoAutopilot;
    const st = kto?.status ?? {};
    const on = kto?.enabled === true;
    const running = st.running === true;

    btnDemo.className = `ss-kto-btn ${mode === "demo" ? "on-demo" : ""}`;
    btnReal.className = `ss-kto-btn ${mode === "real" ? "on-real" : ""}`;

    if (on && running) {
      btnToggle.className = "ss-kto-btn on-active";
      btnToggle.textContent = "Activo";
    } else if (on) {
      btnToggle.className = "ss-kto-btn on-warn";
      btnToggle.textContent = "A ligar…";
    } else {
      btnToggle.className = "ss-kto-btn";
      btnToggle.textContent = "Parado";
    }
