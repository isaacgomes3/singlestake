importScripts("shared.js", "ice-cruzamento-engine.js", "dga-hub.js", "ice-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogIceAutopilotEnabled: false });
  }
  void ensureIcePanelOnOpenTabs();
});
...
/** Janela fixa do painel ICE (não fecha ao clicar na mesa). */
let icePanelWindowId = null;

async function openOrFocusIcePanel() {
  if (icePanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(icePanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      icePanelWindowId = null;
    }
  }
  ...
  icePanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusIcePanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === icePanelWindowId) icePanelWindowId = null;
});

const ICE_DEFAULT_TABLE_ID = 201;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo (/games/tag/… → /play/pragmatic/…). */
...
function isIceRoulettePageUrl(url) {
  ...
  try {
    ...
    return /liveroulettea-pragmaticexternal|pragmaticexternal/i.test(path);
  } catch {
    return /liveroulettea|pragmaticexternal|\/games\//i.test(url);
  }
}

async function ensureIcePanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isIceRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "ice-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-ice-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureIcePanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isIceRoulettePageUrl(tab.url)) {
      await ensureIcePanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isIceRoulettePageUrl(url)
  ) {
    registerMesaTab(ICE_DEFAULT_TABLE_ID, tabId);
    void ensureIcePanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice-autopilot") {
    void SinglestakeIceSignalRunner.setIceAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureIcePanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-ice-autopilot") {
    void SinglestakeIceSignalRunner.getIceAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice-stats") {
    void SinglestakeIceSignalRunner.resetIceStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice-config") {
    void SinglestakeIceSignalRunner.getIceConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice-config") {
    void SinglestakeIceSignalRunner.setIceConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isIceRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(ICE_DEFAULT_TABLE_ID, sender.tab.id);
    }
  ...
  if (message.kind === "calibration-click") {
    void saveCalibrationClick(message, message.tabId ?? sender.tab?.id ?? null)
      .then(sendResponse)
      .catch((e) =>
        sendResponse({
          ok: false,
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isIceRoulettePageUrl(t.url)) ??
  ...
  const iceAutopilot = await SinglestakeIceSignalRunner.getIceAutopilotStatus();
  const iceConfig = await SinglestakeIceSignalRunner.getIceConfigForPopup();
  return {
    ok: true,
    mode,
    iceAutopilot,
...
function isDois2FatoresBridgeContext(context) {
  return (
    ((context?.strategy === "dois2fatores" && context?.singleFactorMode !== true) ||
      context?.strategy === "iceDuzia")
  );
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (context?.fastMultiTarget === true || context?.strategy === "iceDuzia") {
    const prevNum = typeof prevAction?.target === "string" && prevAction.target.startsWith("num-");
    const nextNum = typeof action?.target === "string" && action.target.startsWith("num-");
    if (prevNum && nextNum) return GOG.ICE_NUMBER_CLICK_STAGGER_MS ?? 3;
    if (prevAction?.target?.startsWith?.("dozen") && nextNum) {
      return GOG.ICE_DOZEN_TO_NUMBER_STAGGER_MS ?? 25;
    }
    return GOG.ICE_COVERAGE_CLICK_STAGGER_MS ?? 22;
  }
  if (
...
function isBettingClickTarget(target) {
  return (
    ...
    target === "dozen-2" ||
    target === "repeat-bet" ||
    (typeof target === "string" && target.startsWith("num-"))
  );
}
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  const isCoverage =
    payload.context?.strategy === "iceDuzia" &&
    (payload.context?.fastMultiTarget === true ||
      (typeof payload.context?.signalId === "string" &&
        payload.context.signalId.startsWith("ice:cov:")));
  let filtered = singleFactor && !isCoverage
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy !== "dois2fatores" && context?.strategy !== "iceDuzia") return;
...
async function dispatchClickAction(action, context, sourceTabId) {
  ...
  const betKey =
    typeof action.betKey === "string" && action.betKey.trim()
      ? action.betKey.trim()
      : action.target === "factor-1" || action.target === "dozen-1"
        ? context.factor1BetKey
        : action.target === "factor-2" || action.target === "dozen-2"
  ...
  if (!betKey) {
    return {
      target: action.target,
      ok: false,
      detail: "Sem chave de aposta (betKey) no sinal",
    };
  }
  ...
  const clickDedupeKey =
    signalId && action.target
      ? `${signalId}:${action.target}:${betKey}`
      : null;
  const isIceSignal =
    isIceCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    (context.signalId.startsWith("ice:duz:") || context.signalId.startsWith("ice:cov:"));
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isIceSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  const unitsOverride =
    typeof action.units === "number" && Number.isFinite(action.units)
      ? Math.max(1, Math.floor(action.units))
      : null;
  ...
  const clickResult = await executeBetWithChip(
    ...
    context,
    {
      skipBarSettle:
        ...
        action.target !== "dozen-1" &&
        isDois2FatoresBridgeContext(context),
      unitsOverride,
    },
  );

  if (clickResult.ok && clickDedupeKey && !isIceSignal) {
        ...
        function findOffset(doc, depth) {
          ...
          for (const iframe of iframes) {
            ...
            try {
              ...
              /* cross-origin — usa só o iframe externo */
...
function isIceDuziaContext(context) {
  return (
    context?.strategy === "iceDuzia" ||
    (typeof context?.signalId === "string" && context.signalId.startsWith("ice:duz:"))
  );
}

function isIceCrossingContext(context) {
  return isIceDuziaContext(context);
}

function stakeUnitsForContext(context, chip) {
  const recovery = recoveryFromContext(context);

  if (isIceDuziaContext(context)) {
    const baseStake =
      typeof context?.baseStake === "number" && context.baseStake > 0 ? context.baseStake : 0.5;
    const ICE_FIB = [1, 1, 2, 3, 5, 8, 13, 21, 34];
    const unitsFromCtx =
      typeof context?.units === "number" && Number.isFinite(context.units)
        ? Math.max(1, Math.floor(context.units))
        : null;
    const units =
      unitsFromCtx ??
      ICE_FIB[Math.min(Math.max(0, recovery), ICE_FIB.length - 1)] ??
      1;
    const stakeAmount =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  const unitsOverride =
    typeof execOpts.unitsOverride === "number" && Number.isFinite(execOpts.unitsOverride)
      ? Math.max(1, Math.floor(execOpts.unitsOverride))
      : null;
  let chipResult = {
    ok: true,
    skipped: true,
  ...
  try {
    ...
    const chip = await getSavedChipForTab(tabId);
    const stake = stakeUnitsForContext(context ?? {}, chip);
    const units = unitsOverride ?? stake.units;
    const recovery = stake.recovery;
    const stakeAmount =
      unitsOverride != null
        ? (typeof context?.baseStake === "number" && context.baseStake > 0
            ? context.baseStake
            : 0.5) * units
        : stake.stakeAmount;
    const chipValue = stake.chipValue;
    const staggerMs = clickStaggerMsForContext(context ?? {}, recovery, units);
...
function isCasinoPlayUrl(url) {
  return isIceRoulettePageUrl(url);
}
...
function betGroupFromKey(betKey) {
  ...
  if (betKey.startsWith("num:")) return "numeros";
...
async function saveCalibrationClick(message, tabId) {
  ...
  // Migra entradas antigas (pathname/www diferentes) para a chave estável do ICE.
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail: "Abra a Roulette 2 Extra Time no ICE (ice.bet.br/liveroulettea-pragmaticexternal) num separador e aguarde carregar.",
    };
  }
  ...
  async function injectOverlay() {
    ...
    await chrome.scripting.executeScript({
      ...
      func: (bk, lb, tid) => {
        ...
        window.__gogCalTabId = tid;
      },
      args: [betKey, resolvedLabel, tabId],
    });
...
SinglestakeIceSignalRunner.initIceSignalRunner(handleBridgePayload);
