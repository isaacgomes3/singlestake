...
const GOG = {
  ...
  /** ICE cobertura — entre dúzias / alvos mistos. */
  ICE_COVERAGE_CLICK_STAGGER_MS: 22,
  /** ICE cobertura — entre números consecutivos. */
  ICE_NUMBER_CLICK_STAGGER_MS: 3,
  /** ICE cobertura — transição dúzia → números. */
  ICE_DOZEN_TO_NUMBER_STAGGER_MS: 25,
...
function recoveryFromContext(context) {
  ...
  if (typeof signalId === "string" && signalId.trim()) {
    ...
    // ice:cov:d1-d2:n1-n0:recovery:scale  OR  ice:duz:dozen:n1-n0:recovery:units
    if (parts[0] === "ice" && (parts[1] === "duz" || parts[1] === "cov") && parts.length >= 5) {
...
/** Unidades altas / cobertura multi-alvo — ritmo rápido de clique. */
function iceUnitClickStaggerMs(units) {
  const base = GOG.ICE_NUMBER_CLICK_STAGGER_MS ?? 3;
  const u = Math.max(1, Math.floor(units));
  if (u <= 12) return base;
  const refUnits = 12;
  const refFillMs = (refUnits - 1) * base;
  return Math.max(3, Math.round(refFillMs / (u - 1)));
}

/** @deprecated alias */
function ktoUnitClickStaggerMs(recovery) {
  return iceUnitClickStaggerMs(Math.max(1, 2 ** Math.max(0, Math.floor(recovery))));
}

/** Repetição / ICE — ritmo de clique entre unidades do mesmo alvo. */
function clickStaggerMsForContext(context, recovery, unitsHint) {
  if (context?.fastMultiTarget === true || context?.strategy === "iceDuzia") {
    const units =
      typeof unitsHint === "number" && Number.isFinite(unitsHint)
        ? Math.max(1, Math.floor(unitsHint))
        : typeof context?.units === "number" && Number.isFinite(context.units)
          ? Math.max(1, Math.floor(context.units))
          : 1;
    return iceUnitClickStaggerMs(units);
  }
  const r =
  ...
  if (context?.strategy === "dois2fatores" && context?.singleFactorMode !== true) {
    if (isIceCrossingContext(context) && r > 8) {
      return iceUnitClickStaggerMs(Math.max(1, 2 ** r));
    }
