/**
 * Painel flutuante no ICE — gatilho 2F · 2 dúzias + 11 números · Fibonacci (mesa 201).
 */
(function () {
  if (window !== window.top) return;

  function pageIsIceRoulette() {
    ...
    if (/\/games\/tag\/roulette\/liveroulettea-pragmaticexternal/i.test(path)) return true;
    if (/\/play\/pragmatic\//i.test(path)) return true;
    return /liveroulettea-pragmaticexternal/i.test(location.href);
  }

  if (!pageIsIceRoulette()) return;
  if (document.getElementById("ss-ice-panel-root")) return;

  const PANEL_ID = "ss-ice-panel-root";
  const POLL_MS = 2000;
  let pollTimer = null;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.kind === "ice-panel-ping") {
  ...
  style.textContent = `
    ...
    #${PANEL_ID} .ss-ice-body { padding: 10px 12px 12px; }
    #${PANEL_ID} .ss-ice-sub {
      margin: 0 0 8px;
      font-size: 10px;
      color: #94a3b8;
    ...
    #${PANEL_ID} .ss-ice-status.err { color: #fca5a5; }
    #${PANEL_ID} .ss-ice-stats {
      display: flex;
      gap: 8px;
      margin-bottom: 10px;
    }
    #${PANEL_ID} .ss-ice-stat {
      flex: 1;
      text-align: center;
      padding: 6px;
      border-radius: 8px;
    ...
    #${PANEL_ID} .ss-ice-stat strong {
      display: block;
      font-size: 18px;
      color: #f8fafc;
    }
    #${PANEL_ID} .ss-ice-stat small {
      font-size: 8px;
      font-weight: 700;
      text-transform: uppercase;
      color: #94a3b8;
    }
    #${PANEL_ID} .ss-ice-stat.win { border-color: #166534; background: #052e16; }
    #${PANEL_ID} .ss-ice-stat.win strong { color: #86efac; }
    #${PANEL_ID} .ss-ice-stat.loss { border-color: #991b1b; background: #450a0a; }
    #${PANEL_ID} .ss-ice-stat.loss strong { color: #fca5a5; }
    #${PANEL_ID} .ss-ice-actions {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }
    ...
    #${PANEL_ID} .ss-ice-btn.on-demo {
      border-color: #2563eb;
      background: #172554;
      color: #bfdbfe;
    }
    #${PANEL_ID} .ss-ice-btn.on-real {
      border-color: #16a34a;
      background: #14532d;
      color: #bbf7d0;
    }
    #${PANEL_ID} .ss-ice-btn.on-active {
      border-color: #059669;
      background: #064e3b;
      color: #a7f3d0;
    }
    #${PANEL_ID} .ss-ice-btn.on-warn {
      border-color: #b45309;
      background: #451a03;
      color: #fcd34d;
    }
    #${PANEL_ID} .ss-ice-hidden { display: none !important; }
    #${PANEL_ID}.ss-ice-collapsed .ss-ice-body { display: none; }
  `;

  const root = el("div");
  root.id = PANEL_ID;

  const card = el("div", "ss-ice-card");
  const head = el("div", "ss-ice-head");
  const title = el("div", "ss-ice-title", "stake37 · ICE dúzias");
  const minBtn = el("button", "ss-ice-min", "−");
  minBtn.type = "button";
  head.append(title, minBtn);

  const body = el("div", "ss-ice-body");
  const sub = el(
    "p",
    "ss-ice-sub",
    "Roulette 2 Extra Time · mesa 201 · 2 dúzias + 11 números · Fibonacci",
  );
  const statusEl = el("p", "ss-ice-status", "A ligar à extensão…");

  const stats = el("div", "ss-ice-stats");
  const winStat = el("div", "ss-ice-stat win");
  const winNum = el("strong", null, "0");
  winStat.append(winNum, el("small", null, "Vitórias"));
  const lossStat = el("div", "ss-ice-stat loss");
  const lossNum = el("strong", null, "0");
  lossStat.append(lossNum, el("small", null, "Derrotas"));
  stats.append(winStat, lossStat);

  const actions = el("div", "ss-ice-actions");
  const btnDemo = el("button", "ss-ice-btn on-demo", "Demo");
  const btnReal = el("button", "ss-ice-btn", "Real");
  const btnToggle = el("button", "ss-ice-btn", "Parado");
  const btnReload = el("button", "ss-ice-btn on-warn ss-ice-hidden", "Actualizar página");
  btnDemo.type = "button";
  btnReal.type = "button";
  ...
  minBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    collapsed = !collapsed;
    root.classList.toggle("ss-ice-collapsed", collapsed);
  ...
  btnToggle.addEventListener("click", async () => {
    ...
    const cur = await send("get-ice-autopilot");
    const on = cur?.enabled === true;
    statusEl.textContent = on ? "A parar…" : "A ligar estratégia…";
    const r = await send("set-ice-autopilot", { enabled: !on });
  ...
  function renderStatus(data) {
    ...
    if (!data || data.error) {
      ...
      btnToggle.className = "ss-ice-btn on-warn";
      btnToggle.textContent = "Parado";
      if (contextDead) btnReload.classList.remove("ss-ice-hidden");
      return;
    }

    btnReload.classList.add("ss-ice-hidden");

    const mode = data.mode === "real" ? "real" : "demo";
    const ice = data.iceAutopilot;
    const st = ice?.status ?? {};
    const on = ice?.enabled === true;
    const running = st.running === true;

    btnDemo.className = `ss-ice-btn ${mode === "demo" ? "on-demo" : ""}`;
    btnReal.className = `ss-ice-btn ${mode === "real" ? "on-real" : ""}`;

    if (on && running) {
      btnToggle.className = "ss-ice-btn on-active";
      btnToggle.textContent = "Activo";
    } else if (on) {
      btnToggle.className = "ss-ice-btn on-warn";
      btnToggle.textContent = "A ligar…";
    } else {
      btnToggle.className = "ss-ice-btn";
      btnToggle.textContent = "Parado";
    }
    ...
    const recovery = st.recovery ?? 0;
    if (recovery > 0 && !st.active) {
      parts.push(`próxima fib ${units} un.`);
    } else if (st.active) {
      parts.push(recovery > 0 ? `fib ${units} un.` : `${units} un.`);
    }
    if (st.lastBetUnits != null && st.lastFlash) {
      parts.push(`última aposta ${st.lastBetUnits} un.`);
    }
    if (st.lastFlash) {
