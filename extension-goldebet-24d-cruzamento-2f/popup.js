...
const ice2fStatus = document.getElementById("golde24d2fStatus");
...
const ice2fOnBtn = document.getElementById("golde24d2fOn");
const ice2fOffBtn = document.getElementById("golde24d2fOff");
const ice2fResetBtn = document.getElementById("golde24d2fReset");
const ice2fCasinoId = document.getElementById("golde24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("golde24d2fSave");
const golde24d2fConfigStatus = document.getElementById("golde24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, golde24d2fConfig) {
  ...
  const noGale =
    golde24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = golde24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? golde24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.golde24d2fAutopilot, mode, status?.golde24d2fConfig);

  const cfg = status?.golde24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "goldebet.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.golde24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (golde24d2fConfigStatus) {
    golde24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
