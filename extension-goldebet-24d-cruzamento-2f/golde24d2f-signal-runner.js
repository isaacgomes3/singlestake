/** Autopilot GoldeBet — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_GOLDE24D2F_AUTOPLAY = "gogGolde24d2fAutopilotEnabled";
const STORAGE_GOLDE24D2F_CONFIG = "gogGolde24d2fConfig";
const STORAGE_GOLDE24D2F_STATUS = "gogGolde24d2fAutopilotStatus";
const STORAGE_GOLDE24D2F_STATS = "gogGolde24d2fAutopilotSessionStats";
const STORAGE_GOLDE24D2F_MACHINE = "gogGolde24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeGolde24d2f['createGolde24d2fEngine']>|null} */
let engine = null;
...
function bumpGolde24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function golde24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeGolde24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeGolde24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const GOLDE24D2F_DEFAULTS = {
  tableId: SinglestakeGolde24d2f?.GOLDE24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeGolde24d2f?.GOLDE24D2F_MESA_URL ??
    "https://goldebet.bet.br/play/pragmatic/24d-spin",
...
function knownPairIds() {
  const known = SinglestakeGolde24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeGolde24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeGolde24d2f !== "undefined"
      ? SinglestakeGolde24d2f.applyIce2fEnabledPairIds
        ? SinglestakeGolde24d2f
        : SinglestakeGolde24d2f.default
      : null;
...
async function readGolde24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_AUTOPLAY]);
  return data[STORAGE_GOLDE24D2F_AUTOPLAY] === true;
}

async function readGolde24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_CONFIG]);
  const stored = data[STORAGE_GOLDE24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...GOLDE24D2F_DEFAULTS,
      maxRecoveryPreference: GOLDE24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? GOLDE24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? GOLDE24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : GOLDE24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? GOLDE24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : GOLDE24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : GOLDE24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : GOLDE24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : GOLDE24D2F_DEFAULTS.currency,
    noGale,
...
async function readGolde24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_MACHINE]);
  const raw = data[STORAGE_GOLDE24D2F_MACHINE];
...
async function persistGolde24d2fMachineState(machine) {
...
async function clearGolde24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_GOLDE24D2F_MACHINE]);
}

async function readGolde24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_STATS]);
  const raw = data[STORAGE_GOLDE24D2F_STATS];
...
async function persistGolde24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_GOLDE24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, golde24d2fIdleIndicationPatch());
  }
  await writeGolde24d2fStatus(patch);
}

async function resetGolde24d2fStats() {
  const cfg = await readGolde24d2fConfig();
  const empty =
    globalThis.SinglestakeGolde24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistGolde24d2fStats(empty, cfg.maxRecovery);
  await clearGolde24d2fMachineState();
...
async function getGolde24d2fConfigForPopup() {
  return readGolde24d2fConfig();
}

async function setGolde24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_CONFIG]);
  const stored =
    data[STORAGE_GOLDE24D2F_CONFIG] && typeof data[STORAGE_GOLDE24D2F_CONFIG] === "object"
      ? data[STORAGE_GOLDE24D2F_CONFIG]
      : {};
  const prev = await readGolde24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_GOLDE24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearGolde24d2fMachineState();
    await writeGolde24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readGolde24d2fConfig()) };
}

async function writeGolde24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_STATUS]);
  const prev = data[STORAGE_GOLDE24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_GOLDE24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeGolde24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistGolde24d2fMachineState(live.machine);
    await writeGolde24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readGolde24d2fConfig();
    ...
    await writeGolde24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readGolde24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeGolde24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeGolde24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeGolde24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeGolde24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeGolde24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `golde24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistGolde24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistGolde24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpGolde24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeGolde24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeGolde24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeGolde24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeGolde24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeGolde24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startGolde24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeGolde24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readGolde24d2fAutopilotEnabled();
  if (!enabled) {
    await writeGolde24d2fStatus({ running: false, reason: "autopilot GoldeBet 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeGolde24d2f?.createGolde24d2fEngine) {
    await writeGolde24d2fStatus({
      running: false,
      reason: "golde24d2f-engine.js em falta — corra npm run extension:golde24d2f:build",
    });
    return;
  }

  stopGolde24d2fAutopilot();

  const cfg = await readGolde24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readGolde24d2fStats(cfg.maxRecovery);
  const savedMachine = await readGolde24d2fMachineState();
  engine = SinglestakeGolde24d2f.createGolde24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeGolde24d2fStatus({ log: msg }),
    onStatus: (status) => void writeGolde24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readGolde24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readGolde24d2fConfig().then((cfgNow) =>
...
function stopGolde24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpGolde24d2fIndicationEpoch();
  void writeGolde24d2fStatus(golde24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setGolde24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_GOLDE24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startGolde24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeGolde24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeGolde24d2fBridgeHandler) {
      await writeGolde24d2fStatus({
        running: false,
  ...
  const status = await getGolde24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getGolde24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_GOLDE24D2F_STATUS, STORAGE_GOLDE24D2F_AUTOPLAY, STORAGE_GOLDE24D2F_STATS]);
  const statsPack = data[STORAGE_GOLDE24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_GOLDE24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_GOLDE24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_GOLDE24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_GOLDE24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readGolde24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_GOLDE24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readGolde24d2fConfig();
  const streak =
    globalThis.SinglestakeGolde24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeGolde24d2f?.ice2fPairLabel === "function"
        ? SinglestakeGolde24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_GOLDE24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_GOLDE24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initGolde24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeGolde24d2fBridgeHandler = handleBridgePayload;
  void readGolde24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readGolde24d2fAutopilotEnabled().then((on) => {
    if (on) void startGolde24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_GOLDE24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_GOLDE24D2F_AUTOPLAY].newValue === true) {
        void startGolde24d2fAutopilot(bridgeHandler);
      } else {
        stopGolde24d2fAutopilot();
      }
    }
    if (changes[STORAGE_GOLDE24D2F_CONFIG] && bridgeHandler) {
      void readGolde24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readGolde24d2fAutopilotEnabled().then((on) => {
          if (on) void startGolde24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeGolde24d2fSignalRunner = {
    initGolde24d2fSignalRunner,
    startGolde24d2fAutopilot,
    stopGolde24d2fAutopilot,
    setGolde24d2fAutopilotEnabled,
    getGolde24d2fAutopilotStatus,
    getGolde24d2fConfigForPopup,
    setGolde24d2fConfigFromPopup,
    resetGolde24d2fStats,
    STORAGE_GOLDE24D2F_AUTOPLAY,
    STORAGE_GOLDE24D2F_CONFIG,
  };
}
