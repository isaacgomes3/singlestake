...
const GOG = {
  ...
  GOLDE24D2F_FACTOR_BRIDGE_STAGGER_MS: 5,
  /** Intervalo entre fichas — gale 4 (recovery 4). */
  GOLDE24D2F_GALE4_CHIP_STAGGER_MS: 15,
  /** Intervalo entre fichas — gale 5 (recovery 5). */
  GOLDE24D2F_GALE5_CHIP_STAGGER_MS: 8,
  /** Intervalo entre cliques em Dobrar (gale). */
  GOLDE24D2F_DOUBLE_CLICK_STAGGER_MS: 20,
...
function ice2fChipClickStaggerMs(recovery) {
  const r = Math.max(0, Math.floor(recovery));
  if (r >= 5) return GOG.GOLDE24D2F_GALE5_CHIP_STAGGER_MS ?? 8;
  if (r >= 4) return GOG.GOLDE24D2F_GALE4_CHIP_STAGGER_MS ?? 15;
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isGolde24d2fCrossingContext(context) && context?.singleFactorMode !== true) {
