...
var SinglestakeGolde24d2f = (() => {
  ...
  // extension-goldebet-24d-cruzamento-2f/golde24d2f-strategy-entry.ts
  var golde24d2f_strategy_entry_exports = {};
  __export(golde24d2f_strategy_entry_exports, {
    GOLDE24D2F_MAX_GALES: () => GOLDE24D2F_MAX_GALES,
    GOLDE24D2F_MESA_URL: () => GOLDE24D2F_MESA_URL,
    GOLDE24D2F_TABLE_ID: () => GOLDE24D2F_TABLE_ID,
    ...
    createGolde24d2fEngine: () => createGolde24d2fEngine,
    default: () => golde24d2f_strategy_entry_default,
  ...
  // extension-goldebet-24d-cruzamento-2f/golde24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var GOLDE24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var GOLDE24D2F_MESA_URL = "https://goldebet.bet.br/play/pragmatic/24d-spin";
  var GOLDE24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var GOLDE24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var GOLDE24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var GOLDE24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of GOLDE24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createGolde24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && GOLDE24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && GOLDE24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && GOLDE24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && GOLDE24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && GOLDE24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && GOLDE24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && GOLDE24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = GOLDE24D2F_MESA_URL) {
      ...
      const signalId = `golde24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: GOLDE24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? GOLDE24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createGolde24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeGolde24d2f = api;
  }
  var golde24d2f_strategy_entry_default = api;
  return __toCommonJS(golde24d2f_strategy_entry_exports);
})();
