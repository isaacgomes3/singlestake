importScripts("shared.js", "golde24d2f-engine.js", "dga-hub.js", "golde24d2f-signal-runner.js");
...
async function openOrFocusGolde24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (golde24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(golde24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      golde24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    golde24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: golde24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogGolde24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureGolde24d2fPanelOnOpenTabs();
  void openOrFocusGolde24d2fPanel();
});
...
/** Janela fixa do painel GoldeBet 24D 2F (não fecha ao clicar na mesa). */
let golde24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === golde24d2fPanelWindowId) golde24d2fPanelWindowId = null;
});

const GOLDE24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const GOLDE24D2F_CALIB_SITE_KEY = "goldebet.bet.br|pragmatic-24d-spin";
...
function isGolde24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "goldebet.bet.br" || host.endsWith(".goldebet.bet.br"))) return false;
    return /\/play\/pragmatic\/24d[-_]?spin(?:\/|$|\?|#)/i.test(path) || /24d[-_]?spin|24dspin|3426/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureGolde24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isGolde24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "golde24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-golde24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureGolde24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isGolde24d2fGameUrl(tab.url)) {
      await ensureGolde24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isGolde24d2fGameUrl(url)
  ) {
    registerMesaTab(GOLDE24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureGolde24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeGolde24d2fSignalRunner.setGolde24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureGolde24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusGolde24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusGolde24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeGolde24d2fSignalRunner.getGolde24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeGolde24d2fSignalRunner.resetGolde24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeGolde24d2fSignalRunner.getGolde24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeGolde24d2fSignalRunner.setGolde24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isGolde24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(GOLDE24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isGolde24d2fGameUrl(t.url)) ??
  ...
  const golde24d2fAutopilot = await SinglestakeGolde24d2fSignalRunner.getGolde24d2fAutopilotStatus();
  const golde24d2fConfig = await SinglestakeGolde24d2fSignalRunner.getGolde24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    golde24d2fAutopilot,
...
function isGolde24d2fCrossingContext(context) {
  return context?.strategy === "golde24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isGolde24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isGolde24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isGolde24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "golde24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isGolde24d2fSignal =
    isGolde24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("golde24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isGolde24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isGolde24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isGolde24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isGolde24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isGolde24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeGolde24d2f?.golde24d2fPadFactorPlacementMs === "function"
          ? SinglestakeGolde24d2f.golde24d2fPadFactorPlacementMs(padUnits)
...
function isCasinoPlayUrl(url) {
  return isGolde24d2fGameUrl(url);
}
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "goldebet.bet.br" || host.endsWith(".goldebet.bet.br") || host.endsWith(".goldebet.bet.br")) {
      return GOLDE24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[GOLDE24D2F_CALIB_SITE_KEY]) {
    return { siteKey: GOLDE24D2F_CALIB_SITE_KEY, site: sites[GOLDE24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: GOLDE24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[GOLDE24D2F_CALIB_SITE_KEY]) return { siteKey: GOLDE24D2F_CALIB_SITE_KEY, site: sites[GOLDE24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? GOLDE24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === GOLDE24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isGolde24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeGolde24d2f !== "undefined" && SinglestakeGolde24d2f.GOLDE24D2F_MESA_URL) ||
          "https://goldebet.bet.br/play/pragmatic/24d-spin",
        mesaProvider: null,
        currentTableId: GOLDE24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba GoldeBet 24D encontrada. Abra o 24D Spin (goldebet.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeGolde24d2fSignalRunner.initGolde24d2fSignalRunner(handleBridgePayload);
