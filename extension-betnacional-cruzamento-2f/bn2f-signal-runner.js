/** Autopilot Bet Nacional — Cruzamento 2F (225) · cruzamento sequencial 2 fatores. */
const STORAGE_BN2F_AUTOPLAY = "gogBn2fAutopilotEnabled";
const STORAGE_BN2F_CONFIG = "gogBn2fConfig";
const STORAGE_BN2F_STATUS = "gogBn2fAutopilotStatus";
const STORAGE_BN2F_STATS = "gogBn2fAutopilotSessionStats";
const STORAGE_BN2F_MACHINE = "gogBn2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeBn2f['createBn2fEngine']>|null} */
let engine = null;
...
const BN2F_DEFAULTS = {
  tableId: SinglestakeBn2f?.BN2F_TABLE_ID ?? 225,
  mesaUrl:
    SinglestakeBn2f?.BN2F_MESA_URL ??
...
function knownPairIds() {
  const known = SinglestakeBn2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeBn2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeBn2f !== "undefined"
      ? SinglestakeBn2f.applyIce2fEnabledPairIds
        ? SinglestakeBn2f
        : SinglestakeBn2f.default
      : null;
...
async function readBn2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_BN2F_AUTOPLAY]);
  return data[STORAGE_BN2F_AUTOPLAY] === true;
}

async function readBn2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_BN2F_CONFIG]);
  const stored = data[STORAGE_BN2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...BN2F_DEFAULTS,
      maxRecoveryPreference: BN2F_DEFAULTS.maxRecovery,
...
async function readBn2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_BN2F_MACHINE]);
  const raw = data[STORAGE_BN2F_MACHINE];
...
async function persistBn2fMachineState(machine) {
...
async function clearBn2fMachineState() {
  await chrome.storage.local.remove([STORAGE_BN2F_MACHINE]);
}

async function readBn2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_BN2F_STATS]);
  const raw = data[STORAGE_BN2F_STATS];
...
async function persistBn2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_BN2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeBn2fStatus({
    wins: stats.wins ?? 0,
...
async function resetBn2fStats() {
  const cfg = await readBn2fConfig();
  const empty =
    globalThis.SinglestakeBn2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistBn2fStats(empty, cfg.maxRecovery);
  await clearBn2fMachineState();
...
async function getBn2fConfigForPopup() {
  return readBn2fConfig();
}

async function setBn2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_BN2F_CONFIG]);
  const stored =
    data[STORAGE_BN2F_CONFIG] && typeof data[STORAGE_BN2F_CONFIG] === "object"
      ? data[STORAGE_BN2F_CONFIG]
      : {};
  const prev = await readBn2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_BN2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearBn2fMachineState();
    await writeBn2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readBn2fConfig()) };
}

async function writeBn2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_BN2F_STATUS]);
  const prev = data[STORAGE_BN2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_BN2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistBn2fMachineState(live.machine);
    await writeBn2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeBn2f?.ice2fBetDelayMs === "function"
      ? SinglestakeBn2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeBn2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeBn2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeBn2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `bn2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistBn2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistBn2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeBn2f?.ice2fWatchLabelForMachine ??
      SinglestakeBn2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeBn2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeBn2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startBn2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeBn2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readBn2fAutopilotEnabled();
  if (!enabled) {
    await writeBn2fStatus({ running: false, reason: "autopilot Bet Nacional 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeBn2f?.createBn2fEngine) {
    await writeBn2fStatus({
      running: false,
      reason: "bn2f-engine.js em falta — corra npm run extension:bn2f:build",
    });
    return;
  }
  ...
  const saved = await readBn2fStats(cfg.maxRecovery);
  const savedMachine = await readBn2fMachineState();
  engine = SinglestakeBn2f.createBn2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeBn2fStatus({ log: msg }),
    onStatus: (status) => void writeBn2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readBn2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readBn2fConfig().then((cfgNow) =>
...
function stopBn2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeBn2fStatus({ running: false, active: false, waitingBet: false });
}

async function setBn2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_BN2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startBn2fAutopilot(bridgeHandler ?? globalThis.__singlestakeBn2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeBn2fBridgeHandler) {
      await writeBn2fStatus({
        running: false,
  ...
  return getBn2fAutopilotStatus();
}

async function getBn2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_BN2F_STATUS, STORAGE_BN2F_AUTOPLAY, STORAGE_BN2F_STATS]);
  const statsPack = data[STORAGE_BN2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_BN2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_BN2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_BN2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_BN2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readBn2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_BN2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readBn2fConfig();
  const streak =
    globalThis.SinglestakeBn2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeBn2f?.ice2fPairLabel === "function"
        ? SinglestakeBn2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_BN2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_BN2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initBn2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeBn2fBridgeHandler = handleBridgePayload;
  void readBn2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readBn2fAutopilotEnabled().then((on) => {
    if (on) void startBn2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_BN2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_BN2F_AUTOPLAY].newValue === true) {
        void startBn2fAutopilot(bridgeHandler);
      } else {
        stopBn2fAutopilot();
      }
    }
    if (changes[STORAGE_BN2F_CONFIG] && bridgeHandler) {
      void readBn2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readBn2fAutopilotEnabled().then((on) => {
          if (on) void startBn2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeBn2fSignalRunner = {
    initBn2fSignalRunner,
    startBn2fAutopilot,
    stopBn2fAutopilot,
    setBn2fAutopilotEnabled,
    getBn2fAutopilotStatus,
    getBn2fConfigForPopup,
    setBn2fConfigFromPopup,
    resetBn2fStats,
    STORAGE_BN2F_AUTOPLAY,
    STORAGE_BN2F_CONFIG,
  };
}
