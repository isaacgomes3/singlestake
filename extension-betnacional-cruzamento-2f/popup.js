...
const bn2fConfigStatus = document.getElementById("bn2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, bn2fConfig) {
  ...
  const noGale =
    bn2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = bn2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? bn2fConfig?.enabledPairIds,
  );
  ...
  if (!on) {
    ice2fStatus.textContent = "Parada — clique Ligar para seguir a mesa 225 via DGA.";
    return;
  }
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.bn2fAutopilot, mode, status?.bn2fConfig);

  const cfg = status?.bn2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "betnacional.bet.br/liveroulettea-pragmaticexternal"}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa Bet Nacional 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Auto Roulette no Bet Nacional num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa Bet Nacional 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-bn2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-bn2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-bn2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.bn2fConfig ?? {};
  await send("set-bn2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-bn2f-config", { config });
  if (bn2fConfigStatus) {
    bn2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
