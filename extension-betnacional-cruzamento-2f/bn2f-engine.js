...
var SinglestakeBn2f = (() => {
  ...
  // extension-betnacional-cruzamento-2f/bn2f-strategy-entry.ts
  var bn2f_strategy_entry_exports = {};
  __export(bn2f_strategy_entry_exports, {
    BN2F_MAX_GALES: () => BN2F_MAX_GALES,
    BN2F_MESA_URL: () => BN2F_MESA_URL,
    BN2F_TABLE_ID: () => BN2F_TABLE_ID,
    ...
    createBn2fEngine: () => createBn2fEngine,
    default: () => bn2f_strategy_entry_default
  });
  ...
  var BN2F_TABLE_ID = 225;
  var BN2F_MESA_URL = "https://betnacional.bet.br/casino/game/225a54";
  var BN2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var BN2F_POSITIONS = new Set(
    getIce2fComparePairs().flatMap((p) => [...p.positions])
  );
  var BN2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var BN2F_PAIR_IDS = new Set(getIce2fComparePairs().map((p) => p.id));
  ...
  function createBn2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && BN2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && BN2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && BN2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && BN2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && BN2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && BN2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && BN2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = BN2F_MESA_URL) {
      ...
      const signalId = `bn2f:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}`;
      ...
      const actions = [
        {
          ...
          target: "factor-1",
          label: f1Label,
          reason: `Bet Nacional 2F \xB7 ${f1Label}${galeSuffix}`
        },
        {
          kind: "click",
          target: "factor-2",
          label: f2Label,
          reason: `Bet Nacional 2F \xB7 ${f2Label}${galeSuffix}`
        }
      ];
      for (let i = 0; i < doubles; i++) {
        actions.push({
          ...
          target: "repeat-bet",
          label: "Dobrar",
          reason: `Bet Nacional 2F \xB7 Dobrar ${i + 1}/${doubles}${galeSuffix}`
        });
      }
      return {
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeBn2f = api;
  }
  var bn2f_strategy_entry_default = api;
  return __toCommonJS(bn2f_strategy_entry_exports);
})();
