importScripts("shared.js", "bn2f-engine.js", "dga-hub.js", "bn2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogBn2fAutopilotEnabled: false });
  }
  void ensureBn2fPanelOnOpenTabs();
});
...
let bn2fPanelWindowId = null;

async function openOrFocusBn2fPanel() {
  if (bn2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(bn2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      bn2fPanelWindowId = null;
    }
  }
  ...
  bn2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusBn2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === bn2fPanelWindowId) bn2fPanelWindowId = null;
});

const BN2F_DEFAULT_TABLE_ID = 225;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const BN2F_CALIB_SITE_KEY = "betnacional.bet.br|pragmatic-roulette";

function isBn2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "betnacional.bet.br" || host.endsWith(".betnacional.bet.br");
  } catch {
    return /betnacional\.bet\.br/i.test(url);
  }
}

function isBn2fRoulettePageUrl(url) {
  if (!isBn2fHost(url)) return false;
  try {
    const u = new URL(url);
    const path = `${u.pathname}${u.hash}${u.search}`.toLowerCase();
    return /\/casino\/game\/225a54/i.test(path) || /225a54/i.test(url);
  } catch {
    return /betnacional\.bet\.br/i.test(url);
  }
}

async function ensureBn2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isBn2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "bn2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-bn2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureBn2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isBn2fRoulettePageUrl(tab.url)) {
      await ensureBn2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isBn2fRoulettePageUrl(url)
  ) {
    registerMesaTab(BN2F_DEFAULT_TABLE_ID, tabId);
    void ensureBn2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-bn2f-autopilot") {
    void SinglestakeBn2fSignalRunner.setBn2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureBn2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-bn2f-autopilot") {
    void SinglestakeBn2fSignalRunner.getBn2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-bn2f-stats") {
    void SinglestakeBn2fSignalRunner.resetBn2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-bn2f-config") {
    void SinglestakeBn2fSignalRunner.getBn2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-bn2f-config") {
    void SinglestakeBn2fSignalRunner.setBn2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isBn2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(BN2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isBn2fRoulettePageUrl(t.url)) ??
  ...
  const bn2fAutopilot = await SinglestakeBn2fSignalRunner.getBn2fAutopilotStatus();
  const bn2fConfig = await SinglestakeBn2fSignalRunner.getBn2fConfigForPopup();
  return {
    ok: true,
    mode,
    bn2fAutopilot,
...
function isBn2fCrossingContext(context) {
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isBn2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.BN2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isBn2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.BN2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isBn2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "bn2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const isBn2fSignal =
    ...
    context.signalId.startsWith("bn2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isBn2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isBn2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isBn2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isBn2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isBn2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeBn2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeBn2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (BN2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isBn2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Auto Roulette no Bet Nacional num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "betnacional.bet.br" || host.endsWith(".betnacional.bet.br")) {
      return BN2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[BN2F_CALIB_SITE_KEY]) {
    return { siteKey: BN2F_CALIB_SITE_KEY, site: sites[BN2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: BN2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isBn2fHost(url)) {
    ...
    if (sites[BN2F_CALIB_SITE_KEY]) return { siteKey: BN2F_CALIB_SITE_KEY, site: sites[BN2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isBn2fHost(tab.url) ? BN2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === BN2F_CALIB_SITE_KEY && isBn2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isBn2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isBn2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isBn2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeBn2f !== "undefined" && SinglestakeBn2f.BN2F_MESA_URL) ||
          "https://betnacional.bet.br/casino/game/225a54",
        mesaProvider: null,
        currentTableId: BN2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
...
SinglestakeBn2fSignalRunner.initBn2fSignalRunner(handleBridgePayload);
