...
const GOG = {
  ...
  BN2F_FACTOR_BRIDGE_STAGGER_MS: 5,
  ...
  BN2F_DOUBLE_CLICK_STAGGER_MS: 20,
...
function bn2fStakeUnitsForRecovery(recovery) {
  ...
  return BN2F_STAKE_UNITS[Math.min(r, BN2F_STAKE_UNITS.length - 1)] ?? 1;
}

/** Gale 3 (8 unidades) — referência de ritmo de digitação ICE 3F. */
const BN2F_GALE3_REFERENCE_UNITS = 8;

/** Intervalo entre cliques de ficha — 2F (gale 4/5 fixos). */
function bn2fChipClickStaggerMs(recovery) {
  const r = Math.max(0, Math.floor(recovery));
...
function clickStaggerMsForContext(context, recovery) {
  const r =
    typeof recovery === "number" && Number.isFinite(recovery)
      ...
      : 0;
  if (isBn2fCrossingContext(context) && context?.singleFactorMode !== true) {
    return bn2fChipClickStaggerMs(r);
  }
...
function panelSignalToBridge(data) {
  ...
  return {
    ...
    context: {
      ...
      signalId,
      stakeAmount: 0.5 * bn2fStakeUnitsForRecovery(recovery),
