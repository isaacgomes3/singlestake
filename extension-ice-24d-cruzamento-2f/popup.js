...
const ice2fStatus = document.getElementById("ice24d2fStatus");
...
const ice2fOnBtn = document.getElementById("ice24d2fOn");
const ice2fOffBtn = document.getElementById("ice24d2fOff");
const ice2fResetBtn = document.getElementById("ice24d2fReset");
const ice2fCasinoId = document.getElementById("ice24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("ice24d2fSave");
const ice24d2fConfigStatus = document.getElementById("ice24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, ice24d2fConfig) {
  ...
  const noGale =
    ice24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = ice24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? ice24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.ice24d2fAutopilot, mode, status?.ice24d2fConfig);

  const cfg = status?.ice24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "ice.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.ice24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (ice24d2fConfigStatus) {
    ice24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
