...
var SinglestakeIce24d2f = (() => {
  ...
  // extension-ice-24d-cruzamento-2f/ice24d2f-strategy-entry.ts
  var ice24d2f_strategy_entry_exports = {};
  __export(ice24d2f_strategy_entry_exports, {
    ICE24D2F_MAX_GALES: () => ICE24D2F_MAX_GALES,
    ICE24D2F_MESA_URL: () => ICE24D2F_MESA_URL,
    ICE24D2F_TABLE_ID: () => ICE24D2F_TABLE_ID,
    ...
    createIce24d2fEngine: () => createIce24d2fEngine,
    default: () => ice24d2f_strategy_entry_default,
  ...
  // extension-ice-24d-cruzamento-2f/ice24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var ICE24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var ICE24D2F_MESA_URL = "https://ice.bet.br/games/tag/game-shows/24dspin-pragmaticexternal";
  var ICE24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var ICE24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var ICE24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var ICE24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of ICE24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createIce24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && ICE24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && ICE24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && ICE24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && ICE24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && ICE24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && ICE24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && ICE24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = ICE24D2F_MESA_URL) {
      ...
      const signalId = `ice24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: ICE24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? ICE24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createIce24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeIce24d2f = api;
  }
  var ice24d2f_strategy_entry_default = api;
  return __toCommonJS(ice24d2f_strategy_entry_exports);
})();
