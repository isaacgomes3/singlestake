/** Autopilot ICE — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_ICE24D2F_AUTOPLAY = "gogIce24d2fAutopilotEnabled";
const STORAGE_ICE24D2F_CONFIG = "gogIce24d2fConfig";
const STORAGE_ICE24D2F_STATUS = "gogIce24d2fAutopilotStatus";
const STORAGE_ICE24D2F_STATS = "gogIce24d2fAutopilotSessionStats";
const STORAGE_ICE24D2F_MACHINE = "gogIce24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeIce24d2f['createIce24d2fEngine']>|null} */
let engine = null;
...
function bumpIce24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function ice24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeIce24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeIce24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const ICE24D2F_DEFAULTS = {
  tableId: SinglestakeIce24d2f?.ICE24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeIce24d2f?.ICE24D2F_MESA_URL ??
    "https://ice.bet.br/games/tag/game-shows/24dspin-pragmaticexternal",
...
function knownPairIds() {
  const known = SinglestakeIce24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeIce24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeIce24d2f !== "undefined"
      ? SinglestakeIce24d2f.applyIce2fEnabledPairIds
        ? SinglestakeIce24d2f
        : SinglestakeIce24d2f.default
      : null;
...
async function readIce24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_AUTOPLAY]);
  return data[STORAGE_ICE24D2F_AUTOPLAY] === true;
}

async function readIce24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_CONFIG]);
  const stored = data[STORAGE_ICE24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...ICE24D2F_DEFAULTS,
      maxRecoveryPreference: ICE24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? ICE24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? ICE24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : ICE24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? ICE24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : ICE24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : ICE24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : ICE24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : ICE24D2F_DEFAULTS.currency,
    noGale,
...
async function readIce24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_MACHINE]);
  const raw = data[STORAGE_ICE24D2F_MACHINE];
...
async function persistIce24d2fMachineState(machine) {
...
async function clearIce24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_ICE24D2F_MACHINE]);
}

async function readIce24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_STATS]);
  const raw = data[STORAGE_ICE24D2F_STATS];
...
async function persistIce24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_ICE24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, ice24d2fIdleIndicationPatch());
  }
  await writeIce24d2fStatus(patch);
}

async function resetIce24d2fStats() {
  const cfg = await readIce24d2fConfig();
  const empty =
    globalThis.SinglestakeIce24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistIce24d2fStats(empty, cfg.maxRecovery);
  await clearIce24d2fMachineState();
...
async function getIce24d2fConfigForPopup() {
  return readIce24d2fConfig();
}

async function setIce24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_CONFIG]);
  const stored =
    data[STORAGE_ICE24D2F_CONFIG] && typeof data[STORAGE_ICE24D2F_CONFIG] === "object"
      ? data[STORAGE_ICE24D2F_CONFIG]
      : {};
  const prev = await readIce24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_ICE24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearIce24d2fMachineState();
    await writeIce24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readIce24d2fConfig()) };
}

async function writeIce24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_STATUS]);
  const prev = data[STORAGE_ICE24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_ICE24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeIce24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistIce24d2fMachineState(live.machine);
    await writeIce24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readIce24d2fConfig();
    ...
    await writeIce24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readIce24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeIce24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeIce24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeIce24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeIce24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeIce24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `ice24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistIce24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistIce24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpIce24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeIce24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeIce24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeIce24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeIce24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeIce24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startIce24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeIce24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readIce24d2fAutopilotEnabled();
  if (!enabled) {
    await writeIce24d2fStatus({ running: false, reason: "autopilot ICE 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeIce24d2f?.createIce24d2fEngine) {
    await writeIce24d2fStatus({
      running: false,
      reason: "ice24d2f-engine.js em falta — corra npm run extension:ice24d2f:build",
    });
    return;
  }

  stopIce24d2fAutopilot();

  const cfg = await readIce24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readIce24d2fStats(cfg.maxRecovery);
  const savedMachine = await readIce24d2fMachineState();
  engine = SinglestakeIce24d2f.createIce24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeIce24d2fStatus({ log: msg }),
    onStatus: (status) => void writeIce24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readIce24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readIce24d2fConfig().then((cfgNow) =>
...
function stopIce24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpIce24d2fIndicationEpoch();
  void writeIce24d2fStatus(ice24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setIce24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_ICE24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startIce24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeIce24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeIce24d2fBridgeHandler) {
      await writeIce24d2fStatus({
        running: false,
  ...
  const status = await getIce24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getIce24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_ICE24D2F_STATUS, STORAGE_ICE24D2F_AUTOPLAY, STORAGE_ICE24D2F_STATS]);
  const statsPack = data[STORAGE_ICE24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_ICE24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_ICE24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_ICE24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_ICE24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readIce24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_ICE24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readIce24d2fConfig();
  const streak =
    globalThis.SinglestakeIce24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeIce24d2f?.ice2fPairLabel === "function"
        ? SinglestakeIce24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_ICE24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_ICE24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initIce24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeIce24d2fBridgeHandler = handleBridgePayload;
  void readIce24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readIce24d2fAutopilotEnabled().then((on) => {
    if (on) void startIce24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_ICE24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_ICE24D2F_AUTOPLAY].newValue === true) {
        void startIce24d2fAutopilot(bridgeHandler);
      } else {
        stopIce24d2fAutopilot();
      }
    }
    if (changes[STORAGE_ICE24D2F_CONFIG] && bridgeHandler) {
      void readIce24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readIce24d2fAutopilotEnabled().then((on) => {
          if (on) void startIce24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeIce24d2fSignalRunner = {
    initIce24d2fSignalRunner,
    startIce24d2fAutopilot,
    stopIce24d2fAutopilot,
    setIce24d2fAutopilotEnabled,
    getIce24d2fAutopilotStatus,
    getIce24d2fConfigForPopup,
    setIce24d2fConfigFromPopup,
    resetIce24d2fStats,
    STORAGE_ICE24D2F_AUTOPLAY,
    STORAGE_ICE24D2F_CONFIG,
  };
}
