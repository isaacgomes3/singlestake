importScripts("shared.js", "ice24d2f-engine.js", "dga-hub.js", "ice24d2f-signal-runner.js");
...
async function openOrFocusIce24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (ice24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(ice24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      ice24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    ice24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: ice24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogIce24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureIce24d2fPanelOnOpenTabs();
  void openOrFocusIce24d2fPanel();
});
...
/** Janela fixa do painel ICE 24D 2F (não fecha ao clicar na mesa). */
let ice24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === ice24d2fPanelWindowId) ice24d2fPanelWindowId = null;
});

const ICE24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const ICE24D2F_CALIB_SITE_KEY = "ice.bet.br|pragmatic-24d-spin";
...
function isIce24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "ice.bet.br" || host.endsWith(".ice.bet.br"))) return false;
    return /24d[-_]?spin|24dspin|3426/i.test(path) || /\/play\/pragmatic\/24d/i.test(path) || /\/games\/tag\/.*24d/i.test(path);
  } catch {
    return /24d[-_]?spin|24dspin|3426/i.test(url);
  }
}

async function ensureIce24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isIce24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "ice24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-ice24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureIce24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isIce24d2fGameUrl(tab.url)) {
      await ensureIce24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isIce24d2fGameUrl(url)
  ) {
    registerMesaTab(ICE24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureIce24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeIce24d2fSignalRunner.setIce24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureIce24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusIce24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusIce24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeIce24d2fSignalRunner.getIce24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeIce24d2fSignalRunner.resetIce24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeIce24d2fSignalRunner.getIce24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeIce24d2fSignalRunner.setIce24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isIce24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(ICE24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isIce24d2fGameUrl(t.url)) ??
  ...
  const ice24d2fAutopilot = await SinglestakeIce24d2fSignalRunner.getIce24d2fAutopilotStatus();
  const ice24d2fConfig = await SinglestakeIce24d2fSignalRunner.getIce24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    ice24d2fAutopilot,
...
function isIce24d2fCrossingContext(context) {
  return context?.strategy === "ice24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isIce24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isIce24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isIce24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "ice24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isIce24d2fSignal =
    isIce24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("ice24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isIce24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isIce24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isIce24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isIce24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isIce24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeIce24d2f?.ice24d2fPadFactorPlacementMs === "function"
          ? SinglestakeIce24d2f.ice24d2fPadFactorPlacementMs(padUnits)
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "ice.bet.br" || host.endsWith(".ice.bet.br") || host.endsWith(".ice.bet.br")) {
      return ICE24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[ICE24D2F_CALIB_SITE_KEY]) {
    return { siteKey: ICE24D2F_CALIB_SITE_KEY, site: sites[ICE24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: ICE24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[ICE24D2F_CALIB_SITE_KEY]) return { siteKey: ICE24D2F_CALIB_SITE_KEY, site: sites[ICE24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? ICE24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === ICE24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isIce24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeIce24d2f !== "undefined" && SinglestakeIce24d2f.ICE24D2F_MESA_URL) ||
          "https://ice.bet.br/games/tag/game-shows/24dspin-pragmaticexternal",
        mesaProvider: null,
        currentTableId: ICE24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba ICE 24D encontrada. Abra o 24D Spin (ice.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeIce24d2fSignalRunner.initIce24d2fSignalRunner(handleBridgePayload);
