/** Autopilot Lotogreen — 24D Spin Cruzamento 2F (3426) · 2×4. */
const STORAGE_LOTO24D2F_AUTOPLAY = "gogLoto24d2fAutopilotEnabled";
const STORAGE_LOTO24D2F_CONFIG = "gogLoto24d2fConfig";
const STORAGE_LOTO24D2F_STATUS = "gogLoto24d2fAutopilotStatus";
const STORAGE_LOTO24D2F_STATS = "gogLoto24d2fAutopilotSessionStats";
const STORAGE_LOTO24D2F_MACHINE = "gogLoto24d2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeLoto24d2f['createLoto24d2fEngine']>|null} */
let engine = null;
...
function bumpLoto24d2fIndicationEpoch() {
  indicationEpoch += 1;
  return indicationEpoch;
}

function loto24d2fIdleIndicationPatch(extra = {}) {
  return {
    active: false,
    label: null,
    lastTrigger: null,
...
async function writeLoto24d2fIndicationStatus(patch, epoch = indicationEpoch) {
  if (epoch !== indicationEpoch) return null;
  return writeLoto24d2fStatus({ ...patch, indicationEpoch: epoch });
}

const LOTO24D2F_DEFAULTS = {
  tableId: SinglestakeLoto24d2f?.LOTO24D2F_TABLE_ID ?? 3426,
  mesaUrl:
    SinglestakeLoto24d2f?.LOTO24D2F_MESA_URL ??
    "https://lotogreen.bet.br/play/pragmatic/24d-spin",
...
function knownPairIds() {
  const known = SinglestakeLoto24d2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeLoto24d2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeLoto24d2f !== "undefined"
      ? SinglestakeLoto24d2f.applyIce2fEnabledPairIds
        ? SinglestakeLoto24d2f
        : SinglestakeLoto24d2f.default
      : null;
...
async function readLoto24d2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_AUTOPLAY]);
  return data[STORAGE_LOTO24D2F_AUTOPLAY] === true;
}

async function readLoto24d2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_CONFIG]);
  const stored = data[STORAGE_LOTO24D2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...LOTO24D2F_DEFAULTS,
      maxRecoveryPreference: LOTO24D2F_DEFAULTS.maxRecovery,
  ...
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? LOTO24D2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? LOTO24D2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : LOTO24D2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? LOTO24D2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : LOTO24D2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : LOTO24D2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : LOTO24D2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : LOTO24D2F_DEFAULTS.currency,
    noGale,
...
async function readLoto24d2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_MACHINE]);
  const raw = data[STORAGE_LOTO24D2F_MACHINE];
...
async function persistLoto24d2fMachineState(machine) {
...
async function clearLoto24d2fMachineState() {
  await chrome.storage.local.remove([STORAGE_LOTO24D2F_MACHINE]);
}

async function readLoto24d2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_STATS]);
  const raw = data[STORAGE_LOTO24D2F_STATS];
...
async function persistLoto24d2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_LOTO24D2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  ...
  if (!liveIndicationCycle()) {
    Object.assign(patch, loto24d2fIdleIndicationPatch());
  }
  await writeLoto24d2fStatus(patch);
}

async function resetLoto24d2fStats() {
  const cfg = await readLoto24d2fConfig();
  const empty =
    globalThis.SinglestakeLoto24d2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistLoto24d2fStats(empty, cfg.maxRecovery);
  await clearLoto24d2fMachineState();
...
async function getLoto24d2fConfigForPopup() {
  return readLoto24d2fConfig();
}

async function setLoto24d2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_CONFIG]);
  const stored =
    data[STORAGE_LOTO24D2F_CONFIG] && typeof data[STORAGE_LOTO24D2F_CONFIG] === "object"
      ? data[STORAGE_LOTO24D2F_CONFIG]
      : {};
  const prev = await readLoto24d2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_LOTO24D2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearLoto24d2fMachineState();
    await writeLoto24d2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readLoto24d2fConfig()) };
}

async function writeLoto24d2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_STATUS]);
  const prev = data[STORAGE_LOTO24D2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_LOTO24D2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  await writeLoto24d2fIndicationStatus(
    {
      active: true,
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistLoto24d2fMachineState(live.machine);
    await writeLoto24d2fIndicationStatus(
      {
  ...
  try {
    ...
    if (placed && engine?.markBetPlaced) {
      ...
      const cfgRetry = await readLoto24d2fConfig();
    ...
    await writeLoto24d2fIndicationStatus({ lastError: msg }, epochAtStart);
    const cfgErr = await readLoto24d2fConfig();
    void enqueueEngineResult(() =>
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeLoto24d2f?.ice2fBetDelayMs === "function"
      ? SinglestakeLoto24d2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeLoto24d2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 9500)
        : (SinglestakeLoto24d2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeLoto24d2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 9500);
...
async function scheduleBetAttempt(result, mesaUrl, cfg, epoch = indicationEpoch) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `loto24d2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistLoto24d2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistLoto24d2fStats(result.stats, cfg.maxRecovery);
  }

  let epoch = indicationEpoch;

  if (result.missedBetWindow) {
    epoch = bumpLoto24d2fIndicationEpoch();
    lastEmittedSignalId = null;
    engine.abortBetCommit?.();
    clearPendingBetTimers();
    await writeLoto24d2fIndicationStatus(
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeLoto24d2f?.ice2fWatchLabelForMachine ??
      SinglestakeLoto24d2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeLoto24d2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeLoto24d2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startLoto24d2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeLoto24d2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readLoto24d2fAutopilotEnabled();
  if (!enabled) {
    await writeLoto24d2fStatus({ running: false, reason: "autopilot Lotogreen 24D 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeLoto24d2f?.createLoto24d2fEngine) {
    await writeLoto24d2fStatus({
      running: false,
      reason: "loto24d2f-engine.js em falta — corra npm run extension:loto24d2f:build",
    });
    return;
  }

  stopLoto24d2fAutopilot();

  const cfg = await readLoto24d2fConfig();
  applyEnabledPairsFromConfig(cfg);
  const saved = await readLoto24d2fStats(cfg.maxRecovery);
  const savedMachine = await readLoto24d2fMachineState();
  engine = SinglestakeLoto24d2f.createLoto24d2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeLoto24d2fStatus({ log: msg }),
    onStatus: (status) => void writeLoto24d2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readLoto24d2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readLoto24d2fConfig().then((cfgNow) =>
...
function stopLoto24d2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  bumpLoto24d2fIndicationEpoch();
  void writeLoto24d2fStatus(loto24d2fIdleIndicationPatch({ running: false, waitingBet: false }));
}

async function setLoto24d2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_LOTO24D2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startLoto24d2fAutopilot(bridgeHandler ?? globalThis.__singlestakeLoto24d2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeLoto24d2fBridgeHandler) {
      await writeLoto24d2fStatus({
        running: false,
  ...
  const status = await getLoto24d2fAutopilotStatus();
  return { ok: true, ...status };
}

async function getLoto24d2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_LOTO24D2F_STATUS, STORAGE_LOTO24D2F_AUTOPLAY, STORAGE_LOTO24D2F_STATS]);
  const statsPack = data[STORAGE_LOTO24D2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_LOTO24D2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_LOTO24D2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_LOTO24D2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_LOTO24D2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readLoto24d2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_LOTO24D2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readLoto24d2fConfig();
  const streak =
    globalThis.SinglestakeLoto24d2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeLoto24d2f?.ice2fPairLabel === "function"
        ? SinglestakeLoto24d2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  const storedStatus = data[STORAGE_LOTO24D2F_STATUS] ?? { running: false };
  ...
  return {
    enabled: data[STORAGE_LOTO24D2F_AUTOPLAY] === true,
    status: {
      ...reconciledStatus,
      wins,
      losses,
...
function initLoto24d2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeLoto24d2fBridgeHandler = handleBridgePayload;
  void readLoto24d2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readLoto24d2fAutopilotEnabled().then((on) => {
    if (on) void startLoto24d2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_LOTO24D2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_LOTO24D2F_AUTOPLAY].newValue === true) {
        void startLoto24d2fAutopilot(bridgeHandler);
      } else {
        stopLoto24d2fAutopilot();
      }
    }
    if (changes[STORAGE_LOTO24D2F_CONFIG] && bridgeHandler) {
      void readLoto24d2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readLoto24d2fAutopilotEnabled().then((on) => {
          if (on) void startLoto24d2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeLoto24d2fSignalRunner = {
    initLoto24d2fSignalRunner,
    startLoto24d2fAutopilot,
    stopLoto24d2fAutopilot,
    setLoto24d2fAutopilotEnabled,
    getLoto24d2fAutopilotStatus,
    getLoto24d2fConfigForPopup,
    setLoto24d2fConfigFromPopup,
    resetLoto24d2fStats,
    STORAGE_LOTO24D2F_AUTOPLAY,
    STORAGE_LOTO24D2F_CONFIG,
  };
}
