...
var SinglestakeLoto24d2f = (() => {
  ...
  // extension-lotogreen-24d-cruzamento-2f/loto24d2f-strategy-entry.ts
  var loto24d2f_strategy_entry_exports = {};
  __export(loto24d2f_strategy_entry_exports, {
    ...
    LOTO24D2F_MAX_GALES: () => LOTO24D2F_MAX_GALES,
    LOTO24D2F_MESA_URL: () => LOTO24D2F_MESA_URL,
    LOTO24D2F_TABLE_ID: () => LOTO24D2F_TABLE_ID,
    ...
    createLoto24d2fEngine: () => createLoto24d2fEngine,
    default: () => loto24d2f_strategy_entry_default,
  ...
  // extension-lotogreen-24d-cruzamento-2f/loto24d2f-strategy-entry.ts
  configureIce2fDefaultComparePairs();
  var LOTO24D2F_TABLE_ID = ICE_2F_ROULETTE_TABLE_ID;
  var LOTO24D2F_MESA_URL = "https://lotogreen.bet.br/play/pragmatic/24d-spin";
  var LOTO24D2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
  ...
  var BASE_STAKE = 0.5;
  var LOTO24D2F_POSITIONS = new Set(
    ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions])
  );
  var LOTO24D2F_AXES = /* @__PURE__ */ new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
  var LOTO24D2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));
  function parsePersistedWatch(raw) {
    if (!raw || typeof raw !== "object") return null;
    const base = defaultIce2fMachineState().watch;
    for (const id of LOTO24D2F_PAIR_IDS) {
      const slot = raw[id];
  ...
  function createLoto24d2fEngine(options = {}) {
    ...
    if (options.initialMachine) {
      ...
      machine = {
        ...
        nextEntryAxis: saved.nextEntryAxis && LOTO24D2F_AXES.has(saved.nextEntryAxis) ? saved.nextEntryAxis : "cor-altura",
        lockedPosition: typeof saved.lockedPosition === "number" && LOTO24D2F_POSITIONS.has(saved.lockedPosition) ? saved.lockedPosition : null,
        ...
        gatePairId: typeof saved.gatePairId === "string" && LOTO24D2F_PAIR_IDS.has(saved.gatePairId) ? saved.gatePairId : machine.gatePairId,
    ...
    function ingestHistorySnapshot(spins) {
      ...
      if (preservedCycle?.phase === "awaiting_result") {
        ...
        const axis = (pendingRestore.nextEntryAxis && LOTO24D2F_AXES.has(pendingRestore.nextEntryAxis) ? pendingRestore.nextEntryAxis : null) ?? (pendingRestore.axis && LOTO24D2F_AXES.has(pendingRestore.axis) ? pendingRestore.axis : null) ?? "cor-altura";
        const locked = typeof pendingRestore.lockedPosition === "number" && LOTO24D2F_POSITIONS.has(pendingRestore.lockedPosition) ? pendingRestore.lockedPosition : null;
        ...
        const gatePairId = typeof pendingRestore.gatePairId === "string" && LOTO24D2F_PAIR_IDS.has(pendingRestore.gatePairId) ? pendingRestore.gatePairId : machine.gatePairId;
        pendingRestore = null;
        machine = {
          ...machine,
          nextEntryAxis: axis,
    ...
    function buildBridgePayload(mesaEmbedUrl = LOTO24D2F_MESA_URL) {
      ...
      const signalId = `loto24d2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
      ...
      return {
        ...
        context: {
          sessionMode: "active",
          prepareTableId: null,
          currentTableId: LOTO24D2F_TABLE_ID,
          mesaEmbedUrl: mesaEmbedUrl ?? LOTO24D2F_MESA_URL,
          mesaProvider: "outro",
  ...
  var api = {
    ...
    createLoto24d2fEngine
  };
  if (typeof globalThis !== "undefined") {
    globalThis.SinglestakeLoto24d2f = api;
  }
  var loto24d2f_strategy_entry_default = api;
  return __toCommonJS(loto24d2f_strategy_entry_exports);
})();
