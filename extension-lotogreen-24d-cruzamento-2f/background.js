importScripts("shared.js", "loto24d2f-engine.js", "dga-hub.js", "loto24d2f-signal-runner.js");
...
async function openOrFocusLoto24d2fPanel() {
  try {
    await chrome.action.setPopup({ popup: "" });
  } catch {
    /* ignore */
  }
  if (loto24d2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(loto24d2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return { ok: true, focused: true };
      }
    } catch {
      loto24d2fPanelWindowId = null;
    }
  }
  ...
  try {
    ...
    loto24d2fPanelWindowId = win?.id ?? null;
    return { ok: true, created: true, windowId: loto24d2fPanelWindowId };
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogLoto24d2fAutopilotEnabled: false });
  }
  void chrome.action.setPopup({ popup: "" });
  void ensureLoto24d2fPanelOnOpenTabs();
  void openOrFocusLoto24d2fPanel();
});
...
/** Janela fixa do painel Lotogreen 24D 2F (não fecha ao clicar na mesa). */
let loto24d2fPanelWindowId = null;
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === loto24d2fPanelWindowId) loto24d2fPanelWindowId = null;
});

const LOTO24D2F_DEFAULT_TABLE_ID = 3426;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const LOTO24D2F_CALIB_SITE_KEY = "lotogreen.bet.br|pragmatic-24d-spin";
...
function isLoto24d2fGameUrl(url) {
  if (!url) return false;
  ...
  try {
    ...
    if (!(host === "lotogreen.bet.br" || host.endsWith(".lotogreen.bet.br"))) return false;
...
async function ensureLoto24d2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isLoto24d2fGameUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "loto24d2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-loto24d2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureLoto24d2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isLoto24d2fGameUrl(tab.url)) {
      await ensureLoto24d2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isLoto24d2fGameUrl(url)
  ) {
    registerMesaTab(LOTO24D2F_DEFAULT_TABLE_ID, tabId);
    void ensureLoto24d2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-ice2f-autopilot") {
    void SinglestakeLoto24d2fSignalRunner.setLoto24d2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) {
        await ensureLoto24d2fPanelOnTab(sender.tab?.id ?? null);
        await openOrFocusLoto24d2fPanel();
      }
      sendResponse(resp);
    });
    return true;
  ...
  if (message.kind === "open-panel") {
    void openOrFocusLoto24d2fPanel().then((r) => sendResponse(r ?? { ok: true }));
    return true;
  }

  if (message.kind === "get-ice2f-autopilot") {
    void SinglestakeLoto24d2fSignalRunner.getLoto24d2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-ice2f-stats") {
    void SinglestakeLoto24d2fSignalRunner.resetLoto24d2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-ice2f-config") {
    void SinglestakeLoto24d2fSignalRunner.getLoto24d2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-ice2f-config") {
    void SinglestakeLoto24d2fSignalRunner.setLoto24d2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isLoto24d2fGameUrl(sender.tab.url)) {
      registerMesaTab(LOTO24D2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isLoto24d2fGameUrl(t.url)) ??
  ...
  const loto24d2fAutopilot = await SinglestakeLoto24d2fSignalRunner.getLoto24d2fAutopilotStatus();
  const loto24d2fConfig = await SinglestakeLoto24d2fSignalRunner.getLoto24d2fConfigForPopup();
  return {
    ok: true,
    mode,
    loto24d2fAutopilot,
...
function isLoto24d2fCrossingContext(context) {
  return context?.strategy === "loto24d2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isLoto24d2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.ICE2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isLoto24d2fCrossingContext(context) && action?.target === "repeat-bet") {
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isLoto24d2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "loto24d2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isLoto24d2fSignal =
    isLoto24d2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("loto24d2f:") ||
    context.signalId.startsWith("ice2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isLoto24d2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isLoto24d2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isLoto24d2fGameUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isLoto24d2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isLoto24d2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeLoto24d2f?.loto24d2fPadFactorPlacementMs === "function"
          ? SinglestakeLoto24d2f.loto24d2fPadFactorPlacementMs(padUnits)
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "lotogreen.bet.br" || host.endsWith(".lotogreen.bet.br") || host.endsWith(".lotogreen.bet.br")) {
      return LOTO24D2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[LOTO24D2F_CALIB_SITE_KEY]) {
    return { siteKey: LOTO24D2F_CALIB_SITE_KEY, site: sites[LOTO24D2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: LOTO24D2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isIceHost(url)) {
    ...
    if (sites[LOTO24D2F_CALIB_SITE_KEY]) return { siteKey: LOTO24D2F_CALIB_SITE_KEY, site: sites[LOTO24D2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isIceHost(tab.url) ? LOTO24D2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === LOTO24D2F_CALIB_SITE_KEY && isIceHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isLoto24d2fGameUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeLoto24d2f !== "undefined" && SinglestakeLoto24d2f.LOTO24D2F_MESA_URL) ||
          "https://lotogreen.bet.br/play/pragmatic/24d-spin",
        mesaProvider: null,
        currentTableId: LOTO24D2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Lotogreen 24D encontrada. Abra o 24D Spin (lotogreen.bet.br) num separador normal do Chrome.",
    };
  }
...
SinglestakeLoto24d2fSignalRunner.initLoto24d2fSignalRunner(handleBridgePayload);
