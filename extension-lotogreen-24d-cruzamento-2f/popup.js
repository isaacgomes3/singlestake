...
const ice2fStatus = document.getElementById("loto24d2fStatus");
...
const ice2fOnBtn = document.getElementById("loto24d2fOn");
const ice2fOffBtn = document.getElementById("loto24d2fOff");
const ice2fResetBtn = document.getElementById("loto24d2fReset");
const ice2fCasinoId = document.getElementById("loto24d2fCasinoId");
const maxGales = document.getElementById("maxGales");
const ice2fSaveBtn = document.getElementById("loto24d2fSave");
const loto24d2fConfigStatus = document.getElementById("loto24d2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, loto24d2fConfig) {
  ...
  const noGale =
    loto24d2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = loto24d2fConfig?.observeOnly === true || st.observeOnly === true;
  ...
  renderPairIndication(
    st.pairIndication,
    st.knownPairs,
    st.enabledPairIds ?? loto24d2fConfig?.enabledPairIds,
  );
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.loto24d2fAutopilot, mode, status?.loto24d2fConfig);

  const cfg = status?.loto24d2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "lotogreen.bet.br/play/24DSpin"}`);
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.loto24d2fConfig ?? {};
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  if (loto24d2fConfigStatus) {
    loto24d2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
