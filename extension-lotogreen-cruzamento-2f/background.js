importScripts("shared.js", "loto2f-engine.js", "dga-hub.js", "loto2f-signal-runner.js");
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    void chrome.storage.local.set({ gogLoto2fAutopilotEnabled: false });
  }
  void ensureLoto2fPanelOnOpenTabs();
});
...
let loto2fPanelWindowId = null;

async function openOrFocusLoto2fPanel() {
  if (loto2fPanelWindowId != null) {
    try {
      const existing = await chrome.windows.get(loto2fPanelWindowId);
      if (existing?.id != null) {
        await chrome.windows.update(existing.id, { focused: true });
        return;
      }
    } catch {
      loto2fPanelWindowId = null;
    }
  }
  ...
  loto2fPanelWindowId = win?.id ?? null;
}

chrome.action.onClicked.addListener(() => {
  void openOrFocusLoto2fPanel();
});

chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === loto2fPanelWindowId) loto2fPanelWindowId = null;
});

const LOTO2F_DEFAULT_TABLE_ID = 225;
/** Chave única de calibração — o ICE muda o pathname ao carregar o jogo. */
const LOTO2F_CALIB_SITE_KEY = "lotogreen.bet.br|pragmatic-roulette";

function isLoto2fHost(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === "lotogreen.bet.br" || host.endsWith(".lotogreen.bet.br");
  } catch {
    return /lotogreen\.bet\.br/i.test(url);
  }
}

function isLoto2fRoulettePageUrl(url) {
  if (!isLoto2fHost(url)) return false;
  try {
    const u = new URL(url);
    const path = `${u.pathname}${u.hash}${u.search}`.toLowerCase();
    return /\/play\/pragmatic\/auto-roulette/i.test(path);
  } catch {
    return /lotogreen\.bet\.br/i.test(url);
  }
}

async function ensureLoto2fPanelOnTab(tabId) {
  if (tabId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isLoto2fRoulettePageUrl(tab.url)) return;
    const live = await chrome.tabs
      .sendMessage(tabId, { kind: "loto2f-panel-ping" })
      .then((r) => r?.ok === true)
      .catch(() => false);
    if (live) return;
    await chrome.scripting.executeScript({
      target: { tabId },
      files: ["content-loto2f-panel.js"],
    });
  } catch {
    /* aba restrita */
  }
...
async function ensureLoto2fPanelOnOpenTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    if (tab.id != null && isLoto2fRoulettePageUrl(tab.url)) {
      await ensureLoto2fPanelOnTab(tab.id);
    }
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    isLoto2fRoulettePageUrl(url)
  ) {
    registerMesaTab(LOTO2F_DEFAULT_TABLE_ID, tabId);
    void ensureLoto2fPanelOnTab(tabId);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.kind === "set-loto2f-autopilot") {
    void SinglestakeLoto2fSignalRunner.setLoto2fAutopilotEnabled(message.enabled === true).then(async (resp) => {
      if (message.enabled === true) await ensureLoto2fPanelOnTab(sender.tab?.id ?? null);
      sendResponse(resp);
    });
    return true;
  }

  if (message.kind === "get-loto2f-autopilot") {
    void SinglestakeLoto2fSignalRunner.getLoto2fAutopilotStatus().then(sendResponse);
    return true;
  }

  if (message.kind === "reset-loto2f-stats") {
    void SinglestakeLoto2fSignalRunner.resetLoto2fStats().then(sendResponse);
    return true;
  }

  if (message.kind === "get-loto2f-config") {
    void SinglestakeLoto2fSignalRunner.getLoto2fConfigForPopup().then(sendResponse);
    return true;
  }

  if (message.kind === "set-loto2f-config") {
    void SinglestakeLoto2fSignalRunner.setLoto2fConfigFromPopup(message.config ?? {}).then(sendResponse);
    return true;
  }
  ...
  if (message.kind === "ping") {
    if (sender.tab?.id != null && isLoto2fRoulettePageUrl(sender.tab.url)) {
      registerMesaTab(LOTO2F_DEFAULT_TABLE_ID, sender.tab.id);
    }
...
async function buildStatus() {
  ...
  const tabs = await chrome.tabs.query({});
  const mesaTab =
    tabs.find((t) => t.url && isLoto2fRoulettePageUrl(t.url)) ??
  ...
  const loto2fAutopilot = await SinglestakeLoto2fSignalRunner.getLoto2fAutopilotStatus();
  const loto2fConfig = await SinglestakeLoto2fSignalRunner.getLoto2fConfigForPopup();
  return {
    ok: true,
    mode,
    loto2fAutopilot,
...
function isLoto2fCrossingContext(context) {
  return context?.strategy === "loto2fcruzamento";
}
...
function bridgeActionStaggerMs(prevAction, action, context) {
  if (
    isLoto2fCrossingContext(context) &&
    prevAction?.target === "factor-1" &&
    action?.target === "factor-2"
  ) {
    return GOG.LOTO2F_FACTOR_BRIDGE_STAGGER_MS ?? 5;
  }
  if (isLoto2fCrossingContext(context) && action?.target === "repeat-bet") {
    return GOG.LOTO2F_DOUBLE_CLICK_STAGGER_MS ?? 20;
  }
...
async function runBridgePlan(payload, sourceTabId) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    if (i > 0) {
      ...
      const motorSettled =
        isLoto2fCrossingContext(payload.context) &&
...
async function waitForCrossingBetDelay(context) {
  if (context?.strategy === "loto2fcruzamento") return;
...
async function dispatchClickAction(action, context, sourceTabId, execOpts = {}) {
  ...
  const clickDedupeKey =
    signalId && (action.target === "factor-1" || action.target === "factor-2")
      ...
      : null;
  const isLoto2fSignal =
    isLoto2fCrossingContext(context) &&
    typeof context?.signalId === "string" &&
    context.signalId.startsWith("loto2f:");
  if (clickDedupeKey && lastExecutedClickKeys.has(clickDedupeKey) && !isLoto2fSignal) {
    return {
      target: action.target,
      ok: true,
  ...
  if (clickResult.ok && clickDedupeKey && !isLoto2fSignal) {
...
async function resolveMesaTabIdForClose(tableId, mesaUrl) {
  ...
  const tabs = await chrome.tabs.query({});
  const rouletteTab = tabs.find((t) => t.url && isLoto2fRoulettePageUrl(t.url));
...
function stakeUnitsForContext(context, chip) {
  ...
  if (isLoto2fCrossingContext(context)) {
    const baseStake =
...
async function executeBetWithChip(tabId, betKey, label, dryRun, context, execOpts = {}) {
  ...
  try {
    ...
    if (isLoto2fCrossingContext(context ?? {})) {
      ...
      const padMs =
        typeof SinglestakeLoto2f?.ice2fPadFactorPlacementMs === "function"
          ? SinglestakeLoto2f.ice2fPadFactorPlacementMs(padUnits)
          : Math.max(0, (LOTO2F_GALE3_REFERENCE_UNITS - padUnits) * (GOG.CROSSING_FACTOR_CLICK_STAGGER_MS ?? 150));
...
function isCasinoPlayUrl(url) {
  return isLoto2fRoulettePageUrl(url);
}
...
async function scanExteriorBets(tabId) {
  ...
  if (id == null) {
    return { ok: false, detail: "Abra Auto Roulette no Lotogreen num separador e aguarde carregar." };
  }
...
function siteKeyFromUrl(url) {
  try {
    ...
    if (host === "lotogreen.bet.br" || host.endsWith(".lotogreen.bet.br")) {
      return LOTO2F_CALIB_SITE_KEY;
    }
...
function lookupAnyIceCalib(store) {
  ...
  if (sites[LOTO2F_CALIB_SITE_KEY]) {
    return { siteKey: LOTO2F_CALIB_SITE_KEY, site: sites[LOTO2F_CALIB_SITE_KEY] };
  }
  for (const [key, site] of Object.entries(sites)) {
    if (/ice\.bet\.br/i.test(key)) return { siteKey: key, site };
  }
  return { siteKey: LOTO2F_CALIB_SITE_KEY, site: null };
}
...
function lookupSiteCalib(store, url) {
  ...
  if (isLoto2fHost(url)) {
    ...
    if (sites[LOTO2F_CALIB_SITE_KEY]) return { siteKey: LOTO2F_CALIB_SITE_KEY, site: sites[LOTO2F_CALIB_SITE_KEY] };
  }
...
async function saveCalibrationClick(message, tabId) {
  ...
  const siteKey = isLoto2fHost(tab.url) ? LOTO2F_CALIB_SITE_KEY : siteKeyFromUrl(tab.url);
  const store = await readCalibrationStore();
  ...
  if (siteKey === LOTO2F_CALIB_SITE_KEY && isLoto2fHost(tab.url)) {
...
async function findIceCalibrationTabId(preferredId) {
  ...
  for (const t of tabs) {
    ...
    if (isLoto2fRoulettePageUrl(t.url)) scored.push({ id: t.id, score: 5, url: t.url });
    else if (isLoto2fHost(t.url)) scored.push({ id: t.id, score: 4, url: t.url });
...
async function armCalibration(betKey, label, chipValue) {
  ...
  try {
    ...
    for (const w of normalWins) {
      ...
      if (active?.id != null && (isLoto2fHost(active.url) || /pragmaticplay/i.test(active.url || ""))) {
  ...
  if (tabId == null) {
    tabId = await resolveMesaTabId(
      {
        mesaEmbedUrl:
          (typeof SinglestakeLoto2f !== "undefined" && SinglestakeLoto2f.LOTO2F_MESA_URL) ||
          "https://lotogreen.bet.br/play/pragmatic/auto-roulette",
        mesaProvider: null,
        currentTableId: LOTO2F_DEFAULT_TABLE_ID,
      },
      preferredId,
    );
  }
  ...
  if (tabId == null) {
    return {
      ok: false,
      detail:
        "Nenhuma aba Lotogreen encontrada. Abra Auto Roulette no Lotogreen num separador normal do Chrome (não neste painel).",
    };
  }
...
SinglestakeLoto2fSignalRunner.initLoto2fSignalRunner(handleBridgePayload);
