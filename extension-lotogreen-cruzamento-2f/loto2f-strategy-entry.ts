/**
 * Bundle entry — motor Lotogreen Cruzamento 2F para extensão Chrome.
 */
...
export const LOTO2F_TABLE_ID = 225;
export const LOTO2F_MESA_URL =
  "https://lotogreen.bet.br/play/pragmatic/auto-roulette";
export const LOTO2F_MAX_GALES = ICE_2F_MAX_RECOVERY;
...
const LOTO2F_POSITIONS = new Set(
  ICE_2F_KNOWN_COMPARE_PAIRS.flatMap((p) => [...p.positions]),
);
const LOTO2F_AXES = new Set(["cor-altura", "altura-paridade", "cor-paridade"]);
const LOTO2F_PAIR_IDS = new Set(ICE_2F_KNOWN_COMPARE_PAIRS.map((p) => p.id));

export type Loto2fPersistedMachine = {
...
function parsePersistedWatch(
  raw: Loto2fPersistedMachine["watch"],
...
export type CreateLoto2fEngineOptions = {
  maxRecovery?: number;
  initialStats?: RotatingRoomSessionStats | null;
  initialMachine?: Loto2fPersistedMachine | null;
};
...
function pendingRecoveryFromSaved(saved: Loto2fPersistedMachine): number {
  const stored =
...
export type Loto2fEngineSpinResult = {
...
export function createLoto2fEngine(options: CreateLoto2fEngineOptions = {}) {
  ...
  if (options.initialMachine) {
    ...
    machine = {
      ...
      nextEntryAxis:
        saved.nextEntryAxis && LOTO2F_AXES.has(saved.nextEntryAxis)
          ? saved.nextEntryAxis
          : "cor-altura",
      lockedPosition:
        typeof saved.lockedPosition === "number" &&
        LOTO2F_POSITIONS.has(saved.lockedPosition)
      ...
      pendingRecovery:
        typeof saved.pendingRecovery === "number" && Number.isFinite(saved.pendingRecovery)
          ...
          : 0,
      gatePairId:
        typeof saved.gatePairId === "string" && LOTO2F_PAIR_IDS.has(saved.gatePairId)
          ? saved.gatePairId
          : machine.gatePairId,
      ...(restoredWatch ? { watch: restoredWatch } : {}),
    };
  }
  let pendingRestore: Loto2fPersistedMachine | null = options.initialMachine ?? null;
  let stats =
  ...
  function runTick(): Loto2fEngineSpinResult {
  ...
  function ingestHistorySnapshot(spins: { number: number; gameId: string }[]) {
    ...
    if (preservedCycle?.phase === "awaiting_result") {
      ...
      const axis =
        (pendingRestore.nextEntryAxis && LOTO2F_AXES.has(pendingRestore.nextEntryAxis)
          ? pendingRestore.nextEntryAxis
          : null) ??
        (pendingRestore.axis && LOTO2F_AXES.has(pendingRestore.axis)
          ? pendingRestore.axis
          : null) ??
        "cor-altura";
      const locked =
        typeof pendingRestore.lockedPosition === "number" &&
        LOTO2F_POSITIONS.has(pendingRestore.lockedPosition)
      ...
      const gatePairId =
        ...
        LOTO2F_PAIR_IDS.has(pendingRestore.gatePairId)
  ...
  function buildBridgePayload(mesaEmbedUrl: string | null = LOTO2F_MESA_URL) {
    ...
    const signalId = `loto2f:${active.pairId ?? "pair"}:pos${active.criticalPosition}:${active.axis}:ref${active.referenceNumber}:r${recovery}:h${machine.cycle.armedHead}`;
    ...
    const actions: Array<{
    ...
    }> = [
      {
        ...
        target: "factor-1",
        label: f1Label,
        reason: `Lotogreen 2F · ${f1Label}${galeSuffix}`,
      },
      {
        kind: "click",
        target: "factor-2",
        label: f2Label,
        reason: `Lotogreen 2F · ${f2Label}${galeSuffix}`,
      },
    ];
    for (let i = 0; i < doubles; i++) {
      actions.push({
        ...
        target: "repeat-bet",
        label: "Dobrar",
        reason: `Lotogreen 2F · Dobrar ${i + 1}/${doubles}${galeSuffix}`,
      });
    }
    ...
    return {
      ...
      context: {
        ...
        maxRecovery,
        executionMode: null,
        strategy: "loto2fcruzamento" as const,
...
// named exports; sem isto SinglestakeLoto2f.applyIce2fEnabledPairIds fica undefined.
export {
  applyIce2fEnabledPairIds,
  configureIce2fComparePairs,
  configureIce2fDefaultComparePairs,
...
if (typeof globalThis !== "undefined") {
  (globalThis as unknown as { SinglestakeLoto2f?: typeof api }).SinglestakeLoto2f = api;
}
