...
const loto2fConfigStatus = document.getElementById("loto2fConfigStatus");
...
function renderKtoAutopilot(ice2f, mode, loto2fConfig) {
  ...
  const noGale =
    loto2fConfig?.noGale === true || st.noGale === true || st.maxRecovery === 0;
  const observeOnly = loto2fConfig?.observeOnly === true || st.observeOnly === true;
...
function renderStatus(status) {
  ...
  setModeUi(mode);
  renderKtoAutopilot(status?.loto2fAutopilot, mode, status?.loto2fConfig);

  const cfg = status?.loto2fConfig ?? {};
  ...
  lines.push(`Mesa: ${cfg.mesaUrl ?? "lotogreen.bet.br/liveroulettea-pragmaticexternal"}`);
  ...
  if (status?.calibration?.bets) {
    ...
    lines.push("", "Calibração: pendente — use 📍 na mesa Lotogreen 2F");
  }
  ...
  if (calStatus) {
    calStatus.textContent = status?.calibrationArmed
      ...
      : status?.calibration?.bets
        ...
        : "Abra Auto Roulette no Lotogreen num separador e aguarde carregar.";
  }
}
...
modeRealBtn?.addEventListener("click", async () => {
  if (!window.confirm("Modo REAL — a extensão vai clicar na mesa Lotogreen 2F. Confirma?")) return;
  await send("set-mode", { mode: "real" });
  await loadStatus();
});

ice2fOnBtn?.addEventListener("click", async () => {
  await send("set-loto2f-autopilot", { enabled: true });
  await loadStatus();
});

ice2fOffBtn?.addEventListener("click", async () => {
  await send("set-loto2f-autopilot", { enabled: false });
  await loadStatus();
});

ice2fResetBtn?.addEventListener("click", async () => {
  await send("reset-loto2f-stats");
  await loadStatus();
});
...
noGaleToggle?.addEventListener("change", async () => {
  ...
  const status = await send("get-status");
  const prev = status?.loto2fConfig ?? {};
  await send("set-loto2f-config", {
    config: {
      ...prev,
      noGale: want,
...
ice2fSaveBtn?.addEventListener("click", async () => {
  ...
  const r = await send("set-loto2f-config", { config });
  if (loto2fConfigStatus) {
    loto2fConfigStatus.textContent = r?.ok !== false ? "Guardado — reinicia autopilot se activo" : r?.error ?? "Erro";
  }
  await loadStatus();
});
