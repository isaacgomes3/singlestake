/** Autopilot Lotogreen — Cruzamento 2F (225) · cruzamento sequencial 2 fatores. */
const STORAGE_LOTO2F_AUTOPLAY = "gogLoto2fAutopilotEnabled";
const STORAGE_LOTO2F_CONFIG = "gogLoto2fConfig";
const STORAGE_LOTO2F_STATUS = "gogLoto2fAutopilotStatus";
const STORAGE_LOTO2F_STATS = "gogLoto2fAutopilotSessionStats";
const STORAGE_LOTO2F_MACHINE = "gogLoto2fMachineState";
...
let dgaHub = null;
/** @type {ReturnType<SinglestakeLoto2f['createLoto2fEngine']>|null} */
let engine = null;
...
const LOTO2F_DEFAULTS = {
  tableId: SinglestakeLoto2f?.LOTO2F_TABLE_ID ?? 225,
  mesaUrl:
    SinglestakeLoto2f?.LOTO2F_MESA_URL ??
    "https://lotogreen.bet.br/play/pragmatic/auto-roulette",
...
function knownPairIds() {
  const known = SinglestakeLoto2f?.ICE_2F_KNOWN_COMPARE_PAIRS;
...
function defaultEnabledPairIds() {
  const ids = SinglestakeLoto2f?.ICE_2F_DEFAULT_ENABLED_PAIR_IDS;
...
function applyEnabledPairsFromConfig(cfg) {
  ...
  const api =
    typeof SinglestakeLoto2f !== "undefined"
      ? SinglestakeLoto2f.applyIce2fEnabledPairIds
        ? SinglestakeLoto2f
        : SinglestakeLoto2f.default
      : null;
...
async function readLoto2fAutopilotEnabled() {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_AUTOPLAY]);
  return data[STORAGE_LOTO2F_AUTOPLAY] === true;
}

async function readLoto2fConfig() {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_CONFIG]);
  const stored = data[STORAGE_LOTO2F_CONFIG];
  if (!stored || typeof stored !== "object") {
    return {
      ...LOTO2F_DEFAULTS,
      maxRecoveryPreference: LOTO2F_DEFAULTS.maxRecovery,
      enabledPairIds: defaultEnabledPairIds(),
    };
  }
  const legacyWrong =
    (typeof stored.tableId === "number" && stored.tableId !== 225) ||
    (typeof stored.mesaUrl === "string" &&
      stored.mesaUrl.trim() !== "" &&
      !stored.mesaUrl.toLowerCase().includes("lotogreen.bet.br"));
  const noGale = stored.noGale === true;
  const maxRecoveryPreference = clampMaxGales(
    stored.maxRecovery ?? LOTO2F_DEFAULTS.maxRecovery,
  );
  return {
    tableId:
      legacyWrong
        ? LOTO2F_DEFAULTS.tableId
        : typeof stored.tableId === "number" && stored.tableId > 0
          ? stored.tableId
          : LOTO2F_DEFAULTS.tableId,
    mesaUrl: legacyWrong
      ? LOTO2F_DEFAULTS.mesaUrl
      : typeof stored.mesaUrl === "string" && stored.mesaUrl.trim()
        ? stored.mesaUrl.trim()
        : LOTO2F_DEFAULTS.mesaUrl,
    wsUrl:
      typeof stored.wsUrl === "string" && stored.wsUrl.trim() ? stored.wsUrl.trim() : LOTO2F_DEFAULTS.wsUrl,
    casinoId:
      typeof stored.casinoId === "string" && stored.casinoId.trim()
        ? stored.casinoId.trim()
        : LOTO2F_DEFAULTS.casinoId,
    currency:
      typeof stored.currency === "string" && stored.currency.trim()
        ? stored.currency.trim()
        : LOTO2F_DEFAULTS.currency,
    noGale,
...
async function readLoto2fMachineState() {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_MACHINE]);
  const raw = data[STORAGE_LOTO2F_MACHINE];
...
async function persistLoto2fMachineState(machine) {
...
async function clearLoto2fMachineState() {
  await chrome.storage.local.remove([STORAGE_LOTO2F_MACHINE]);
}

async function readLoto2fStats(maxRecovery) {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_STATS]);
  const raw = data[STORAGE_LOTO2F_STATS];
...
async function persistLoto2fStats(stats, maxRecovery) {
  const mr = clampMaxGales(maxRecovery);
  await chrome.storage.local.set({
    [STORAGE_LOTO2F_STATS]: { stats, maxRecovery: mr, updatedAt: new Date().toISOString() },
  });
  await writeLoto2fStatus({
    wins: stats.wins ?? 0,
...
async function resetLoto2fStats() {
  const cfg = await readLoto2fConfig();
  const empty =
    globalThis.SinglestakeLoto2f?.emptyIce2fStats?.(cfg.maxRecovery) ?? {
      wins: 0,
      losses: 0,
      winsAtRecovery: [],
      lossesAtRecovery: [],
  ...
  await persistLoto2fStats(empty, cfg.maxRecovery);
  await clearLoto2fMachineState();
...
async function getLoto2fConfigForPopup() {
  return readLoto2fConfig();
}

async function setLoto2fConfigFromPopup(config) {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_CONFIG]);
  const stored =
    data[STORAGE_LOTO2F_CONFIG] && typeof data[STORAGE_LOTO2F_CONFIG] === "object"
      ? data[STORAGE_LOTO2F_CONFIG]
      : {};
  const prev = await readLoto2fConfig();
  ...
  await chrome.storage.local.set({ [STORAGE_LOTO2F_CONFIG]: next });
  applyEnabledPairsFromConfig(next);
  if (noGale) {
    await clearLoto2fMachineState();
    await writeLoto2fStatus({ recovery: 0, waitingReference: false });
  }
  return { ok: true, ...(await readLoto2fConfig()) };
}

async function writeLoto2fStatus(patch) {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_STATUS]);
  const prev = data[STORAGE_LOTO2F_STATUS] ?? {};
  const next = { ...prev, ...patch, updatedAt: new Date().toISOString() };
  await chrome.storage.local.set({ [STORAGE_LOTO2F_STATUS]: next });
  return next;
}
...
async function executeBridgePayload(payload, mesaUrl) {
  ...
  if (observeOnly) {
    ...
    if (live?.machine) await persistLoto2fMachineState(live.machine);
    await writeLoto2fStatus({
...
function betDelayRemainingMs(result) {
  ...
  const settleMs =
    typeof SinglestakeLoto2f?.ice2fBetDelayMs === "function"
      ? SinglestakeLoto2f.ice2fBetDelayMs(recovery, immediate)
      : immediate
        ? (SinglestakeLoto2f?.ICE_2F_IMMEDIATE_REBET_DELAY_MS ?? 6000)
        : (SinglestakeLoto2f?.ROTATING_ROOM_CROSSING_BET_DELAY_MS ??
          SinglestakeLoto2f?.ICE_2F_RECOVERY_BET_DELAY_MS ?? 6000);
...
async function scheduleBetAttempt(result, mesaUrl, cfg) {
  ...
  if (!payload?.context?.signalId) {
    ...
    const signalKey = `loto2f:pos${active?.criticalPosition ?? "?"}:${active?.axis ?? "?"}:${result.recovery}`;
...
async function processEngineResultLocked(result, mesaUrl, cfg) {
  ...
  if (result.machine) {
    await persistLoto2fMachineState(result.machine);
  }
  if (result.stats) {
    await persistLoto2fStats(result.stats, cfg.maxRecovery);
  }
  ...
  if (machineState?.watch) {
    const watchLabelFn =
      SinglestakeLoto2f?.ice2fWatchLabelForMachine ??
      SinglestakeLoto2f?.formatIce2fWatchLabel;
    if (typeof watchLabelFn === "function") {
      await writeLoto2fStatus({
        watchLabel: watchLabelFn(
          SinglestakeLoto2f?.ice2fWatchLabelForMachine ? machineState : machineState.watch,
        ),
...
async function startLoto2fAutopilot(handleBridgePayload) {
  if (typeof handleBridgePayload === "function") {
    bridgeHandler = handleBridgePayload;
    globalThis.__singlestakeLoto2fBridgeHandler = handleBridgePayload;
  }
  const enabled = await readLoto2fAutopilotEnabled();
  if (!enabled) {
    await writeLoto2fStatus({ running: false, reason: "autopilot Lotogreen 2F desligado" });
    return;
  }

  if (!globalThis.SinglestakeLoto2f?.createLoto2fEngine) {
    await writeLoto2fStatus({
      running: false,
      reason: "loto2f-engine.js em falta — corra npm run extension:loto2f:build",
    });
    return;
  }
  ...
  const saved = await readLoto2fStats(cfg.maxRecovery);
  const savedMachine = await readLoto2fMachineState();
  engine = SinglestakeLoto2f.createLoto2fEngine({
    maxRecovery: cfg.maxRecovery,
  ...
  dgaHub = SinglestakeDgaHub.createDgaHub({
    ...
    onLog: (msg) => void writeLoto2fStatus({ log: msg }),
    onStatus: (status) => void writeLoto2fStatus({ dga: status }),
    onHistorySnapshot: async (_tableId, spins) => {
      if (!engine) return;
      const result = engine.ingestHistorySnapshot(spins);
      const cfgNow = await readLoto2fConfig();
    ...
    onSpin: (_tableId, spin) => {
      ...
      if (!result) return;
      void readLoto2fConfig().then((cfgNow) =>
...
function stopLoto2fAutopilot() {
  clearPendingBetTimers();
  dgaHub?.stop();
  dgaHub = null;
  engine = null;
  lastEmittedSignalId = null;
  void writeLoto2fStatus({ running: false, active: false, waitingBet: false });
}

async function setLoto2fAutopilotEnabled(enabled) {
  await chrome.storage.local.set({ [STORAGE_LOTO2F_AUTOPLAY]: enabled === true });
  if (enabled) {
    await startLoto2fAutopilot(bridgeHandler ?? globalThis.__singlestakeLoto2fBridgeHandler ?? null);
    if (!bridgeHandler && !globalThis.__singlestakeLoto2fBridgeHandler) {
      await writeLoto2fStatus({
        running: false,
  ...
  return getLoto2fAutopilotStatus();
}

async function getLoto2fAutopilotStatus() {
  const data = await chrome.storage.local.get([STORAGE_LOTO2F_STATUS, STORAGE_LOTO2F_AUTOPLAY, STORAGE_LOTO2F_STATS]);
  const statsPack = data[STORAGE_LOTO2F_STATS];
  const live = engine?.getState?.();
  const wins = live?.stats?.wins ?? statsPack?.stats?.wins ?? data[STORAGE_LOTO2F_STATUS]?.wins ?? 0;
  const losses = live?.stats?.losses ?? statsPack?.stats?.losses ?? data[STORAGE_LOTO2F_STATUS]?.losses ?? 0;
  const pairIndication = mergePairIndicationForUi(
    live?.stats?.pairIndication ??
      statsPack?.stats?.pairIndication ??
      data[STORAGE_LOTO2F_STATUS]?.pairIndication ??
      {},
  );
  const stored = data[STORAGE_LOTO2F_STATUS] ?? {};
    ...
    if (stored.waitingReference && typeof stored.recovery === "number") {
      ...
      pendingRecovery =
        typeof stored.recovery === "number"
          ? stored.recovery
          : (await readLoto2fMachineState())?.recovery ?? 0;
    }
  }
  const maxRecovery =
    live?.maxRecovery ?? statsPack?.maxRecovery ?? data[STORAGE_LOTO2F_STATUS]?.maxRecovery ?? DEFAULT_MAX_GALES;
  const cfg = await readLoto2fConfig();
  const streak =
    globalThis.SinglestakeLoto2f?.buildIce2fStreakChartMetrics?.(
      live?.stats ?? statsPack?.stats ?? { wins, losses, outcomeHistory: [] },
    ) ?? null;
  const knownPairs = knownPairIds().map((id) => ({
    id,
    label:
      typeof SinglestakeLoto2f?.ice2fPairLabel === "function"
        ? SinglestakeLoto2f.ice2fPairLabel(id)
        : String(id).replace(/x/gi, "×"),
  }));
  const enabledPairIds = normalizeEnabledPairIds(cfg.enabledPairIds);
  return {
    enabled: data[STORAGE_LOTO2F_AUTOPLAY] === true,
    status: {
      ...(data[STORAGE_LOTO2F_STATUS] ?? { running: false }),
      wins,
      losses,
      pairIndication,
      knownPairs,
...
function initLoto2fSignalRunner(handleBridgePayload) {
  bridgeHandler = handleBridgePayload;
  globalThis.__singlestakeLoto2fBridgeHandler = handleBridgePayload;
  void readLoto2fConfig().then((cfg) => applyEnabledPairsFromConfig(cfg));
  void readLoto2fAutopilotEnabled().then((on) => {
    if (on) void startLoto2fAutopilot(handleBridgePayload);
  });
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes[STORAGE_LOTO2F_AUTOPLAY] && bridgeHandler) {
      if (changes[STORAGE_LOTO2F_AUTOPLAY].newValue === true) {
        void startLoto2fAutopilot(bridgeHandler);
      } else {
        stopLoto2fAutopilot();
      }
    }
    if (changes[STORAGE_LOTO2F_CONFIG] && bridgeHandler) {
      void readLoto2fConfig().then((cfg) => {
        applyEnabledPairsFromConfig(cfg);
        return readLoto2fAutopilotEnabled().then((on) => {
          if (on) void startLoto2fAutopilot(bridgeHandler);
        });
      });
    }
  });
...
if (typeof globalThis !== "undefined") {
  globalThis.SinglestakeLoto2fSignalRunner = {
    initLoto2fSignalRunner,
    startLoto2fAutopilot,
    stopLoto2fAutopilot,
    setLoto2fAutopilotEnabled,
    getLoto2fAutopilotStatus,
    getLoto2fConfigForPopup,
    setLoto2fConfigFromPopup,
    resetLoto2fStats,
    STORAGE_LOTO2F_AUTOPLAY,
    STORAGE_LOTO2F_CONFIG,
  };
}
